	...
	while(pkt = pRakClient->Receive())
	{
		...
		switch(packetIdentifier)
		{
			...
			case ID_CONNECTION_ATTEMPT_FAILED:
				CChat::AddDebugMessage(xorstr("Отсутствует соединение. Для решения: попробуйте переподключить роутер, либо сменить интернет, либо воспользоваться VPN."));
				SetGameState(GAMESTATE_WAIT_CONNECT);
			...
			case ID_CONNECTION_REQUEST_ACCEPTED:
				CChat::AddDebugMessage(xorstr("DERIXTON SOURCE: connected..."));
				Packet_ConnectionSucceeded(pkt);
				break;
			case ID_CONNECTION_LOST:
				CChat::AddDebugMessage(xorstr("DERIXTON SOURCE: Kick unto server..."));
