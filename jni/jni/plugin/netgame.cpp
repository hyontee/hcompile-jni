...
static bool ShouldSkipRemotePlayer(uint16_t playerId)
{
	CLocalPlayer* localPlayer = CNetGame::GetPlayerPool()->GetLocalPlayer();
	if (!localPlayer) {
		return true;
	}

	return localPlayer->GetLocalPlayerID() != playerId;
}
	...
	while(pkt = pRakClient->Receive())
	{
		...
		switch(packetIdentifier)
		{
			...
			case ID_AUTH_KEY:
				...
				Packet_AuthKey(pkt); // ID_AUTH_KEY
				break;
...
void CNetGame::Packet_AimSync(Packet* pkt)
{
	...
	if (ShouldSkipRemotePlayer(playerId)) {
		return;
	}
