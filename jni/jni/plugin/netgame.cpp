...
static bool g_PendingAwaitJoin = false;
static uint64_t g_ConnectionLostAtMs = 0;
...
static uint16_t PackSampHealthArmourForBR(uint8_t value)
{
	if (value > 100) {
		value = 100;
	}
...
static bool ReadPacketPlayerHeader(Packet* pkt, RakNet::BitStream& bsData, uint8_t expectedPacketId, uint16_t& playerId)
{
	if(!pkt || !pkt->data) {
		return false;
	}
	...
	if ((uint8_t)pkt->data[0] == ID_TIMESTAMP) {
		const size_t minLength = sizeof(uint8_t) + sizeof(RakNetTime) + sizeof(uint8_t) + sizeof(uint16_t);
		if (pkt->length < minLength) {
			return false;
		}
		bsData.IgnoreBits(8);
		bsData.IgnoreBits(static_cast<int>(sizeof(RakNetTime) * 8));
	}
	if(!bsData.ReadBits((unsigned char *)&packetId, 8)) {
		return false;
	}
	if(packetId != expectedPacketId) {
		return false;
	}
	if(!bsData.ReadBits((unsigned char *)&playerId, 16)) {
		return false;
	}
	return true;
}
...
uint8_t GetPacketID(Packet *p)
{
	...
	if ((uint8_t)p->data[0] == ID_TIMESTAMP) {
		size_t offset = sizeof(uint8_t) + sizeof(RakNetTime);
		if (p->length <= offset) {
			return 255;
		}
...
{
	if (g_PendingAwaitJoin) {
		const uint64_t now = GetTickCountMS();
		if (now - g_ConnectionLostAtMs >= 15000ULL) {
			SetGameState(GAMESTATE_WAIT_CONNECT);
			g_PendingAwaitJoin = false;
		}
	}
	...
	while(pkt = pRakClient->Receive())
	{
		...
		switch(packetIdentifier)
		{
			...
			case ID_CONNECTION_ATTEMPT_FAILED:
				CChat::AddDebugMessage(xorstr("Отсутствует соединение. Для решения: попробуйте переподключить роутер, либо сменить интернет, либо воспользоваться VPN. @Fayuer"));
				SetGameState(GAMESTATE_WAIT_CONNECT);
				
				break;
			case ID_NO_FREE_INCOMING_CONNECTIONS:
				CChat::AddDebugMessage(xorstr("Сервер полон. Переподключение...@Fayuer"));
				SetGameState(GAMESTATE_WAIT_CONNECT);
				pRakClient->Disconnect(0, 0);
				break;
			case ID_CONNECTION_BANNED:
				CChat::AddDebugMessage(xorstr("Вы были заблокированы на этом сервере @Fayuer."));
				break;
			...
			case ID_CONNECTION_REQUEST_ACCEPTED:
				CChat::AddDebugMessage(xorstr("сделано by @Fayuer // tgk @GriffMobile"));
				Packet_ConnectionSucceeded(pkt);
				break;
			case ID_CONNECTION_LOST:
				CChat::AddDebugMessage(xorstr("Переподключение через 15 секунд...@Fayuer"));

				g_ConnectionLostAtMs = GetTickCountMS();
				g_PendingAwaitJoin = true;
    			break;
			case ID_DISCONNECTION_NOTIFICATION:
				CChat::AddDebugMessage(xorstr("Сервер оборвал соединение.@Fayuer"));
				CChat::AddDebugMessage(xorstr("Вы были отключены от сервера.@Fayuer"));
...
void CNetGame::Packet_ConnectionLost(Packet* pkt)
{
	...
	uintptr_t packetConnectionLostAddr = CGameAPI::GetBase(xorstr("CNetGame::Packet_ConnectionLost"));
	uintptr_t netGameAddr = CGameAPI::GetBase(xorstr("CNetGame::m_pRakClient"));
	if(!packetConnectionLostAddr || !netGameAddr) {
		return;
	}
	reinterpret_cast<void(*)(uintptr_t)>(packetConnectionLostAddr)(netGameAddr);
}
...
void CNetGame::Packet_AimSync(Packet* pkt)
{
    ...
    if(!ReadPacketPlayerHeader(pkt, bsData, ID_AIM_SYNC, playerId)) {
		return;
	}
    if(!bsData.ReadBits((unsigned char *)&aimSyncBuffer, 31 * 8)) {
		return;
	}
...
void CNetGame::Packet_PlayerSync(Packet* pkt)
{
	...
	ofSync.quaty = ty;
	ofSync.quatz = tz;
	ofSync.health = PackSampHealthArmourForBR(byteHealth);
	ofSync.armour = PackSampHealthArmourForBR(byteArmour);
...
void CNetGame::Packet_VehicleSync(Packet* pkt)
{
	...
	if(!ReadPacketPlayerHeader(pkt, bsData, ID_VEHICLE_SYNC, playerId)) {
		return;
	}
	...
	if(byteHlTemp == 0xF) {
		...
		playerHealth = byteHlTemp * 7;
	}
	icsync.playerArmour = PackSampHealthArmourForBR(playerArmour);
	icsync.playerHealth = PackSampHealthArmourForBR(playerHealth);
...
void CNetGame::Packet_PassengerSync(Packet* pkt)
{
	...
	if(!ReadPacketPlayerHeader(pkt, bsData, ID_PASSENGER_SYNC, playerId)) {
		return;
	}
	if(!bsData.ReadBits((unsigned char *)&passengerSync, 26 * 8)) {
		return;
	}
...
void CNetGame::Packet_BulletSync(Packet* pkt)
{
	...
	if(!ReadPacketPlayerHeader(pkt, bsData, ID_BULLET_SYNC, playerId)) {
		return;
	}
	if(!bsData.ReadBits((unsigned char *)&bulletSync, 40 * 8)) {
		return;
	}
...
{
	uintptr_t packetTurnLightsSyncAddr = CGameAPI::GetBase(xorstr("CNetGame::Packet_TurnLightsSync"));
	if(!packetTurnLightsSyncAddr) {
		return;
	}
	reinterpret_cast<void(*)(Packet*)>(packetTurnLightsSyncAddr)(pkt);
}
...
{
	uintptr_t packetGUIAddr = CGameAPI::GetBase(xorstr("CNetGame::Packet_GUI"));
	if(!packetGUIAddr) {
		return;
	}
    reinterpret_cast<void(*)(Packet*)>(packetGUIAddr)(pkt);
}
