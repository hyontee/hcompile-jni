	...
	while(pkt = pRakClient->Receive())
	{
...
#if __has_include(<android/log.h>)
        __android_log_print(ANDROID_LOG_INFO, "BRBridge-16.34", "SA-MP RX packet id=%u len=%u",
                            static_cast<unsigned>(packetIdentifier), static_cast<unsigned>(pkt->length));
#endif
		switch(packetIdentifier)
		{
			...
			case ID_DISCONNECTION_NOTIFICATION:
				CChat::AddDebugMessage(xorstr("Сервер оборвал соединение."));
				CChat::AddDebugMessage(xorstr("Вы были отключены от сервера."));
...
void CNetGame::Packet_AuthKey(Packet* pkt)
{
    ...
    if(byteAuthLen == 0 || byteAuthLen >= sizeof(szAuth)) {
        return;
    }
    ...
    const uint8_t keyLen = static_cast<uint8_t>(strlen(szAuthKey));
    if(keyLen == 0) {
        return;
    }
...
void CNetGame::Packet_ConnectionLost(Packet* pkt)
{
    (void)pkt;
#ifdef __aarch64__
    // BR 16.34 handles connection-lost inline inside ProcessNetwork; there is no standalone target function.
    g_ConnectionLostAtMs = GetTickCountMS();
    g_PendingAwaitJoin = true;
#else
    uintptr_t packetConnectionLostAddr = CGameAPI::GetBase(xorstr("CNetGame::Packet_ConnectionLost"));
    if(packetConnectionLostAddr) {
        reinterpret_cast<void(*)()>(packetConnectionLostAddr)();
    }
#endif
}
...
#if __has_include(<android/log.h>)
        __android_log_print(ANDROID_LOG_ERROR, "BRBridge-16.34",
                            "JOIN BLOCKED: playerPool=%p localPlayer=%p; check ARM64 pool offsets",
                            pool, pool ? pool->GetLocalPlayer() : nullptr);
#endif
		return;
	}
