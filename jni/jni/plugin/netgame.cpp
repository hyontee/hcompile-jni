    ...
    while(pkt = pRakClient->Receive())
    {
        ...
        switch(packetIdentifier)
        {
            ...
            case ID_INVALID_PASSWORD:
                ...
                break;
            case ID_AUTH_KEY:
                // 16.34 turnlights transport reuses ID_AUTH_KEY after connection.
                // Before connection it is still the normal authentication-key packet.
            ...
            case ID_CONNECTION_REQUEST_ACCEPTED:
                 CChat::AddDebugMessage(xorstr("Лилдраг чмо... не забудь от него отписаться!"));
            ...
            case 252:
{
    ...
    if(guiId == 114)
    {
        ...
        CallOnJsonDataIncoming(33, jsonData);
        delete[] jsonData;
    }
    else if(guiId == 115)
    {
        ...
        bsData.Read(jsonData, jsonLength);
        jsonData[jsonLength] = '\0';
        CallOnJsonDataIncoming(34, jsonData);
        delete[] jsonData;
    }
    else
    {
...
CChat::AddDebugMessage(xorstr("Лилдраг сосни хуйца"));
}
...
{
    uintptr_t packetTurnLightsSyncAddr =
        CGameAPI::GetBase(xorstr("CNetGame::Packet_TurnLightsSync"));
