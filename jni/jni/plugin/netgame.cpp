	...
	while(pkt = pRakClient->Receive())
	{
		...
		switch(packetIdentifier)
		{
			case ID_FAILED_INITIALIZE_ENCRIPTION:
				CChat::AddDebugMessage(xorstr("{FFD700} [SUPPORT-LIMERUSSIA] Не удалось настроить шифрование."));
				break;
			case ID_CONNECTION_ATTEMPT_FAILED:
				CChat::AddDebugMessage(xorstr("{FFD700} [SUPPORT-LIMERUSSIA] Сервер не отвечает. Переподключение..."));
				SetGameState(GAMESTATE_WAIT_CONNECT);
				
				break;
			case ID_NO_FREE_INCOMING_CONNECTIONS:
				CChat::AddDebugMessage(xorstr("{FFD700} [SUPPORT-LIMERUSSIA] Сервер полон. Переподключение..."));
				SetGameState(GAMESTATE_WAIT_CONNECT);
				pRakClient->Disconnect(0, 0);
				break;
			case ID_CONNECTION_BANNED:
				CChat::AddDebugMessage(xorstr("{FFD700} [SUPPORT-LIMERUSSIA] Вы были заблокированы на этом сервере."));
				break;
			case ID_INVALID_PASSWORD:
				CChat::AddDebugMessage(xorstr("{FFD700} [SUPPORT-LIMERUSSIA] Неверный пароль от сервера."));
			...
			case ID_CONNECTION_LOST:
				CChat::AddDebugMessage(xorstr("{FFD700} [SUPPORT-LIMERUSSIA] Переподключение через 15 секунд..."));
				Packet_ConnectionLost(pkt);
				break;
			case ID_DISCONNECTION_NOTIFICATION:
				CChat::AddDebugMessage(xorstr("{FFD700} [SUPPORT-LIMERUSSIA] Вы отключились от сервера..."));
