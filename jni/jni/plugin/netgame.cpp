    ...
    while(pkt = pRakClient->Receive())
    {
        ...
        switch(packetIdentifier)
        {
            ...
            case ID_NO_FREE_INCOMING_CONNECTIONS:
                CChat::AddDebugMessage(xorstr("Сервер полон @Fayuer"));
                SetGameState(GAMESTATE_WAIT_CONNECT);
                pRakClient->Disconnect(0, 0);
                break;
            case ID_CONNECTION_BANNED:
                CChat::AddDebugMessage(xorstr("{ff6347}Сервер закрыл соединение, можете выйти. @Fayuer"));
                break;
           ...
           //      CChat::AddDebugMessage(xorstr("Вы войшли на сервер, ожидайте загрузки."));
            ...
            case ID_CONNECTION_LOST:
                CChat::AddDebugMessage(xorstr("{ff6347}Вас отключили от сервера, можете выйти. @Fayuer"));
