...
enum BRPacketEnumeration : int32_t
{
    ...
    BR_ID_TURNLIGHTS_SYNC = 0xFD,       // custom turn signals
...
enum BRRpcIds : int32_t
{
  ...
  //  BR_RPC_ScrCustomizeVehicle = 0xA7,
    ...
    // Compatibility aliases. The old source used 0x169/0x170, which collide
    // with ShowDialog / VehicleDestroyed.
    BR_RPC_ScrCreateWayPoint = BR_RPC_CreateWaypoint,
    BR_RPC_ScrRemoveWayPoint = BR_RPC_DestroyWaypoint
};
...
static_assert(sizeof(BRInCarSyncData) == 65u, "Unexpected BRInCarSyncData layout");
#pragma pack(pop)

#pragma pack(push, 1)
// SA-MP-compatible 24-byte passenger wire layout.
// Byte 2 contains seat (6 bits), drive-by (1 bit) and CUFFED (1 bit).
// Byte 3 contains weapon (6 bits) and the two special-key bits.
struct BRPassengerSyncData
{
	uint16_t VehicleID;
	uint8_t byteSeatAndFlags;
	uint8_t byteWeaponAndSpecial;
...
static_assert(sizeof(OnFootSyncData) == 68u, "Unexpected OnFootSyncData layout");
#pragma pack(pop)
...
static_assert(sizeof(BRTurnSignalData) == 5u, "Unexpected BRTurnSignalData layout");
...
{
	uint16_t VehicleID;
	uint8_t byteSeatAndFlags; // seat:6, drive-by:1, cuffed:1
	uint8_t byteWeaponAndSpecial; // weapon:6, special-key:2
	uint8_t bytePlayerHealth;
	uint8_t bytePlayerArmour;
	uint16_t lrAnalog;
	uint16_t udAnalog;
...
static_assert(sizeof(PassengerSyncData) == 24u, "Unexpected PassengerSyncData wire layout");
#pragma pack(pop)
