...
// BR Nitro RPC (250)
//
// The important binary finding is that NEW libblackrussia-client.so has a
// single native vehicle-component routine at 0xA7F744.  The normal
// RPC_ScrCommonStuff(168), type=15/action=0 path calls it with nativeMode=2.
// The vehicle-spawn/mod-list path calls the SAME routine with nativeMode=1.
// That distinction is what we use here: we do NOT send RPC 168 and we do NOT
// route through the normal component RPC path.
//
// RPC payload:
//   uint16 vehicleId
//   uint16 componentId (1008/1009/1010)
//   uint8  nativeMode   (1; the WorldVehicleAdd/mod-list path)
// -----------------------------------------------------------------------------
static void RPC_ScrNitro_Handler(RPCParameters* rpcParams)
{
    if (!rpcParams || !rpcParams->input)
        return;

    const unsigned int bytes = (rpcParams->numberOfBitsOfData + 7u) / 8u;
    if (bytes < 4)
        return;

    RakNet::BitStream bs(rpcParams->input, bytes, false);

    uint16_t vehicleId = 0;
    uint16_t componentId = 0;
    uint8_t nativeMode = 1;

    if (!bs.Read(vehicleId) || !bs.Read(componentId))
        return;
    ...
    // For Nitro we deliberately use the mode used by the vehicle spawn/mod
    // list path in the NEW client. Do not pass mode 2 (that is RPC 168's path).
    nativeMode = 1;

    const uintptr_t base = CGameAPI::m_address;

    // NEW 64-bit lib:
    //   +0x12609C0 -> global vehicle-pool pointer
    //   +0xAA91EC -> vehicle-pool lookup(vehiclePool, vehicleId)
    //   +0xA7F744 -> vehicle component native(vehicle, componentId, mode)
    void* vehiclePool = *reinterpret_cast<void**>(base + 0x12609C0);
    if (!vehiclePool)
        return;

    using GetVehicleFn = void* (*)(void*, uint16_t);
    using VehicleComponentFn = void (*)(void*, uint16_t, uint8_t);

    auto getVehicle = reinterpret_cast<GetVehicleFn>(base + 0xAA91EC);
    auto addVehicleComponent = reinterpret_cast<VehicleComponentFn>(base + 0xA7F744);

    void* vehicle = getVehicle(vehiclePool, vehicleId);
    if (!vehicle)
        return;

    addVehicleComponent(vehicle, componentId, nativeMode);
}
...
{
#ifdef __aarch64__ 
    pInterface->RegisterAsRemoteProcedureCall(&RPC_ScrNitro, RPC_ScrNitro_Handler);

 // ===== ДОБАВЛЕНО: отсутствовало в netrpc (2).cpp =====
pInterface->RegisterAsRemoteProcedureCall(&RPC_ScrFlashGangZone, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA97C8C))); // verified from 16.34 -> current lib
pInterface->RegisterAsRemoteProcedureCall(&RPC_ConnectionRejected, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA889A4))); // verified from 16.34 -> current lib
pInterface->RegisterAsRemoteProcedureCall(&RPC_ScrSetVehicleHealth, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA8C4EC))); // verified from 16.34 -> current lib
pInterface->RegisterAsRemoteProcedureCall(&RPC_ScrDisableMapIcon, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA91984))); // verified from 16.34 -> current lib
pInterface->RegisterAsRemoteProcedureCall(&RPC_ScrClearActorAnimations, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA9119C))); // verified from 16.34 -> current lib
pInterface->RegisterAsRemoteProcedureCall(&RPC_ScrSetObjectRotation, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA8E4B4))); // verified from 16.34 -> current lib
pInterface->RegisterAsRemoteProcedureCall(&RPC_ScrSetPlayerPosFindZ, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA8DC4C))); // verified from 16.34 -> current lib
pInterface->RegisterAsRemoteProcedureCall(&RPC_ScrSetPlayerWantedLevel, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA8DAF0))); // verified from 16.34 -> current lib
pInterface->RegisterAsRemoteProcedureCall(&RPC_ScrSetPlayerVelocity, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA8C9F8))); // verified from 16.34 -> current lib
pInterface->RegisterAsRemoteProcedureCall(&RPC_ScrSetMapIcon, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA91790))); // verified from 16.34 -> current lib
pInterface->RegisterAsRemoteProcedureCall(&RPC_ScrLinkVehicle, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA8C350))); // verified from 16.34 -> current lib
pInterface->RegisterAsRemoteProcedureCall(&RPC_WorldTime, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA8CBB8))); // verified from 16.34 -> current lib
pInterface->RegisterAsRemoteProcedureCall(&RPC_SetArmedWeapon, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA8F7C0))); // verified from 16.34 -> current lib
pInterface->RegisterAsRemoteProcedureCall(&RPC_ChatBubble, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA8F8B8))); // verified from 16.34 -> current lib
 pInterface->RegisterAsRemoteProcedureCall(&RPC_ScrCreateWayPoint, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA8A624))); // verified from 16.34 -> current lib
  pInterface->RegisterAsRemoteProcedureCall(&RPC_ScrRemoveWayPoint, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA8A784))); // verified from 16.34 -> current lib
    pInterface->RegisterAsRemoteProcedureCall(&RPC_ScrCreateObject, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA8DD60)));
    pInterface->RegisterAsRemoteProcedureCall(&RPC_ScrStopObject, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA8E3E4)));
    pInterface->RegisterAsRemoteProcedureCall(&RPC_ScrMoveObject, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA97E3C)));
    ...
    pInterface->RegisterAsRemoteProcedureCall(&RPC_ScrClearPlayerAnimations, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA90B28)));
    ...
    pInterface->RegisterAsRemoteProcedureCall(&RPC_ScrApplyPlayerAnimation, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA90560)));
    ...
    pInterface->RegisterAsRemoteProcedureCall(&RPC_ScrPutPlayerInVehicle, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA8C184)));
    pInterface->RegisterAsRemoteProcedureCall(&RPC_PlayerUpdate, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA930B0)));
    pInterface->RegisterAsRemoteProcedureCall(&RPC_ScrCreate3DTextLabel, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA8A478)));
    ...
    pInterface->RegisterAsRemoteProcedureCall(&RPC_ScrGivePlayerWeapon, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA8A0A4)));
  ...
  //  pInterface->RegisterAsRemoteProcedureCall(&RPC_SvrStats, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA8D038)));
    ...
    pInterface->RegisterAsRemoteProcedureCall(&RPC_ScrVehicleParamsEx, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA8EA60)));
    pInterface->RegisterAsRemoteProcedureCall(&RPC_ScrSetPlayerAttachedObject, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA91A7C)));
    pInterface->RegisterAsRemoteProcedureCall(&RPC_StopAudioStream, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA9293C)));
    pInterface->RegisterAsRemoteProcedureCall(&RPC_ScrInterpolateCamera, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA97844)));
    pInterface->RegisterAsRemoteProcedureCall(&RPC_ScrSelectTextDraw, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA8B0D8)));
    pInterface->RegisterAsRemoteProcedureCall(&RPC_ScrApplyActorAnimation, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA90C74)));
    pInterface->RegisterAsRemoteProcedureCall(&RPC_SetActorPos, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA8D1E4)));
    pInterface->RegisterAsRemoteProcedureCall(&RPC_HideActor, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA8D038)));
    pInterface->RegisterAsRemoteProcedureCall(&RPC_SetActorHealth, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA8D0F4)));
   pInterface->RegisterAsRemoteProcedureCall(&RPC_ShowActor, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA8CEA8)));
   pInterface->RegisterAsRemoteProcedureCall(&RPC_SetActorFacingAngle, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA8D2D4)));
   pInterface->RegisterAsRemoteProcedureCall(&RPC_ScrSetPlayerName, ((void (*)(RPCParameters*))(CGameAPI::m_address + 0xA8D9DC)));  
#endif
}
