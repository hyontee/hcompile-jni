...
namespace
{
    ...
    void BR_HandleCreateWaypoint(RPCParameters* rpcParams)
    {
        CVector pos{};
        unsigned int bytes = 0;
        if (GetRpcPayload(rpcParams, bytes))
...
void RegisterRPCs(RakClientInterface* pInterface)
{
  ...
  RegisterTracked(pInterface, &RPC_ScrCreateWayPoint, ((RpcHandler)(CGameAPI::m_address + 0x4FA7A4)), BR_HandleCreateWaypoint, g_waypointHandler);
