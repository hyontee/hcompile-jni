...
static uint8_t EncodeSyncStat(uint16_t value)
{
    if (value >= 100) {
        return 0xF;
    }
    ...
    uint16_t scaled = static_cast<uint16_t>(std::lround(static_cast<float>(value) / 7.0f));
    if (scaled > 0xE) {
        scaled = 0xE;
    }
...
static uint8_t EncodeHealthArmour(uint16_t health, uint16_t armour)
{
    return static_cast<uint8_t>((EncodeSyncStat(health) << 4) | EncodeSyncStat(armour));
}
...
int ConvertBRIDToSampID(BRRpcIds value)
{
    ...
    if(value == BR_RPC_PickedUpWeapon) { return RPC_PickedUpWeapon; }
...
int NormalizeOutgoingRpcId(int value)
{
    const int mappedRpcId = ConvertBRIDToSampID(static_cast<BRRpcIds>(value));
    if (mappedRpcId != -1) {
        return mappedRpcId;
    }

    switch (value) {
        case 26:  return RPC_EnterVehicle;
        case 32:  return RPC_WorldPlayerAdd;
        case 152:  return RPC_Weather;
        case 110:  return RPC_QueueGame;
        case 156:  return RPC_ScrSetInterior;
        case 118:  return RPC_SetInteriorId;
        case 42:  return RPC_ScrStopAudioStream;
        case 29:  return RPC_SetTimeEx;
        case 41:  return RPC_ScrPlayAudioStream;
        case 11:  return RPC_ScrSetPlayerName;
        case 50:  return RPC_ServerCommand;
        case 52:  return RPC_Spawn;
        case 59:  return RPC_ChatBubble;
        case 72:  return RPC_ScrSetPlayerColor;
        case 119:  return RPC_MapMarker;
        case 62:  return RPC_DialogResponse;
        case 68:  return RPC_ScrSetSpawnInfo;
        case 83: return RPC_ClickTextDraw;
        case 95: return RPC_Pickup;
        case 63: return DestroyPickup;
        case 97:  return RPC_PickedUpWeapon;
        case 101: return RPC_Chat;
        case 108: return RPC_PickedUpWeapon;
        case 115: return RPC_PlayerGiveTakeDamage;
        case 128: return RPC_RequestClass;
        case 129: return RPC_RequestSpawn;
        case 131: return RPC_PickedUpPickup;
        case 160: return RPC_ScrSetVehicleZAngle;
        case 132: return RPC_MenuSelect;
        case 140: return RPC_MenuQuit;
        case 153: return RPC_ScrSetPlayerSkin;
        case 154: return RPC_ExitVehicle;
        case 155: return RPC_UpdateScoresPingsIPs;
        case 139: return RPC_InitGame;
        case 94: return RPC_WorldTime;
        case 163: return RPC_WorldPlayerRemove;
        case 164: return RPC_WorldVehicleAdd;
        case 165: return RPC_WorldVehicleRemove;
        default:  return -1;
    }
}
...
void ConvertBROnFootSyncToSampSync(RakNet::BitStream* bs, BROnFootSyncData data)
{
    ...
    bs->Write((const char *)&data.vecPos, sizeof(data.vecPos));
    bs->WriteNormQuat(data.quatw, data.quatx, data.quaty, data.quatz);

    const uint8_t healthArmour = EncodeHealthArmour(data.health, data.armour);
    bs->WriteBits((const unsigned char *)&healthArmour, 8);
    ...
    if (data.wSurfInfo != 0 || data.vecSurfOffsets.x != 0.0f || data.vecSurfOffsets.y != 0.0f || data.vecSurfOffsets.z != 0.0f) {
        bs->Write1();
        bs->WriteBits((const unsigned char *)&data.wSurfInfo, 16);
        bs->WriteBits((const unsigned char *)&data.vecSurfOffsets.x, 32);
        bs->WriteBits((const unsigned char *)&data.vecSurfOffsets.y, 32);
        bs->WriteBits((const unsigned char *)&data.vecSurfOffsets.z, 32);
    } else {
        bs->Write0();
    }
}
...
void ConvertBRInCarSyncToSampSync(RakNet::BitStream* bs, BRInCarSyncData data)
{
    ...
    const uint16_t vehicleHealth = static_cast<uint16_t>(data.fCarHealth < 0.0f ? 0.0f : data.fCarHealth);
    bs->WriteBits((const unsigned char *)&vehicleHealth, 16);

    const uint8_t healthArmour = EncodeHealthArmour(data.playerHealth, data.playerArmour);
    bs->WriteBits((const unsigned char *)&healthArmour, 8);

    const uint8_t weapon = static_cast<uint8_t>(data.byteCurrentWeapon & 0x3F);
    bs->WriteBits((const unsigned char *)&weapon, 8);

    const bool sirenOn = data.byteSirenOn != 0;
    const bool landingGear = data.byteLandingGearState != 0;
    const bool hasTrailer = data.TrailerID != 0;
    bs->WriteCompressed(sirenOn);
    bs->WriteCompressed(landingGear);
    bs->WriteCompressed(hasTrailer);
    if (hasTrailer) {
...
void ConvertBRPassengerSyncToSampSync(RakNet::BitStream* bs, BRPassengerSyncData data)
{
    ...
    bs->WriteBits((const unsigned char *)&data.byteCurrentWeapon, 8);
    const uint8_t playerHealth = static_cast<uint8_t>(data.playerHealth > 100 ? 100 : data.playerHealth);
    const uint8_t playerArmour = static_cast<uint8_t>(data.playerArmour > 100 ? 100 : data.playerArmour);
