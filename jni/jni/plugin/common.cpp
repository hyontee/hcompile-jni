...
void FixBrokenRPC(int rpcId, RPCParameters* rpcParams, void (*staticFunc)(RPCParameters*))
{
	...
	if(rpcId == RPC_ServerJoin) {
        ...
        char nickName[25] = {};
        uint16_t playerId = 0;
        if (!bsData.Read(playerId) || bsData.GetNumberOfUnreadBits() < 48u) return;
        bsData.IgnoreBits(40);
        bsData.Read(nickNameLen);
        const uint8_t safeNickNameLen = (nickNameLen < sizeof(nickName)) ? nickNameLen : static_cast<uint8_t>(sizeof(nickName) - 1);
        if (bsData.GetNumberOfUnreadBits() < static_cast<unsigned int>(nickNameLen) * 8u) return;
        if (safeNickNameLen > 0) bsData.Read(nickName, safeNickNameLen);
        if (nickNameLen > safeNickNameLen) bsData.IgnoreBits(static_cast<unsigned int>(nickNameLen - safeNickNameLen) * 8u);
        nickName[safeNickNameLen] = 0;
        nickNameLen = safeNickNameLen;
