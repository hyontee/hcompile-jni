...
static void UpdateRemotePlayerName(CRemotePlayer* remotePlayer, const char* name, size_t nameLen)
{
    if (!remotePlayer) {
        return;
    }

    std::memset(remotePlayer->m_szName, 0, sizeof(remotePlayer->m_szName));
    if (!name || nameLen == 0) {
        return;
    }

    if (nameLen >= sizeof(remotePlayer->m_szName)) {
        nameLen = sizeof(remotePlayer->m_szName) - 1;
    }

    std::memcpy(remotePlayer->m_szName, name, nameLen);
}
...
{
    bs->WriteBits((const unsigned char *)&data.VehicleID, 16);
	uint8_t combinedByte = static_cast<uint8_t>((data.byteSeatFlags & 0x7F) | ((data.byteDriveBy & 0x01) << 7));
...
void FixBrokenRPC(int rpcId, RPCParameters* rpcParams, void (*staticFunc)(RPCParameters*))
{
	...
	if(rpcId == RPC_ServerJoin) {
        ...
        const uint8_t maxNameLen = static_cast<uint8_t>(sizeof(nickName) - 1);
        uint8_t safeLen = nickNameLen;
        if (safeLen > maxNameLen) {
            safeLen = maxNameLen;
        }

        if (safeLen > 0) {
            if (!bsData.Read(nickName, safeLen)) {
                return;
            }
        }
        nickName[safeLen] = 0;

        if (nickNameLen > safeLen) {
            bsData.IgnoreBits((nickNameLen - safeLen) * 8);
        }
        ...
        if (pool) {
            ...
            UpdateRemotePlayerName(remote_player, nickName, safeLen);
        }
        return;
    }
