...
void FixBrokenRPC(int rpcId, RPCParameters* rpcParams, void (*staticFunc)(RPCParameters*))
{
	...
	if(rpcId == RPC_ServerJoin) {
		...
		// fix: nickNameLen приходит от сервера (0..255) и не проверялся,
		// из-за чего Read()/nickName[nickNameLen]=0 переполняли 25-байтный
		// стековый буфер nickName -> stack corruption detected / SIGABRT.
		if (nickNameLen > sizeof(nickName) - 1) {
			nickNameLen = sizeof(nickName) - 1;
		}
