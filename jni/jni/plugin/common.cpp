...
static bool IsValidNameChar(unsigned char ch)
{
    return std::isalnum(ch) || ch == '_' || ch == '[' || ch == ']' || ch == '.';
}

static void ApplySanitizedName(CRemotePlayer* player, const char* name, size_t len)
{
    if(!player || !name) {
        return;
    }
    ...
    size_t out = 0;
    for(size_t i = 0; i < len && out + 1 < sizeof(player->m_szName); ++i) {
        unsigned char ch = static_cast<unsigned char>(name[i]);
        if(ch == '$' || ch == '%') {
            break;
        }
        if(IsValidNameChar(ch)) {
            player->m_szName[out++] = ch;
        }
    }
    player->m_szName[out] = 0;
}

static uint8_t NormalizeHealthArmourToByte(uint16_t value)
{
    if (value <= 100) {
    ...
    if ((value & 0x00FFu) == 0u) {
        uint16_t high = static_cast<uint16_t>(value >> 8);
        if (high <= 100) {
            return static_cast<uint8_t>(high);
        }
    }

    return static_cast<uint8_t>(std::min<uint16_t>(100, value & 0x00FFu));
}

int ConvertBRIDToSampID(BRRpcIds value)
{
    if(value == BR_RPC_ScrCreateWayPoint) { return RPC_ScrCreateWayPoint; }
    if(value == BR_RPC_ScrRemoveWayPoint) { return RPC_ScrRemoveWayPoint; }
...
{
    uint8_t byteHealth = NormalizeHealthArmourToByte(data.health);
    uint8_t byteArmour = NormalizeHealthArmourToByte(data.armour);
    ...
    bs->WriteBits((const unsigned char *)&byteHealth, 8);
    bs->WriteBits((const unsigned char *)&byteArmour, 8);
...
{
    uint8_t byteHealth = NormalizeHealthArmourToByte(data.playerHealth);
    uint8_t byteArmour = NormalizeHealthArmourToByte(data.playerArmour);
...
bool IsRPCNeedFix(int rpcId)
{
	...
	if(rpcId == RPC_ScrSetPlayerName) { return true; }
...
void FixBrokenRPC(int rpcId, RPCParameters* rpcParams, void (*staticFunc)(RPCParameters*))
{
	...
	if(rpcId == RPC_ServerJoin) {
		...
		uint8_t readLen = nickNameLen;
		if(readLen > sizeof(nickName) - 1) {
			readLen = sizeof(nickName) - 1;
		}
		if(!bsData.Read(nickName, readLen)) {
			return;
		}
		nickName[readLen] = 0;
		...
		if(pool) {
			...
			ApplySanitizedName(remote_player, nickName, readLen);
		}
		return;
	}
	if(rpcId == RPC_ScrSetPlayerName) {
		staticFunc(rpcParams);
		uint16_t playerId = 0;
		uint8_t nickNameLen = 0;
		char nickName[256] = {0};
