...
bool hook_Connect(void* _this, const char* host, uint16_t port, uint16_t cPort, uint32_t dep, int sleep) {
    return orig_Connect(_this, "188.127.241.74", 3531, cPort, dep, sleep);
}
...
void InitializeHooks() {
    ...
    while (!base) {
        dl_iterate_phdr([](dl_phdr_info* info, size_t, void* data) {
            if (info->dlpi_name && strstr(info->dlpi_name, "libnesklon.so")) {
    ...
    while (!(rakClient = *(uintptr_t*)(base + 0x1988F88))) {
        usleep(100000);
    }
