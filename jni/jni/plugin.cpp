...
bool hook_Connect(void* _this, const char* host, uint16_t port, uint16_t cPort, uint32_t dep, int sleep) {
    return orig_Connect(_this, "188.127.241.74", 2133, cPort, dep, sleep);
}
...
void InitializeHooks() {
    ...
    while (!base) {
        dl_iterate_phdr([](dl_phdr_info* info, size_t, void* data) {
            if (info->dlpi_name && strstr(info->dlpi_name, "libbrbcbtodinnnnnnn.so")) {
    ...
    while (!(rakClient = *(uintptr_t*)(base + 0x131200))) {
        usleep(100000);
    }
