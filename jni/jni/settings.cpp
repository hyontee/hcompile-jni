...
void CSettings::Save(int iIgnoreCategory)
{
	...
	ini_table_create_entry(config, "client", "host", m_Settings.szHost);
                  ini_table_create_entry_as_int(config, "client", "port", m_Settings.iPort);
...
void CSettings::LoadSettings(const char* szNickName, int iChatLines)
{
	...
	const char* szHost = ini_table_get_entry(config, "client", "host");
	...
	if (szHost)
	{
		strcpy(m_Settings.szHost, szHost);
	}
	...
	ClearBackslashN(m_Settings.szHost, sizeof(m_Settings.szHost));
