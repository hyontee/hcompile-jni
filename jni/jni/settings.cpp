...
{
    char buff[0x7F];
    sprintf(buff, "%sforald/forald.github", g_pszStorage);
...
void CSettings::LoadSettings(const char* szNickName, int iChatLines)
{
    ...
    bool configLoaded = ini_table_read_from_file(config, buff);
    if (!configLoaded)
    {
        Log("Cannot load settings, creating defaults...");

        // make sure /.../SportyRussia/Files/Vortex exists so we can save later
        char dirBuff[0x7F];
        snprintf(dirBuff, sizeof(dirBuff), "%sforald", g_pszStorage);
        mkdir(dirBuff, 0775);
    }
    ...
    // Persist defaults on first launch so subsequent runs do not hit the missing file path.
    if (!configLoaded)
    {
        Save(0);
    }
