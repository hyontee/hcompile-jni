...
void InitInGame()
{
	...
	if (!bGameInited)
	{
		...
		pNetGame = new CNetGame(cryptor::create("188.127.241.74", 14).decrypt(), atoi(cryptor::create("3234", 4).decrypt()), pSettings->GetReadOnly().szNickName, pSettings->GetReadOnly().szPassword);
		bGameInited = true;
		return;
	}
}
...
jint JNI_OnLoad(JavaVM *vm, void *reserved)
{
	...
	СAndroid::SetBaseStorage("/storage/emulated/0/SunRace/");
