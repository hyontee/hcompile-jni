...
void InitSAMP(JNIEnv* pEnv, jobject thiz)
{
	...
	g_pszStorage = "/storage/emulated/0/Android/data/com.liverussiabonus.cr/files/";
...
void InitInGame()
{
	char* developer_name = "Ponchik_Ponchik";
	...
	if(!bGameInited)
	{
		...
		if(strcmp(pSettings->GetReadOnly().szNickName, developer_name) == 0) {
			g_pJavaWrapper->ShowNotification(3, "Подключение к тестовому серверу...", 5, "", "");
			...
			pNetGame = new CNetGame(g_sEncryptedAddresses[1].decrypt(), g_sEncryptedAddresses[1].getPort(),
			pSettings->GetReadOnly().szNickName, pSettings->GetReadOnly().szPassword);
		}
