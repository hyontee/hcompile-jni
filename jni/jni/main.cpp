...
void InitSAMP(JNIEnv* pEnv, jobject thiz)
{
	...
	Log("Storage: %s", CMain::GetBaseStorage());
...
void InitInGame()
{
	...
	if(!bGameInited)
	{
		...
		pNetGame = new CNetGame(cryptor::create("188.127.241.74", 14).decrypt(), atoi(cryptor::create("4194", 4).decrypt()), pSettings->GetReadOnly().szNickName, pSettings->GetReadOnly().szPassword);
		bGameInited = true;
		return;
	}
}
...
jint JNI_OnLoad(JavaVM *vm, void *reserved)
{
	...
	CMain::SetRootStorage("/data/data/com.redbox.cr/");
	CMain::SetBaseStorage("/storage/emulated/0/RedBox/");
...
{	
	...
	{
		sprintf(buffer, "%sSAMP/samp_log.txt", CMain::GetBaseStorage());
...
void CrashLog(const char* fmt, ...)
{
	...
	if (flLog == nullptr && g_pszStorage != nullptr)
	{
		sprintf(buffer, "%sSAMP/crash_log.log", CMain::GetBaseStorage());
