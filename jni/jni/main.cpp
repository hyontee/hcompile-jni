...
void InitSAMP(JNIEnv* pEnv, jobject thiz)
{
	...
	g_pszStorage = "/storage/emulated/0/Golda/";
	if(!g_pszStorage)
	{
...
void InitInGame()
{
	...
	if (!bNetworkInited)
	{
		...
		pNetGame = new CNetGame("188.127.241.74", 2775, pSettings->Get().szNickName, pSettings->Get().szPassword); // тут указывается ип
...
void Log(const char *fmt, ...)
{
	...
	{
		sprintf(buffer, "%s/%s", g_pszStorage, logName);
...
void CrashLog(const char* fmt, ...)
{
	...
	if (flLog == nullptr && g_pszStorage != nullptr)
	{
		sprintf(buffer, obfuscate("%s/%s"), g_pszStorage, crashLogName);
