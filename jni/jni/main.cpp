...
// Solar Russia server endpoint configured by the Android launcher through JNI.
static char g_SolarServerHost[128] = "YOUR_SERVER_IP";
static int g_SolarServerPort = 7777;
...
extern "C"
{
	...
	// Called by Solar Russia Launcher before the game Activity starts.
	JNIEXPORT void JNICALL Java_com_grand_launcher_activity_NativeBridge_nativeConfigureServer(
		JNIEnv* env, jclass, jstring host, jint port)
	{
		if (host == nullptr)
			return;

		const char* utfHost = env->GetStringUTFChars(host, nullptr);
		if (utfHost == nullptr)
			return;

		strncpy(g_SolarServerHost, utfHost, sizeof(g_SolarServerHost) - 1);
		g_SolarServerHost[sizeof(g_SolarServerHost) - 1] = '\\0';
		g_SolarServerPort = (int)port;

		env->ReleaseStringUTFChars(host, utfHost);
		Log("Solar Russia JNI: server=%s:%d", g_SolarServerHost, g_SolarServerPort);
	}
}
...
void InitSAMP(JNIEnv* pEnv, jobject thiz)
{
	...
	g_pszStorage = "/storage/emulated/0/SolarRussia/";
