...
void PrintBuildInfo()
{
	 	Log(OBFUSCATE("Client by EDGAR 3.0"));
		Log(OBFUSCATE("SA-MP version: %s"), SAMP_VERSION);
		Log(OBFUSCATE("Build times: %s %s"), __TIME__, __DATE__);
}
...
{
	CrashLog(OBFUSCATE("КЛИЕНТ SPACE RP | SPACE RP CLIENT"));
	CrashLog(OBFUSCATE("Время создания | Build time: " __DATE__ " " __TIME__));

	//CrashLog(OBFUSCATE("Версия клиента | SA-MP version: %s"), SAMP_VERSION);
	CrashLog(OBFUSCATE("Последний использованный объект | Last rendered object: %d"), g_iLastRenderedObject);
	CrashLog(OBFUSCATE("Последнее использованное авто | Last rendered vehicle: %d"), g_usLastProcessedModelIndexAutomobile);
	CrashLog(OBFUSCATE("Последний использованный скин | Last rendered skin: %d"), g_iLastProcessedSkinCollision);

	CrashLog(OBFUSCATE("Последнее обработанное авто и объект: %d %d | Last processed auto and entity: %d %d"), g_usLastProcessedModelIndexAutomobile, g_iLastProcessedModelIndexAutoEnt, g_usLastProcessedModelIndexAutomobile, g_iLastProcessedModelIndexAutoEnt);
	CrashLog(OBFUSCATE("Последний обработанный скин и объект: %d %d | Last processed skin and entity: %d %d"), g_iLastProcessedSkinCollision, g_iLastProcessedEntityCollision, g_iLastProcessedSkinCollision, g_iLastProcessedEntityCollision);
}
...
void InitInMenu()
{
	...
	pTurnLeftAndRight = new CTurnLeftAndRight();
...
void handler3(int signum, siginfo_t* info, void* contextPtr)
{
	...
	if (info->si_signo == SIGBUS)
	{
		...
		CrashLog("Последний отрисованный объект | Last rendered object: %d", g_iLastRenderedObject);
...
{	
	...
	{
		//sprintf(buffer, "%sSPACE/samp_log.txt", g_pszStorage);
...
void CrashLog(const char* fmt, ...)
{
	...
	if (flLog == nullptr && g_pszStorage != nullptr)
	{
		//sprintf(buffer, "%sSPACE/crash_log.log", g_pszStorage);
		//flLog = fopen(buffer, "a");
	}
