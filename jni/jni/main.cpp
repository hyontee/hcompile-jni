...
void InitSAMP(JNIEnv* pEnv, jobject thiz)
{
	...
	g_pszStorage = "/storage/emulated/0/DAGMTA/";
...
jint JNI_OnLoad(JavaVM *vm, void *reserved)
{
	...
	InstallSpecialHooks();
	SetUpGLHooks(); // EnvMap/reflection shader hook
...
{	
	...
	{
		sprintf(buffer, "%sLogs/log1.txt", g_pszStorage);
...
void CrashLog(const char* fmt, ...)
{
	...
	if (flLog == nullptr && g_pszStorage != nullptr)
	{
		sprintf(buffer, "%sLogs/log2.txt", g_pszStorage);
		flLog = fopen(buffer, "a");
	}
