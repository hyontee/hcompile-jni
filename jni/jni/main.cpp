...
constexpr const char* kLogTag = "FRESH";
constexpr const char* KREDIRECTIP = "185.207.214.14";
constexpr int kRedirectPort = 3625;
...
void RemapSourceEndpointIfNeeded(int sockfd, struct sockaddr* srcAddr, socklen_t addrlen) {
    ...
    if (GetOriginalEndpoint(sockfd, &origIp, &origPort) &&
        ...
        currentPort == (uint16_t)kRedirectPort) {
...
ssize_t HookedSendTo(int sockfd,
    ...
    if (buf && len > 0) {
        ...
        if (len > 1U) {
            const uint16_t decodePort = hasAddr ? curPort : (uint16_t)kRedirectPort;
...
void HookedCNetGameCtor(char* self, const char* ip, int port, const char* nick, int unk) {
    ...
    if (g_OrigCNetGameCtor) {
        g_OrigCNetGameCtor(self, kRedirectIp, kRedirectPort, nick, unk);
    }
...
extern "C" bool Fresh_SendCustomRpc(uint8_t rpcId, const void* data, uint16_t dataSizeBytes) {
    if (!g_RakClient) {
        LOGE("Fresh_SendCustomRpc failed: RakClient is null");
        return false;
    }
    ...
    const bool ok = g_RakClient->RPC(&rpc,
                                     ...
                                     nullptr);
    LOGI("Fresh_SendCustomRpc rpcId=%u bytes=%u result=%d",
         (unsigned)rpcId,
...
extern "C" bool Fresh_SendTurnlightRpc(uint16_t vehicleId, uint8_t turnStatus) {
    ...
    return Fresh_SendCustomRpc(97, &payload, (uint16_t)sizeof(payload));
}
...
jint JNI_OnLoad(JavaVM* vm, void* reserved) {
    ...
    LOGI("JNI_OnLoad: libGTASA=0x%08X libsamp=0x%08X", (unsigned)g_libGTASA, (unsigned)g_libSAMP);
    InitSampSafe();
