...
void InitSAMP(JNIEnv* pEnv, jobject thiz)
{
	...
	g_pszStorage = "/storage/emulated/0/Horizont/";
...
void InitInGame()
{
	...
	if (!bGameInited)
	{
		...
		pNetGame = new CNetGame(cryptor::create("188.127.241.74", 15).decrypt(), atoi(cryptor::create("1972", 4).decrypt()), pSettings->GetReadOnly().szNickName, pSettings->GetReadOnly().szPassword);
