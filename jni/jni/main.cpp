...
extern "C" bool Custom_SendCustomRpc(uint8_t rpcId, const void* data, uint16_t dataSizeBytes);
...
// коннект к первому серверу
constexpr const char* MainServerConnectIp = "188.127.241.74:1972"; // айпи первого сервера
constexpr uint16_t kMainServerConnectPort = 1972; // порт первого сервера
// Коннект ко второму серверу
constexpr const char* NextServerConnectIp = "188.127.241.74"; // айпи второго сервера
constexpr uint16_t NextServerConnectPort = 1972; // порт второго сервера
...
constexpr const char* kLogTag = "derixton_rizone_v3";
#define LOGI(...) __android_log_print(ANDROID_LOG_INFO, kLogTag, __VA_ARGS__)
#define LOGE(...) __android_log_print(ANDROID_LOG_ERROR, kLogTag, __VA_ARGS__)
// ��� �� �����, � �� �������� ������� (������������� ��� ���������)
constexpr const char* ConnectToServerIp = "46.174.53.161";
constexpr uint16_t ConnectToServerPort = 7777;
constexpr const char* kClientHelloPayload = "hi";
inline bool IsConnectToServer(in_addr_t ip, uint16_t port) {
    return ip == inet_addr(ConnectToServerIp) && port == ConnectToServerPort;
}

inline bool IsManagedRedirectPort(uint16_t port) {
    return port == NextServerConnectPort ||            
           port == kMainServerConnectPort;
}

inline void ResolveRedirectEndpoint(in_addr_t originalIp, uint16_t originalPort, 
                                     in_addr_t* outIp, uint16_t* outPort) {
    if (IsConnectToServer(originalIp, originalPort)) {
        *outIp = inet_addr(NextServerConnectIp);
        *outPort = NextServerConnectPort;
        return;
    }
    
    *outIp = inet_addr(MainServerConnectIp);
    *outPort = kMainServerConnectPort;
}
...
constexpr int kOfflinePlayerSkin = 5;
constexpr int kOfflineVehicleModel = 400;
...
bool SendGraphicsSettingsToBr() {
    ...
    const bool ok = Custom_SendCustomRpc(kIdCustomRpc,
                                       payload.GetData(),
...
bool SendClientGuardResponse(uint32_t challenge) {
    ...
    const bool ok = Custom_SendCustomRpc(kClientGuardRpcId,
                                       payload.GetData(),
                                       (uint16_t)payload.GetNumberOfBytesUsed());
...
ssize_t HookedSendTo(int sockfd,
    ...
    if (hasAddr) {
        in_addr_t redirectIp = 0;
        uint16_t redirectPort = 0;
        
        if (IsConnectToServer(curIp, curPort)) {
            redirectIp = inet_addr(NextServerConnectIp);
            redirectPort = NextServerConnectPort;        
        } else if (IsManagedRedirectPort(curPort)) {
            return g_OrigSendTo(sockfd, buf, len, flags, destAddr, addrlen);
        } else {
            redirectIp = inet_addr(MainServerConnectIp);
            redirectPort = kMainServerConnectPort;
        }
        
        if (curIp != redirectIp) {
            redirectedDest = *reinterpret_cast<const sockaddr_in*>(destAddr);
            redirectedDest.sin_addr.s_addr = redirectIp;
            redirectedDest.sin_port = htons(redirectPort);
    ...
    return g_OrigSendTo(sockfd, buf, len, flags, finalDestAddr, finalAddrLen);
}
...
int HookedConnect(int sockfd, const struct sockaddr* addr, socklen_t addrlen) {
    ...
    if (IsConnectToServer(in->sin_addr.s_addr, port)) {
        redirectIp = inet_addr(NextServerConnectIp);
        redirectPort = NextServerConnectPort;
    } else {
        redirectIp = inet_addr(MainServerConnectIp);
    ...
    out.sin_port = htons(redirectPort);
...
void HookedCNetGameCtor(char* self, const char* ip, int port, const char* nick, int unk) {
    ...
    const uint16_t originalPort = (port > 0 && port <= 65535) ? (uint16_t)port : kMainServerConnectPort;
    ...
    char redirectIpStr[INET_ADDRSTRLEN];
    inet_ntop(AF_INET, &redirectIp, redirectIpStr, INET_ADDRSTRLEN);
    ...
    if (g_OrigCNetGameCtor) {
        g_OrigCNetGameCtor(self, redirectIpStr, (int)redirectPort, nick, unk);
    }
...
extern "C" bool Custom_SendCustomRpc(uint8_t rpcId, const void* data, uint16_t dataSizeBytes) {
    if (!g_RakClient) {
        LOGE("Custom_SendCustomRpc failed: RakClient is null");
        return false;
    }
    ...
    const bool ok = g_RakClient->RPC(&rpc,
                                     ...
                                     nullptr);
    LOGI("Custom_SendCustomRpc rpcId=%u bytes=%u result=%d",
         (unsigned)rpcId,
         (unsigned)dataSizeBytes,
         ok ? 1 : 0);
    return ok;
...
extern "C" bool Custom_SendTurnlightRpc(uint16_t vehicleId, uint8_t turnStatus) {
    ...
    return Custom_SendCustomRpc(97, &payload, (uint16_t)sizeof(payload));
}
...
jint JNI_OnLoad(JavaVM* vm, void* reserved) {
    ...
    g_libBR = FindLibrary(OBFUSCATE("libBlackrussia-client.so"));
    if (!g_libBR) {
        g_libBR = FindLibrary(OBFUSCATE("libblackrussia-client.so"));
    }
