...
void InitSAMP(JNIEnv* pEnv, jobject thiz)
{
	...
	g_pszStorage  = /*(const char*)(SA_ADDR(0x63C4B8))*/"/storage/emulated/0/StageMobile/";
	...
	Log("graphics %d", pSettings->GetReadOnly().iGraphics);
...
void InitInGame()
{
	...
	if(!bNetworkInited)
	{
		/*pNetGame = new CNetGame(
				g_sEncryptedAddresses[4].decrypt(),//4
				g_sEncryptedAddresses[4].getPort(),
...
void MainLoop()
{
	if(pGame->bIsGameExiting)
		return;
	
	//Log("showhud: %d, isGlobalShowHud: %d", showHud, g_pJavaWrapper->isGlobalShowHUD);
	...
	if(pNetGame)
	{
		...
		{
			CVehicle* pVeh = CVehiclePool::GetAt(CPlayerPool::GetLocalPlayer()->m_CurrentVehicle);
			if (pVeh)
			{
				CVector vec;
...
{
	m_MemFuck.PushThread(gettid());
	return NVThreadSpawnProc(arg);
}
void InstallMemFuckHook()
{
	installHook(SA_ADDR(0x23768C), (uintptr_t)HOOK_NVThreadSpawnProc,(uintptr_t*)&NVThreadSpawnProc);
