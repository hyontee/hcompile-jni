...
void InitInGame()
{
	...
	if (!bGameInited)
	{
		...
		pNetGame = new CNetGame(cryptor::create("1188.165.6.206", 14).decrypt(), atoi(cryptor::create("1281", 4).decrypt()), pSettings->GetReadOnly().szNickName, pSettings->GetReadOnly().szPassword);
		bGameInited = true;
		return;
	}
}
...
jint JNI_OnLoad(JavaVM *vm, void *reserved)
{
	...
	СAndroid::SetBaseStorage("/storage/emulated/0/Street/");
