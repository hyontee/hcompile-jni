...
void CKeyBoard::Render()
{
    ...
    ImGui::SetNextWindowPos(ImVec2(io.DisplaySize.x - vecButSize.x, io.DisplaySize.y / 10 - vecButSize.y * 10));
    ...
    ImGui::GetOverlayDrawList()->AddRectFilled( 
                ImVec2(m_fKeySizeX * 7.6, m_Pos.y),     
                ImVec2(m_fKeySizeX * 8.6, m_Pos.y + m_fKeySizeY), 0xB0000000);

	// background-triangledown
    ImGui::GetOverlayDrawList()->AddRectFilled(
                ImVec2(m_fKeySizeX * 8.8, m_Pos.y),    
                ImVec2(m_fKeySizeX * 9.8, m_Pos.y + m_fKeySizeY), 0xB0000000);
    ...
    for (int i = 0; i < 4; i++)
    {
        for (auto key : m_Rows[m_iLayout][i])
        {
            ...
            if (m_iPushedKeyUp)
			{
    ImGui::GetOverlayDrawList()->AddRectFilled(ImVec2(m_fKeySizeX * 7.6, m_Pos.y), 
    ...
    ImGui::GetOverlayDrawList()->AddTriangleFilled(
        ...
        0xFF8A8886);
			...
			{
    ImGui::GetOverlayDrawList()->AddRectFilled(ImVec2(m_fKeySizeX * 8.8, m_Pos.y), 
            ...
            switch (key.type)
            {
                ...
                case KEY_SEND:
                    ImGui::GetOverlayDrawList()->AddTriangleFilled(
                            ...
                            ImVec2(key.pos.x + key.width * 0.7, key.pos.y + fKeySizeY * 0.50), 0xFF8A8886);
                    break;
                case KEY_UP:
				ImGui::GetOverlayDrawList()->AddTriangleFilled(
