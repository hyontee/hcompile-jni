...
void CGUI::Render()
{
	...
	if (pNetGame && !pDialogWindow->m_bIsActive && pGame->IsToggledHUDElement(HUD_ELEMENT_BUTTONS))
	{
		...
		ImGui::PushStyleColor(ImGuiCol_Button, (ImVec4)ImColor(20,20, 20, 255).Value); // цвет самой кнопки
		ImGui::PushStyleColor(ImGuiCol_WindowBg, (ImVec4)ImColor(0.00f, 0.00f, 0.00f, 0.80f).Value); // фон
		ImGui::PushStyleColor(ImGuiCol_ButtonHovered, (ImVec4)ImColor(255,235,0,255).Value); // нажатие
		ImGui::PushStyleColor(ImGuiCol_ButtonActive, (ImVec4)ImColor(20,20, 20, 255).Value); // после нажатий
		ImGui::GetStyle().FrameBorderSize = 0.2f;
