...
void CGUI::Render()
{
	...
	// Permanent launcher label. The overlay draw list is rendered above every
	// other GUI element and does not depend on a server connection.
	{
		ImDrawList* overlay = ImGui::GetOverlayDrawList();
		ImFont* font = m_pFont ? m_pFont : ImGui::GetFont();
		const float fontSize = m_fFontSize * 1.15f;
		const ImVec2 pos(ScaleX(120.0f), ScaleY(5.0f));
		const ImU32 black = IM_COL32(0, 0, 0, 255);
		const ImU32 white = IM_COL32(255, 255, 255, 255);

		overlay->AddText(font, fontSize, ImVec2(pos.x - 2.0f, pos.y), black, "SKARF");
		overlay->AddText(font, fontSize, ImVec2(pos.x + 2.0f, pos.y), black, "SKARF");
		overlay->AddText(font, fontSize, ImVec2(pos.x, pos.y - 2.0f), black, "SKARF");
		overlay->AddText(font, fontSize, ImVec2(pos.x, pos.y + 2.0f), black, "SKARF");
		overlay->AddText(font, fontSize, pos, white, "SKARF");
	}
