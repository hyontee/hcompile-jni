...
void CGUI::RenderPosition()
{
    ...
    // Позиция: низ экрана, слева с отступом
    ImVec2 pos = ImVec2(10.0f, io.DisplaySize.y - size_font - 10.0f);
    
    // Получение имени игрока (замените на вашу функцию получения имени)
    const char* playerName = GetPlayerName(); // или g_Player->GetName()
    ...
    // Формирование текста
    char text[512];
    sprintf(text, "BLACK RUSSIA | Build: 5.5 : %s | v5.1 | Player: %s | Time: %s", 
            __DATE__, playerName, timeBuffer);
    
    // Рендеринг с тенью (смещение для эффекта)
    ImVec2 shadowOffset = ImVec2(1.0f, 1.0f);
    
    // Тень (черный текст)
    ImGui::GetBackgroundDrawList()->AddText(fontmoney, size_font, 
        ImVec2(pos.x + shadowOffset.x, pos.y + shadowOffset.y), 
        ImColor(IM_COL32_BLACK), text);
    
    // Основной текст (белый)
    ImGui::GetBackgroundDrawList()->AddText(fontmoney, size_font, pos, 
        ImColor(255, 255, 255, 255), text);
}
