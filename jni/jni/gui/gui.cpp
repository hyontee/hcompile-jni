...
/*
	Р’СЃРµ РєРѕРѕСЂРґРёРЅР°С‚С‹ GUI-СЌР»РµРјРµРЅС‚РѕРІ Р·Р°РґР°СЋС‚СЃСЏ
	РѕС‚РЅРѕСЃРёС‚РµР»СЊРЅРѕ СЂР°Р·СЂРµС€РµРЅРёСЏ 1920x1080
*/
...
extern void TextWithColors(ImVec2 pos, ImColor col, const char* szStr, const char* szWithColors = nullptr);
void CGUI::Render()
{
	...
	if(this->m_bMemDebugOn) {
		...
		char szStrMem[256];
		snprintf(&szStrMem[0], sizeof(szStrMem), "RAM: %d",
				 totalAllocatedMB
		);

		uint32_t dwRet;
		ScriptCommand(&get_actor_z_angle, pGame->FindPlayerPed()->m_dwGTAId, &dwRet);

		if(pGame->FindPlayerPed() && pGame->FindPlayerPed()->m_pEntity && pGame->FindPlayerPed()->m_pEntity->mat) {
			ImGui::Text("Pos: %.2f %.2f %.2f", pGame->FindPlayerPed()->m_pEntity->mat->pos.x, pGame->FindPlayerPed()->m_pEntity->mat->pos.y, pGame->FindPlayerPed()->m_pEntity->mat->pos.z);
		}

		ImGui::Text("Angle: %d", (uint8_t)dwRet);

		ImGui::Text("Streaming memory: %d/%d MB", ms_memoryUsed >> 20, ms_memoryAvailable >> 20);
		ImGui::Text("Texture memory: %d/%d MB", storedTexels/(1024*1024), GetMaxStorage/(1024*1024));
		ImGui::Text("Models requested: %d", *(uint32_t*)(g_libGTASA + 0x670680));
		ImGui::Text("%s", szStrMem);

		if(ImGui::Button("Texture debug", ImVec2(ImGui::CalcTextSize("Texture debug").x + 64, 64)))
		{
	...
	if(bShowDebugLabels)
	{
		if(pNetGame)
		{
			...
			{
				CVehicle *pVehicle = CVehiclePool::GetAt(x);
				if(pVehicle)
				{
					if(pVehicle->GetDistanceFromLocalPlayerPed() <= 20.0f)
					{
						...
						sprintf(szTextLabel, "[ид машины: %d | модель: %d | тип машины: %d | здоровье: %.1f | предзагрузка: %d]\nдальность: %.2fm\nпозиция: %.3f, %.3f, %.3f",
								x, pVehicle->GetModelIndex(), pVehicle->GetVehicleSubtype(),
								pVehicle->GetHealth(), 0,
						...
						TextWithColors(vecRealPos, 0x358BD4FF, szTextLabel);
					}
				}
			}
		}
	...
	/*if (pNetGame && showHud && !pDialogWindow->m_bIsActive && !pDialogWindow->m_bActiveDialog && pGame->IsToggledHUDElement(HUD_ELEMENT_BUTTONS))
	{
		ImGuiIO& io = ImGui::GetIO();
		ImVec2 vecButSize = ImVec2(ImGui::GetFontSize() * 3.5, ImGui::GetFontSize() * 2.5);
		ImGui::SetNextWindowPos(ImVec2(5.0f, io.DisplaySize.y / 2.0 - vecButSize.x / 2));
		...
		if (pPlayerPool->GetLocalPlayer()->GetPlayerPed()->IsInVehicle() && !pPlayerPool->GetLocalPlayer()->GetPlayerPed()->IsAPassenger())
		{
			if (ImGui::Button(OBFUSCATE("CTRL"), vecButSize))
...
void CGUI::DrawTextureInfo(uintptr_t pEntity) {
	...
	if(pTextureInfo)
	{
		...
		sprintf(szTextureBufferInfo, "DFF: %f MB\nTextures: %f MB\nID: %d", dff_size, tex_size, usEntityID);
		std::string s(szTextureBufferInfo); // kaef fix
		...
		((void (*)(CVector*, CVector*, float*, float*, bool, bool))(SA_ADDR(0x54EEC0 + 1)))(&TagPos, &Out, 0, 0, 0, 0);
