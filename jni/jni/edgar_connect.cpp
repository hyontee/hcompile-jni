//
// Created by EDGAR 3.0 on 30.07.2023.
//
#include <jni.h>
#include "main.h"
#include "../game/game.h"
#include "../gui/gui.h"
#include "net/netgame.h"
#include "settings.h"
extern CGame *pGame;
extern CNetGame* pNetGame;
extern CSettings* pSettings;

extern "C" {
	JNIEXPORT void JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_EdgarConnect(JNIEnv *env, jobject thiz, jstring host, jint port) {
		const char *host_char = env->GetStringUTFChars(host, 0);
		pNetGame = new CNetGame(
        host_char,
        port,
        pSettings->GetReadOnly().szNickName,
        pSettings->GetReadOnly().szPassword);

        env->ReleaseStringUTFChars(host, host_char);
	}
	JNIEXPORT void JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_EdgarConnect2(JNIEnv *env, jobject thiz, bool debug, jstring nick) {
        const char *nick_char = env->GetStringUTFChars(nick, 0);
		if(debug) {
			CGUI::debug = false;
		} else {
    		CGUI::debug = true;
		}
		pNetGame = new CNetGame(
				"46.174.53.220",
                2275,
				nick_char,
				pSettings->GetReadOnly().szPassword);
	}
}
