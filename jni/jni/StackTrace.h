...
public:
    ...
    {
        CrashHandler::Log(OBFUSCATE("------------ START BACKTRACE ------------"));
        ...
        CrashHandler::Log(OBFUSCATE("------------ END BACKTRACE ------------"));
    }
    ...
    static void printAddressBacktrace(const unsigned address, void* pc, void* lr)
    {
        ...
        while (fgets(buffer, sizeof(buffer), fp))
        {
            ...
            if (start_address <= address && end_address > address)
            {
                ...
                {
                    CrashHandler::Log("Call: %x (GTA: %x PC: %s LR: %s) (SAMP: %x)", address, address - g_libGTASA, info_pc.dli_sname, info_lr.dli_sname, address - FindLibrary("libsamp.so"));
                    break;
                }
            }
        }
