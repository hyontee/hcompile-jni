...
void InitBASSFuncs(const char* package)
{
    ...
    sprintf(handleAddr, obfuscate("/data/data/%s/lib/libbass.so"), package);

	void* handle;
    handle = dlopen(handleAddr, 3);

    Log(obfuscate("InitBASSFuncs | Load: s%"), handleAddr);
    if (!handle)
	{
		Log(obfuscate("InitBASSFuncs | Error: cannot load libbass.so"));
		return;
	}

	BASS_Init_func = GetBASSFunc<BOOL(*)(int device, DWORD freq, DWORD flags, void* win, void*)>(handle, obfuscate("BASS_Init"));
	BASS_StreamCreateURL_func = GetBASSFunc<HSTREAM(*)(const char* url, DWORD offset, DWORD flags, DOWNLOADPROC * proc, void* user)>(handle, obfuscate("BASS_StreamCreateURL"));
	BASS_ChannelPlay_func = GetBASSFunc<BOOL(*)(DWORD handlee, BOOL restart)>(handle, obfuscate("BASS_ChannelPlay"));
    ...
    Log(obfuscate("InitBASSFuncs | Load: succesfull"));
}
...
bool ProcessVoiceCommands(const char* str)
{
	...
	if (strstr(str, "/testn"))
	{
		pJava->ShowNotification(4, "do it work? yea", 10, ">>", ">>");
		return true;
	}
