...
void CVoiceChatClient::OnVoiceInput(CVoiceChatClient* thisptr, void* pData, int iSize)
{
    ...
    while (thisptr->m_RingBufferRecord.BytesToRead() >= FRAME_SIZE_OPUS)
    {
        ...
        *pData = new CRawData(sizeof(uint8_t) + sizeof(uint16_t) + sizeof(uint16_t) + len);
        ...
        (*pData)->Write((uint16_t)CPlayerPool::GetLocalPlayerID());
...
void CVoiceChatClient::OnPacketIncoming(ENetEvent& event)
{
    ...
    switch (id)
    {
        case 244:
        {
            ...
            /*if(pNetGame->GetNotificationPool())
            {
                ImGuiIO &io = ImGui::GetIO();
                char bufferNotify[0xFF+1] = { '\0' };
                sprintf(bufferNotify, "[%d] %s", playerid, pNetGame->GetPlayerPool()->GetPlayerName(playerid));
                pNetGame->GetNotificationPool()->Add(0, playerid, 500, ImVec2(pGUI->GetFontSize() * 5.5f, io.DisplaySize.y * 0.58f), bufferNotify, pGUI->GetFontSize(), 0xFFFAFAFA, true, 0xFF000000, 2, true, ImVec2(32, 32), pNetGame->GetNotificationPool()->GetCachedIconTexture(0), false);
            }*/
...
//            SetNetworkState(VOICECHAT_CONNECTING);
            ...
            if (m_pClient)
            {
                Log("CVoiceChatClient %s:%d", m_szHostOrIp, m_iPort);
                if (m_pClient->Connect(m_szHostOrIp, m_iPort, CPlayerPool::GetLocalPlayerID()))
                {
...
void CVoiceChatClient::PostNewLocalPlayerInfo(PLAYER_VOICE_POS* pData, CVector* pUP, CVector* pFront)
{
