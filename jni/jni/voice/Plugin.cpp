    ...
    if(!Plugin::muteStatus)
    {
        if(Plugin::recordStatus)
        {
            if(GetTickCount() - lasttickmicro >= 20000) {
                ...
                g_pJavaWrapper->toggleMicro(false);
            }
        }
    }
...
void Plugin::PluginConnectHandler(SV::ConnectPacket& connectStruct)
{
    ...
    LogVoice("Plugin::PluginConnectHandler %d", SV::kVersion);
}
...
Java_com_berngames_game_gui_HudManager_clickVoice(JNIEnv *env, jobject thiz) {
// TODO: implement clickVoice()
    int defaultKey = 0x42;
    if (PluginConfig::GetMicroEnable()) {
        if (!Plugin::recordStatus) {
            ...
            g_pJavaWrapper->toggleMicro(true);
