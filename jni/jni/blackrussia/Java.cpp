...
//						pNetGame = new CNetGame(
  //pSettings->GetReadOnly().szHost,
  //pSettings->GetReadOnly().iPort,
  //pSettings->GetReadOnly().szNickName,
  //pSettings->GetReadOnly().szPassword);
...
//    } else Log("CJavaWrapper::ShowHud() was detected, by reason: status");
}
...
void CJavaWrapper::ShowAuth() 
{
	...
	env->CallVoidMethod(this->activity, this->s_showAuth);
}

void CJavaWrapper::ShowReg() 
{
	JNIEnv* env = GetEnv();
	...
	env->CallVoidMethod(this->activity, this->s_showReg);
}
...
CJavaWrapper::CJavaWrapper(JNIEnv* env, jobject activity)
{
	...
	s_showAuth = env->GetMethodID(nvEventClass, "showAuth", "()V");
    s_showReg = env->GetMethodID(nvEventClass, "showReg", "()V");
