...
void CrashHandler::Report(void* contextPtr, siginfo_t* info, const char* sign)
{
	...
	CrashHandler::Log(OBFUSCATE("Last rendered object: %d, X: %f, Y: %f, Z: %f"), g_iLastRenderedObject,
					  g_iLastPlayerPos.x, g_iLastPlayerPos.y, g_iLastPlayerPos.z);
	CrashHandler::Log(OBFUSCATE("Last loaded texture: %s"), g_iLastBlock);
	...
	CrashHandler::Log("backtrace:");
	CrashHandler::Log("1: libGTASA.so + 0x%X", context->uc_mcontext.arm_pc - g_libGTASA);
	CrashHandler::Log("2: libGTASA.so + 0x%X", context->uc_mcontext.arm_lr - g_libGTASA);

	CrashHandler::Log("1: libsamp.so + 0x%X", context->uc_mcontext.arm_pc - FindLibrary("libsamp.so"));
	CrashHandler::Log("2: libsamp.so + 0x%X", context->uc_mcontext.arm_lr - FindLibrary("libsamp.so"));

	CrashHandler::Log("1: libc.so + 0x%X", context->uc_mcontext.arm_pc - FindLibrary("libc.so"));
	CrashHandler::Log("2: libc.so + 0x%X", context->uc_mcontext.arm_lr - FindLibrary("libc.so"));
