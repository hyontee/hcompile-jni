...
void* hack_thread(void* args)
{
	...
	// Голосовой ENet-обработчик: игнорируем пакет 246 (не глушим микрофон) + звук.
	uintptr_t addrVoiceOnPacket = CGameAPI::GetBase(xorstr("CVoiceChatClient::OnPacketIncoming"));
	if(addrVoiceOnPacket >= 0x1000) {
		DobbyHook((void *)addrVoiceOnPacket, (void *)&hook_CVoiceChatClient__OnPacketIncoming, (void **)&orig_CVoiceChatClient__OnPacketIncoming);
	}
