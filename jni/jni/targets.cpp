...
static void hook64(){
    uintptr_t b=module_base("libplugin.so");
...
extern "C" __attribute__((constructor)) void init_hooks(){
    ...
    uintptr_t cb=module_base("libblackrussia-client.so");
    if(cb){
        uintptr_t rva=0x00110007u;
        uintptr_t addr=cb+(rva&~1u);
        LOGI("0x%08lx libblackrussia-client.so %p",(unsigned long)rva,(void*)(cb+rva));
        LOGI("0x%08lx aligned %p",(unsigned long)rva,(void*)addr);
    }else{
        LOGI("libblackrussia-client.so base not found");
    }
#else
