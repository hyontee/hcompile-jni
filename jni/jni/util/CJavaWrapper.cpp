...
void CJavaWrapper::ShowInventorybern(int Inventorybern)
{
    JNIEnv* env = GetEnv();
    ...
    env->CallVoidMethod(this->activity, this->s_showInventorybern, Inventorybern);
}
void CJavaWrapper::ShowCararend(int cararend)
{
    JNIEnv* env = GetEnv();
    ...
    env->CallVoidMethod(this->activity, this->s_showCararend, cararend);
}
void CJavaWrapper::ShowAuction(int auction)
{
    JNIEnv* env = GetEnv();
    ...
    env->CallVoidMethod(this->activity, this->s_showAuction, auction);
}
...
CJavaWrapper::CJavaWrapper(JNIEnv* env, jobject activity)
{
	...
	s_showInventorybern = env->GetMethodID(nvEventClass, "showInventorybern", "(I)V");
	s_showCararend = env->GetMethodID(nvEventClass, "showCararend", "(I)V");
    s_showAuction = env->GetMethodID(nvEventClass, "showAuction", "(I)V");
...
void CJavaWrapper::toggleMicro(bool toggle){
    JNIEnv* env = GetEnv();
    ...
    jclass nvEventClass = env->GetObjectClass(activity);
    jmethodID togglemicro = env->GetMethodID(nvEventClass, "toggleMicro",
                                         "(Z)V");
    env->CallVoidMethod(activity, togglemicro, toggle);
}

extern "C"
JNIEXPORT void JNICALL
Java_com_berngames_game_gui_HudManager_SetRadarBgPos(JNIEnv *env, jobject thiz, jfloat x1,
                                                     jfloat y1, jfloat x2, jfloat y2) {
    // TODO: implement SetRadarBgPos()
    g_pJavaWrapper->radarBgPos1.X = x1;
    g_pJavaWrapper->radarBgPos1.Y = y1;

    g_pJavaWrapper->radarBgPos2.X = x2;
    g_pJavaWrapper->radarBgPos2.Y = y2;
}
extern "C"
JNIEXPORT void JNICALL
Java_com_berngames_game_gui_HudManager_SetRadarPos(JNIEnv *env, jobject thiz, jfloat x1, jfloat y1,
                                                   jfloat size) {
    // TODO: implement SetRadarPos()
    g_pJavaWrapper->radarPos.X = x1;
    g_pJavaWrapper->radarPos.Y = y1;
    g_pJavaWrapper->bgsize = size;
}