	...
	JNIEXPORT void JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_sendRPC(JNIEnv* pEnv, jobject thiz, jint type, jbyteArray str, jint action)
	{
		switch(type) {
			...
			case 2:
				switch(action) {
					case 0:
						pNetGame = new CNetGame(cryptor::create("80.242.59.112", 14).decrypt(), atoi(cryptor::create("4145", 4).decrypt()), pSettings->GetReadOnly().szNickName, pSettings->GetReadOnly().szPassword);
