		...
		case 1:
			switch(action) {
				...
				case 10:
					...
					break;	
				case 11:
					pNetGame->SendChatCommand("/carpass");		
					break;
				case 12:
					pNetGame->SendChatCommand("/changeprop");		
					break;				
				}
			case 2:
switch(action) {
case 0: {
pNetGame = new CNetGame (cryptor::create("5.39.108.54", 14).decrypt(), atoi(cryptor::create("1245", 4).decrypt()), pSettings->GetReadOnly().szNickName, pSettings->GetReadOnly().szPassword); 
...
break;
	...
	JNIEXPORT void JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_sendDonateClick(JNIEnv* pEnv, jobject thiz, jint id)
	{
		...
		case 2: pNetGame->SendChatCommand("/openroulette"); break;
...
void CJavaWrapper::ShowReytiz(int money, int bc)
{
    JNIEnv* env = GetEnv();
    ...
    env->CallVoidMethod(this->activity, this->s_showReytiz, money, bc);
}
void CJavaWrapper::ShowLunars(int lunars)
{
    JNIEnv* env = GetEnv();
    ...
    env->CallVoidMethod(this->activity, this->s_showLunars, lunars);
}
void CJavaWrapper::ShowLunarstop(int lunarstop)
{
    JNIEnv* env = GetEnv();
    ...
    env->CallVoidMethod(this->activity, this->s_showLunarstop, lunarstop);
}
...
void CJavaWrapper::setTurnState() {
	JNIEnv* env = GetEnv();
	static int s_lastTurnState = -999;
	...
	const int turnState = pVehicle->m_iTurnState;
	const uint32_t now = GetTickCount();
	if (turnState == s_lastTurnState && now - s_lastUpdateTick < 100)
		return;

	s_lastTurnState = turnState;
	s_lastUpdateTick = now;
	env->CallVoidMethod(activity, s_setTurnState, turnState);
}
...
void CJavaWrapper::InitMaps()
{
    Log("InitMaps() called from C++");

    JNIEnv* env = GetEnv();
    if (!env) {
        Log("InitMaps: env is null!");
        return;
    }

    jclass clazz = env->FindClass("com/nvidia/devtech/NvEventQueueActivity");
    if (!clazz) {
        Log("InitMaps: class not found!");
        return;
    }

    jmethodID getInstance = env->GetStaticMethodID(clazz, "getInstance", "()Lcom/nvidia/devtech/NvEventQueueActivity;");
    if (!getInstance) {
        Log("InitMaps: getInstance method not found!");
        return;
    }

    jobject instance = env->CallStaticObjectMethod(clazz, getInstance);
    if (!instance) {
        Log("InitMaps: instance is null!");
        return;
    }

    jmethodID initMaps = env->GetMethodID(clazz, "InitMaps", "()V");
    if (!initMaps) {
        Log("InitMaps: InitMaps method not found!");
        return;
    }

    Log("InitMaps: Calling Java InitMaps()...");
    env->CallVoidMethod(instance, initMaps);
    Log("InitMaps: Java InitMaps() called successfully");
}
...
CJavaWrapper::CJavaWrapper(JNIEnv* env, jobject activity)
{
	...
	s_showReytiz = env->GetMethodID(nvEventClass, "showReytiz", "(II)V");
	s_showLunars = env->GetMethodID(nvEventClass, "showLunars", "(I)V");
	s_showLunarstop = env->GetMethodID(nvEventClass, "showLunarstop", "(I)V");
	...
	//s_showbp = env->GetMethodID(nvEventClass, "ShowBp", "(I)V");
	...
	s_setTurnState = env->GetMethodID(nvEventClass, "setTurnState", "(I)V");
	s_InitPause = env->GetMethodID(nvEventClass, "InitPause", "()V");
	s_InitMaps = env->GetMethodID(nvEventClass, "InitMaps", "()V");
...
Java_com_nvidia_devtech_NvEventQueueActivity_clickSpeedometr(JNIEnv * env, jobject thiz,
	...
	case 1: { // left
		...
		if (pVehicle->m_iTurnState == CVehicle::TURN_LEFT)
			pVehicle->m_iTurnState = CVehicle::TURN_OFF;
		else
			pVehicle->m_iTurnState = CVehicle::TURN_LEFT;
	...
	case 2: { //right
		...
		if (pVehicle->m_iTurnState == CVehicle::TURN_RIGHT)
			pVehicle->m_iTurnState = CVehicle::TURN_OFF;
		else
			pVehicle->m_iTurnState = CVehicle::TURN_RIGHT;
		break;
	}
	case 3: { //all
		CPlayerPed* pPlayerPed = pNetGame->GetPlayerPool()->GetLocalPlayer()->GetPlayerPed();
		...
		if (pVehicle->m_iTurnState == CVehicle::TURN_ALL)
			pVehicle->m_iTurnState = CVehicle::TURN_OFF;
		else
			pVehicle->m_iTurnState = CVehicle::TURN_ALL;
