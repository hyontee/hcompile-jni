...
void CJavaWrapper::ShowHud(int health, int armour, int hunger, int weaponid, int ammo, int ammoinclip, int money)
{
	JNIEnv* env = GetEnv();
	...
	env->CallVoidMethod(this->activity, this->s_showHud, health, armour, hunger, weaponid, ammo, ammoinclip, money);
}
...
void CJavaWrapper::ShowDialog(int dialogId, int dialogTypeId, char* caption, char* content, char* leftBtnText, char* rightBtnText)
{
	...
	jbyteArray bytes = global_env->NewByteArray(strlen(caption)); 
	global_env->SetByteArrayRegion(bytes, 0, strlen(caption), (jbyte*)caption); 
	jstring str1 = (jstring)global_env->NewObject(strClass, ctorID, bytes, encoding);

	bytes = global_env->NewByteArray(strlen(content)); 
	global_env->SetByteArrayRegion(bytes, 0, strlen(content), (jbyte*)content); 
	jstring str2 = (jstring)global_env->NewObject(strClass, ctorID, bytes, encoding);

	bytes = global_env->NewByteArray(strlen(leftBtnText)); 
	global_env->SetByteArrayRegion(bytes, 0, strlen(leftBtnText), (jbyte*)leftBtnText); 
	jstring str3 = (jstring)global_env->NewObject(strClass, ctorID, bytes, encoding);

	bytes = global_env->NewByteArray(strlen(rightBtnText)); 
	global_env->SetByteArrayRegion(bytes, 0, strlen(rightBtnText), (jbyte*)rightBtnText); 
	jstring str4 = (jstring)global_env->NewObject(strClass, ctorID, bytes, encoding);

    global_env->CallVoidMethod(this->activity, this->s_showDialog, dialogId, dialogTypeId, str1, str2, str3, str4);
}
...
CJavaWrapper::CJavaWrapper(JNIEnv* env, jobject activity)
{
	...
	s_showHud = env->GetMethodID(nvEventClass, "showHud", "(IIIIIII)V");
