...
extern "C"
{
	...
	JNIEXPORT void JNICALL Java_com_liverussiabonus_cr_gui_HudManager_ToggleKeyBoard(JNIEnv *env, jobject thiz, jboolean toggle) {
		if (toggle) 
		{
			pKeyBoard->Open(0);
		} 
	...
	JNIEXPORT void JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_sendReconClickButton(JNIEnv* pEnv, jobject thiz, jint id)
	{
		pNetGame->SendReconIpacket(id);
	}

	JNIEXPORT void JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_sendStylingClick(JNIEnv* pEnv, jobject thiz, jint id)
	{
		pNetGame->SendStylingIpacket(id);
	}

	JNIEXPORT void JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_sendColorPickerRgbColor(JNIEnv* pEnv, jobject thiz, jint red, jint green, jint blue)
	{
		pNetGame->SendIpacketColorPickerRgb(red, green, blue);
	}

	JNIEXPORT void JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_sendStylingClickNotPay(JNIEnv* pEnv, jobject thiz, jint cost)
	{
		pNetGame->SendStylingIpacketNotPay(cost);
	}

	JNIEXPORT void JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_sendIpacketGiveCoins(JNIEnv* pEnv, jobject thiz, jint coins)
	{
		pNetGame->SendIpacketGiveLiveCoins(coins);
	}

	JNIEXPORT void JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_sendQueryBuyEventItem(JNIEnv* pEnv, jobject thiz, jint cost)
	{
		pNetGame->SendQueryBuyEventItem(cost);
	}
...
void CJavaWrapper::UpdateLvl(int lvl, int exp, int max_exp)
{
    JNIEnv* env = GetEnv();
    ...
    env->CallVoidMethod(this->activity, this->s_updateLvlInfo, lvl, exp, max_exp);
}

void CJavaWrapper::JavaGreenZone(int isShow)
{
    JNIEnv* env = GetEnv();
    ...
    env->CallVoidMethod(this->activity, this->s_showJavaGreenZone, isShow);
}

void CJavaWrapper::HideLogoForChat(int isShow)
{
    JNIEnv* env = GetEnv();
    ...
    env->CallVoidMethod(this->activity, this->s_hideLogoForChat, isShow);
}

void CJavaWrapper::ShowStylingCenter(int money)
{
    JNIEnv* env = GetEnv();
    ...
    env->CallVoidMethod(this->activity, this->s_showStylingCenter, money);
}
void CJavaWrapper::HideStylingCenter()
{
    JNIEnv* env = GetEnv();
    ...
    env->CallVoidMethod(this->activity, this->s_hideStylingCenter);
}

void CJavaWrapper::ShowNotification(int type, char* text, int duration, char* positive_command, char* negative_command) 
{
	JNIEnv* env = GetEnv();
	...
	bytes = env->NewByteArray(strlen(positive_command));
	env->SetByteArrayRegion(bytes, 0, strlen(positive_command), (jbyte*)positive_command);
	jstring jpositive_command = (jstring) env->NewObject(strClass, ctorID, bytes, encoding);

	bytes = env->NewByteArray(strlen(negative_command));
	env->SetByteArrayRegion(bytes, 0, strlen(negative_command), (jbyte*)negative_command);
	jstring jnegative_command = (jstring) env->NewObject(strClass, ctorID, bytes, encoding);

	env->CallVoidMethod(this->activity, this->s_showNotification, type, jtext, duration, jpositive_command, jnegative_command);
}



void CJavaWrapper::ShowAdminRecon(char* name, int id) 
{
	JNIEnv* env = GetEnv();
	...
	env->CallVoidMethod(this->activity, this->s_showAdminRecon, jname, id);
}
...
void CJavaWrapper::ShowAchievements(int progress_1, int progress_2, int progress_3, int progress_4, int progress_5)
{
    JNIEnv* env = GetEnv();
    ...
    env->CallVoidMethod(this->activity, this->s_showAchievements, progress_1, progress_2, progress_3, progress_4, progress_5);
}


void CJavaWrapper::ShowMagicStore(int bronze_coins, int silver_coins, int gold_coins)
{
    JNIEnv* env = GetEnv();
    ...
    env->CallVoidMethod(this->activity, this->s_showMagicStore, bronze_coins, silver_coins, gold_coins);
}

void CJavaWrapper::ShowChooseSpawn(int organizationstate, int stationstate, int exitstate, int garagestate, int housestate)
{
	JNIEnv* env = GetEnv();
	if(!env) return;

	env->CallVoidMethod(this->activity, this->s_showChooseSpawn, organizationstate, stationstate, exitstate, garagestate, housestate);
}
...
CJavaWrapper::CJavaWrapper(JNIEnv* env, jobject activity)
{
	...
	s_updateLvlInfo = env->GetMethodID(nvEventClass, "updateLvlInfo", "(III)V");
	s_showJavaGreenZone = env->GetMethodID(nvEventClass, "javaGreenZone", "(I)V");
	s_hideLogoForChat = env->GetMethodID(nvEventClass, "logoForChat", "(I)V");
    ...
    s_showAdminRecon = env->GetMethodID(nvEventClass, "showAdminRecon", "(Ljava/lang/String;I)V");
	s_showStylingCenter = env->GetMethodID(nvEventClass, "showStylingCenter", "(I)V");
	s_hideStylingCenter = env->GetMethodID(nvEventClass, "hideStylingCenter", "()V");
	s_showAchievements = env->GetMethodID(nvEventClass, "showAchievements", "(IIIII)V");
	...
	s_showMagicStore = env->GetMethodID(nvEventClass, "showMagicStore", "(III)V");
