...
std::string CJavaWrapper::GetClipboardString()
{
	...
	jbyte* pTextCpy = (jbyte*)malloc(env->GetArrayLength(retn) + 1);
	memcpy(pTextCpy, pText, env->GetArrayLength(retn));
	pTextCpy[env->GetArrayLength(retn)] = 0;
	std::string str((char*)pTextCpy);
	free(pTextCpy);
...
JNIEXPORT void JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_sendWoundResponse(JNIEnv* pEnv, jobject thiz, jint type)
{
	if(pNetGame) {
		pNetGame->SendWoundResponse(type);
	}
}
...
void CJavaWrapper::ShowWound(char* nick, int id)
{
	JNIEnv* env = GetEnv();
	...
	env->CallVoidMethod(this->activity, this->s_showWound, jnick, id);
}

void CJavaWrapper::HideWound()
{
	JNIEnv* env = GetEnv();
	...
	env->CallVoidMethod(this->activity, this->s_hideWound);
}
...
CJavaWrapper::CJavaWrapper(JNIEnv* env, jobject activity)
{
	...
	s_showWound = env->GetMethodID(nvEventClass, "showWound", "(Ljava/lang/String;I)V");
	s_hideWound = env->GetMethodID(nvEventClass, "hideWound", "()V");
