...
extern "C"
{
		...
		JNIEXPORT void JNICALL Java_com_nvidia_devtech_NvEventQueueActivity_OtpravkaServer(JNIEnv* pEnv, jobject thiz, jint server)
	{
		if(server == 1)
		{
			pNetGame = new CNetGame("188.127.241.8",1562,pSettings->GetReadOnly().szNickName,pSettings->GetReadOnly().szPassword);
		}
		else if(server == 2)
		{
			pNetGame = new CNetGame("188.127.241.8",1566,pSettings->GetReadOnly().szNickName,pSettings->GetReadOnly().szPassword);
		}
	}
