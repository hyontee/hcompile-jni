...
void UnFuck(uintptr_t ptr, size_t dwSize = 100)
{
	...
	uintptr_t page = PageStart(ptr);
	mprotect((void*)page, PageSpan(ptr, dwSize), PROT_READ | PROT_WRITE | PROT_EXEC);
}
...
void FuckCode(uintptr_t ptr, size_t dwSize = 100)
{
	...
	uintptr_t page = PageStart(ptr);
	mprotect((void*)page, PageSpan(ptr, dwSize), PROT_READ | PROT_EXEC);
}
