...
static uint32_t pageStart(uint32_t addr)
{
	size_t pageSize = getPageSize();
	return addr & ~(uint32_t)(pageSize - 1);
}

static size_t pageSpan(uint32_t addr, size_t size)
{
	size_t pageSize = getPageSize();
	uint32_t start = pageStart(addr);
	uint32_t end = (addr + size + pageSize - 1) & ~(uint32_t)(pageSize - 1);
	return end - start;
}
...
enum ele7en_status registerInlineHook(uint32_t target_addr, uint32_t new_addr, uint32_t **proto_addr)
{
	...
	size_t pageSize = getPageSize();
	item->trampoline_instructions = mmap(NULL, pageSize, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_ANONYMOUS | MAP_PRIVATE, 0, 0);
...
{
	uint32_t target = CLEAR_BIT0(item->target_addr);
	mprotect((void *) pageStart(target), pageSpan(target, item->length), PROT_READ | PROT_WRITE | PROT_EXEC);
	memcpy((void *) CLEAR_BIT0(item->target_addr), item->orig_instructions, item->length);
	mprotect((void *) pageStart(target), pageSpan(target, item->length), PROT_READ | PROT_EXEC);
	munmap(item->trampoline_instructions, getPageSize());
