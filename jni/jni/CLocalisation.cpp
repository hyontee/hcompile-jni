...
char CLocalisation::m_szMessages[E_MSG::MSG_COUNT][MAX_LOCALISATION_LENGTH] = {
	"{bbbbbb}Подключение к серверу...",
	"{bbbbbb}Авторизация в игре...",
	"{bbbbbb}Соединение с сервером было разорвано. Пожалуйста попробуйте войти заново.",
	"{bbbbbb}Обнаружены изменения в некоторых файлах. Рекомендуется переустановить клиент.",
	"{bbbbbb}Неиспользуемая команда.",
	"{bbbbbb}Ваш доступ к серверу был заблокирован.",
	"{bbbbbb}Соединение с сервером потеряно. Попытка переподключения...",
	"{bbbbbb}Возникли проблемы с сетью. Попытка повторного подключения...",
	"{bbbbbb}Сервер заполнен. Пожалуйста, попробуйте позже."
};
