...
char CLocalisation::m_szMessages[E_MSG::MSG_COUNT][MAX_LOCALISATION_LENGTH] = {
        "Подключение к LEVEL MOBILE...",
        "Подключено. Подготовка к игре...",
        "Сервер закрыл соединение.",
        "{ffffff}Клиент {53c5ec}LEVEL MOBILE {ffffff}запущен",
        "unused",
        "Вы были заблокированы сервером. Выйдите из игры и подключитесь заново.",
        "Соединение с сервером потеряно, переподключение через несколько секунд...",
        "Соединение с сервером потеряно, переподключение через несколько секунд...",
        "Сервер полон. Переподключение...",
        "{FB6262}Перезайдите с помощью специального лаунчера LEVEL MOBILE."
};
...
void CLocalisation::Initialise(const char* szFile)
{
	...
	sprintf(&buff[0], "%smultiplayer/%s", g_pszStorage, szFile);
