...
void BitStream::Write( BitStream *bitStream, int numberOfBits )
{
	...
	while (numberOfBits-->0 && bitStream->readOffset + 1 <= bitStream->numberOfBitsUsed)
	{
		...
		{
			// New byte
			const unsigned int bitOffset = bitStream->readOffset++;
			if (bitStream->data[ bitOffset >> 3 ] & ( 0x80 >> ( bitOffset % 8 ) ) )
			{
				// Write 1
...
{
	const unsigned int bitOffset = readOffset++;
	return ( bool ) ( data[ bitOffset >> 3 ] & ( 0x80 >> ( bitOffset & 7 ) ) );
}
