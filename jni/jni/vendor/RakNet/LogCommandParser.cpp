...
// Android build compatibility: older RakNet referenced this limit from a
// remote-console header that is not part of this stripped client tree.
// Keep the same semantics with a bounded local limit.
enum { BR_REMOTE_MAX_TEXT_INPUT = 4096 };
...
void LogCommandParser::WriteLog(const char *channelName, const char *format, ...)
{
	...
	char text[BR_REMOTE_MAX_TEXT_INPUT];
	va_list ap;
	va_start(ap, format);
	_vsnprintf(text, BR_REMOTE_MAX_TEXT_INPUT, format, ap);
	va_end(ap);
	text[BR_REMOTE_MAX_TEXT_INPUT-1]=0;
	...
	if (textLen < BR_REMOTE_MAX_TEXT_INPUT-4)
		strcat(text, "\r\n");
	else
	{
		text[textLen-3]='\r';
