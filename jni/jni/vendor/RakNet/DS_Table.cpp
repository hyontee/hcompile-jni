		...
		for (j=0; j<inclusionFilterColumnIndices.Size(); j++)
		{
			...
			if (row->cells[columnIndex]->isEmpty==false && columnIndex!=(unsigned)-1)
			{
				...
				case QF_GREATER_THAN:
					...
					case BINARY:
					{
						const int lhsLen = row->cells[columnIndex]->i;
						const int rhsLen = inclusionFilters[j].cellValue->i;
						const int commonLen = lhsLen < rhsLen ? lhsLen : rhsLen;
						const int cmp = commonLen > 0 ? memcmp(row->cells[columnIndex]->c, inclusionFilters[j].cellValue->c, commonLen) : 0;
						pass = cmp > 0 || (cmp == 0 && lhsLen > rhsLen);
						break;
					}
					}
					break;
