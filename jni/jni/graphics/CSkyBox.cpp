...
void CSkyBox::Initialise()
{
	...
	if (!dwModelArray[18500])
		return;
// 1.4f
	m_pSkyObject = CreateObjectScaled(18500, 0.4f); //10657 18659

	SetTexture(OBFUSCATE("lkskybox"));
}
...
CObject* CSkyBox::CreateObjectScaled(int iModel, float fScale)
{
	...
	if (!pNetGame->GetObjectPool()) return nullptr;
...
void CSkyBox::ReTexture()
{
	...
	if (pChangeTime != iHours)
	{
		...
		if (iHours >= 0 && iHours <= 5)
			SetTexture(OBFUSCATE("lkskybox")); //night_sky_1
...
uintptr_t CSkyBox::RwFrameForAllObjectsCallback(uintptr_t object, CObject* pObject)
{
	...
	for (int i = 0; i < numMats; i++)
	{
		...
		if (pSkyTexture) pMaterial->texture = pSkyTexture;
	}
	return object;
}
...
void CSkyBox::SetTexture(const char *texName)
{
	...
	pSkyTexture = (RwTexture*)LoadTextureFromDB("gui", texName);
}
