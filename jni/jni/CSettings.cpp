...
void CSettings::Save(int iIgnoreCategory)
{
    ...
    if (iIgnoreCategory != 1)
    {
        ...
        ini_table_create_entry_as_int(config, "gui", "hud", 1);
        ...
        ini_table_create_entry_as_int(config, "gui", "dialog", 1);
    }
    ...
    for (int i = 0; i < E_HUD_ELEMENT::HUD_SIZE; i++)
    {
        ...
        ini_table_create_entry_as_int(config, "hud", buff, -1);
...
void CSettings::LoadSettings(const char* szNickName, int iChatLines)
{
    ...
    m_Settings.fButtonCameraCycleX = ini_table_get_entry_as_float(config, "gui", "CameraCycleX", 0.0f);
    m_Settings.fButtonCameraCycleY = ini_table_get_entry_as_float(config, "gui", "CameraCycleY", 0.0f);
    m_Settings.fButtonCameraCycleSize = ini_table_get_entry_as_float(config, "gui", "CameraCycleSize", 0.0f);
