...
void CGTexture::LoadTextureButton()
{
	p_GTexture = (RwTexture*)LoadTextureFromDB("samp","gtexture");
}

void CGTexture::RenderButton()
{
	...
	ImGui::Begin("Created by TonY", nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoSavedSettings);
				...
				if(pVehicle->GetDistanceFromLocalPlayerPed() < 10.0f)
				{
					...
					if(pPlayerPool)
					{
						...
						if(pLocalPlayer)
						{
							...
							{
								if(ImGui::ImageButton((ImTextureID)p_GTexture->raster, vecButSize_g))
								{
