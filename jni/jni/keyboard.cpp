...
void CKeyBoard::Open(keyboard_callback* handler, bool bHiden)
{
    ...
    if (m_bNewKeyboard)
    {
        ...
        if (g_pJavaWrapper && g_pJavaWrapper->isGlobalShowTab == false){
            g_pJavaWrapper->SetVisibleKeyboard(true, 0);
        }
    }
}
...
void CKeyBoard::Close()
{
    ...
    if(pNetGame)
    {
        if(pNetGame->GetGameState() == GAMESTATE_CONNECTED && showHud)
        {
    ...
    if (m_bNewKeyboard)
    {
        ...
        if (g_pJavaWrapper && g_pJavaWrapper->isGlobalShowTab == false){
            g_pJavaWrapper->SetVisibleKeyboard(false, 0);
        }
    }
}
#include "util/CJavaWrapper.h"
