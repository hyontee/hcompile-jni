...
void COffset::Initialise()
{
	...
	Add(xorstr("CNetGame::Packet_TurnLightsSync"), 0x2EC8A0, eArchType::ARM);
	...
	// Голосовой ENet-обработчик (sub_43F964): хукаем, чтобы игнорить пакет 246.
	Add(xorstr("CVoiceChatClient::OnPacketIncoming"), 0x43F964, eArchType::ARM);
	...
	Add(xorstr("CNetGame::Packet_TurnLightsSync"), 0x4AC558, eArchType::ARM64);
	...
	// Голосовой ENet-обработчик (sub_643E34): хукаем, чтобы игнорить пакет 246.
	Add(xorstr("CVoiceChatClient::OnPacketIncoming"), 0x643E34, eArchType::ARM64);
}
