...
char CLocalisation::m_szMessages[E_MSG::MSG_COUNT][MAX_LOCALISATION_LENGTH] = {
	"{BA55D3}NATIVE RUSSIA",
	"{BA55D3}Соединено. Подготовка к игре...",
	"{BA55D3}Сервер закрыл соеденение, перезайдите",
	"{ffffff}Клиент {BA55D3}NATIVE RUSSIA {ffffff}запущен",
	"{BA55D3}unused",
	"{BA55D3}Вы были заблокированы сервером",
	"{BA55D3}Потеряно соединение к серверу. Переподключение через 15 секунд...",
	"{BA55D3}Отсутствует соединение. Для решения: попробуйте перезагрузить роутер, либо сменить интернет, либо воспользоваться VPN",
	"{BA55D3}Сервер полон. Повторное соединение..."
};
