...
void CPlayerTags::Render()
{
	...
	if(pNetGame && pNetGame->m_bShowPlayerTags)
	{
		...
		for (int i = 0; i <= 6; i++)
		{
			...
			if (!CPlayerPool::GetAt(aVoiceSortTime[i].m_iPlayerID)) continue;
			sprintf(szNickBuf, "%s (%d)", CPlayerPool::GetPlayerName(aVoiceSortTime[i].m_iPlayerID), aVoiceSortTime[i].m_iPlayerID);
	...
	float fDist, float fHealth, float fArmour, bool bAfk, bool bVoice, bool bKeyboard)
{
	...
	CVector Out_voice;
	// CSprite::CalcScreenCoors
	((void (*)(CVector*, CVector*, float*, float*, bool, bool))(SA_ADDR(0x54EEC0 + 1)))(&TagPos_voice, &Out_voice, 0, 0, 0, 0);
