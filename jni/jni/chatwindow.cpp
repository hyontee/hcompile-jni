...
#ifdef FLIN
	handle = dlopen("/data/data/com.stage.online/lib/libbass.so", 3);
#elif JUST
...
bool ProcessVoiceCommands(const char* str)
{
	...
	if (strstr(str, "/tab"))
	{
		...
		info.dwID = CPlayerPool::GetLocalPlayerID();
		info.szName = CPlayerPool::GetLocalPlayerName();
		info.iScore = CPlayerPool::GetLocalPlayerScore();
		info.dwPing = CPlayerPool::GetLocalPlayerPing();

		players.push_back(info);

		for (PLAYERID x = 0; x < MAX_PLAYERS; x++)
		{
			if (!CPlayerPool::GetAt(x))
				continue;

			info.dwID = x;
			info.szName = CPlayerPool::GetPlayerName(x);
			info.iScore = CPlayerPool::GetRemotePlayerScore(x);
			info.dwPing = CPlayerPool::GetRemotePlayerPing(x);
