...
bool ProcessVoiceCommands(const char* str)
{
	...
	if (strstr(str, "/wound"))
	{
		if(g_pJavaWrapper && pNetGame) {
			g_pJavaWrapper->ShowWound(pNetGame->GetPlayerPool()->GetLocalPlayerName(), pNetGame->GetPlayerPool()->GetLocalPlayerID());
		}
		return true;
	}
	if (strstr(str, "/hwound"))
	{
		if(g_pJavaWrapper) g_pJavaWrapper->HideWound();
		return true;
	}
