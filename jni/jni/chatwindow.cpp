...
void InitBASSFuncs()
{
	...
	handle = dlopen("/data/data/com.redbox.cr/lib/libbass.so", 3);
#else
	handle = dlopen("/data/data/com.redbox.cr/lib/libbass.so", 3);
#endif
	if (!handle)
...
bool ProcessVoiceCommands(const char* str)
{
	...
	if (strstr(str, "/vlist"))
	{
		...
		if (g_bShowVoiceList)
		{
			pChatWindow->AddDebugMessage("Âêëþ÷¸í ñïèñîê ãîâîðÿùèõ âîéñà");
		}
		else
		{
			pChatWindow->AddDebugMessage("Âûêëþ÷åí ñïèñîê ãîâîðÿùèõ âîéñà");
		}
		return true;
	}
	...
	if (strstr(str, "/vmute"))
	{
		...
		{
			pChatWindow->AddDebugMessage("Ïîäñêàçêà: /vmute [playerid]");
			return true;
		}
		if (id < 0 || id > 1000)
		{
			pChatWindow->AddDebugMessage("Ïîäñêàçêà: ID äîëæåí áûòü â ãðàíèöàõ îò 0 äî 1000");
			return true;
		}
		pVoice->MutePlayer(id);
		pChatWindow->AddDebugMessage("Áûë çàìó÷åí èãðîê ñ èäîì: %d", id);
		return true;
	}
	...
	if (strstr(str, "/vunmute"))
	{
		...
		if (sscanf(str, "%d", &id) == -1)
		{
			pChatWindow->AddDebugMessage("Ïîäñêàçêà: /vunmute [playerid]");
			return true;
		}
		if (id < 0 || id > 1000)
		{
		...
		pChatWindow->AddDebugMessage("Áûë ðàçìó÷åí èãðîê ñ èäîì: %d", id);
		return true;
	}
	...
	if (strstr(str, "/voicevolume"))
	{
		...
		{
			pChatWindow->AddDebugMessage("Ïîäñêàçêà: /voicevolume [value]");
			return true;
		}
		pChatWindow->AddDebugMessage("Óâåäîìëåíèå: Óñòàíîâëåíà ãðîìêîñòü %d", volume);
	...
	if (strstr(str, "/vplayer"))
	{
		...
		{
			pChatWindow->AddDebugMessage("Ïîäñêàçêà: /vplayer [playerid] [volume]");
			return true;
		}
		if (id < 0 || id > 1000)
		{
		...
		pChatWindow->AddDebugMessage("Óñòàíîâëåíà ãðîìêîñòü %d äëÿ èãðîêà %d", volume, id);
		return true;*/
	}
	return false;
}
