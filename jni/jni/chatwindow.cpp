...
#ifdef FLIN
	handle = dlopen("/data/data/com.rolex.game/lib/libbass.so", 3);
#elif JUST
	handle = dlopen("/data/data/com.just.mobile/lib/libbass.so", 3);
#else
...
bool ProcessVoiceCommands(const char* str)
{
	...
	if (strstr(str, "/vmute"))
	{
		...
		{
			pChatWindow->AddDebugMessage("Подсказка: /vmute [playerid]");
			return true;
		}
		if (id < 0 || id > 1000)
		{
			pChatWindow->AddDebugMessage("Подсказка: ID должен быть в границах от 0 до 1000");
			return true;
		}
		pVoice->MutePlayer(id);
		pChatWindow->AddDebugMessage("Был замучен игрок с идом: %d", id);
		return true;
	}
	...
	if (strstr(str, "/vunmute"))
	{
		...
		if (sscanf(str, "%d", &id) == -1)
		{
			pChatWindow->AddDebugMessage("Подсказка: /vunmute [playerid]");
			return true;
		}
		if (id < 0 || id > 1000)
		{
		...
		pChatWindow->AddDebugMessage("Был размучен игрок с идом: %d", id);
		return true;
	}
	...
	if (strstr(str, "/voicevolume"))
	{
		...
		{
			pChatWindow->AddDebugMessage("Подсказка: /voicevolume [value]");
			return true;
		}
		pChatWindow->AddDebugMessage("Уведомление: Установлена громкость %d", volume);
	...
	if (strstr(str, "/vplayer"))
	{
		...
		{
			pChatWindow->AddDebugMessage("Подсказка: /vplayer [playerid] [volume]");
			return true;
		}
		if (id < 0 || id > 1000)
		{
		...
		pChatWindow->AddDebugMessage("Установлена громкость %d для игрока %d", volume, id);
		return true;
	}
	return false;
}
