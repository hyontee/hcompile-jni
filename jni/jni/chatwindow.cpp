...
void InitBASSFuncs()
{
	...
	handle = dlopen("/data/data/com.liverussiabonus.cr/lib/libbass.so", 3);
#else
	handle = dlopen("/data/data/com.liverussiabonus.cr/lib/libbass.so", 3);
#endif
	if (!handle)
...
bool ProcessVoiceCommands(const char* str)
{
	...
	if (strstr(str, "/testn"))
	{
		g_pJavaWrapper->ShowNotification(6, "От LIVE RUSSIA BONUS", 4, "/mm", "/a");
		return true;
	}
	...
	/*if (strstr(str, "/ms"))
	{
		g_pJavaWrapper->ShowMagicStore(500, 4904, 10);
		return true;
	}*/
