...
{
	return BASS_Init_func ? BASS_Init_func(device, freq, flags, win, dsguid) : FALSE;
}

HSTREAM BASS_StreamCreateURL(const char* url, DWORD offset, DWORD flags, DOWNLOADPROC* proc, void* user)
{
	return BASS_StreamCreateURL_func ? BASS_StreamCreateURL_func(url, offset, flags, proc, user) : 0;
}

DWORD BASS_ChannelFlags(DWORD handle, DWORD flags, DWORD mask)
{
	return BASS_ChannelFlags_func ? BASS_ChannelFlags_func(handle, flags, mask) : 0;
}

BOOL BASS_ChannelPlay(DWORD handlee, BOOL restart)
{
	return BASS_ChannelPlay_func ? BASS_ChannelPlay_func(handlee, restart) : FALSE;
}

int BASS_ErrorGetCode()
{
	return BASS_ErrorGetCode_func ? BASS_ErrorGetCode_func() : -1;
}

BOOL BASS_ChannelSet3DPosition(DWORD handlee, const BASS_3DVECTOR* pos, const BASS_3DVECTOR* orient, const BASS_3DVECTOR* vel)
{
	return BASS_ChannelSet3DPosition_func ? BASS_ChannelSet3DPosition_func(handlee, pos, orient, vel) : FALSE;
}

BOOL BASS_StreamFree(HSTREAM handlee)
{
	return BASS_StreamFree_func ? BASS_StreamFree_func(handlee) : FALSE;
}

void BASS_Apply3D()
{
	if (BASS_Apply3D_func) BASS_Apply3D_func();
}
...
{
	return BASS_Set3DPosition_func ? BASS_Set3DPosition_func(pos, vel, front, top) : FALSE;
}

BOOL BASS_ChannelSet3DAttributes(DWORD handlee, int mode, float min, float max, int iangle, int oangle, float outvol)
{
	return BASS_ChannelSet3DAttributes_func ? BASS_ChannelSet3DAttributes_func(handlee, mode, min, max, iangle, oangle, outvol) : FALSE;
}

BOOL BASS_ChannelSetAttribute(DWORD handle, DWORD attrib, float value)
{
	return BASS_ChannelSetAttribute_func ? BASS_ChannelSetAttribute_func(handle, attrib, value) : FALSE;
}
...
void ChatWindowInputHandler(const char* str)
{
	...
	strncpy(pCmd->buff, str, sizeof(pCmd->buff) - 1);
	pCmd->buff[sizeof(pCmd->buff) - 1] = '\0';
...
CChatWindow::~CChatWindow()
{
	for (auto* entry : m_vChatWindowEntries)
	{
		delete entry;
	}
	m_vChatWindowEntries.clear();
...
void CChatWindow::Render()
{
	...
	const int scrollValue = pScrollbar->GetValue();
	if (scrollValue < 0 || scrollValue > NUM_OF_MESSAGES + m_iMaxMessages)
		return;
	...
	for (int i = 100 - scrollValue; i < 100 - scrollValue + m_iMaxMessages + 1; i++)
	{
...
void CChatWindow::ReInit()
{
	...
	m_iMaxMessages = std::max(1, std::min(pSettings->GetReadOnly().iChatMaxMessages, NUM_OF_MESSAGES));
...
void CChatWindow::AddInfoMessage(char* szFormat, ...)
{
	...
	vsnprintf(tmp_buf, sizeof(tmp_buf), szFormat, args);
	va_end(args);
...
uint8_t CChatWindow::GetAlphaFromLastTimePushedMessage()
{
	...
	const uint32_t elapsed = GetTickCount() - m_uiLastTimePushedMessage;
	if (elapsed >= m_uiTimeChatHideBegin)
	{
		uint32_t time = elapsed - m_uiTimeChatHideBegin;
		uint32_t distance = (m_uiMaxTimeChatHide > m_uiTimeChatHideBegin)
			? (m_uiMaxTimeChatHide - m_uiTimeChatHideBegin)
			: 0;
		if (!distance)
		{
			return 0xFF;
	...
	uint32_t dwTextColor, uint32_t dwNickColor)
{
	...
	if (szNick)
	{
		snprintf(entry.szNick, sizeof(entry.szNick), "%s:", szNick);
	}
	else
	{
		entry.szNick[0] = '\0';
