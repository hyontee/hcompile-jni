...
{
	pChatWindow->AddInfoMessage("Потеряно соеденение с сервером, переподключение...");
...
void ConnectionRejected(RPCParameters *rpcParams)
{
	...
	{
		pChatWindow->AddInfoMessage("С данным никнеймом зайти в игру нельзя");
		pChatWindow->AddInfoMessage("Возможные причины:");
		pChatWindow->AddInfoMessage("1) На сервере кто-то играет под этим никнеймом");
		pChatWindow->AddInfoMessage("2) В никнейме указаны недопустимые символы");
		pChatWindow->AddInfoMessage("Или просто попробуйте перезайти в игру");
	}
