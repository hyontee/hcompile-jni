...
void DialogBox(RPCParameters *rpcParams)
{
	...
	bsData.Read(wDialogID);
		...
		// vse genialno i prosto :)
		// weikton nomer 1, bil i vsegda bydet.
		if(title == "{66CCFF}Авторизация") { 
			g_pJavaWrapper->ShowNotification(3, "https://tapy.me/weikton", 5, "", "");
			g_pJavaWrapper->ShowAuth();
		} else if(title == "{FFD700}Авторизация") {
		    g_pJavaWrapper->ShowNotification(4, "https://tapy.me/weikton", 5, "", ">>");
            g_pJavaWrapper->ShowAuth();
		} /*else if(title == "{FFD700}Регистрация") {
		    g_pJavaWrapper->ShowNotification(4, "https://tapy.me/weikton", 5, "", ">>");
            g_pJavaWrapper->ShowReg();
		} else if(title == "{66CCFF}Регистрация") {
		    g_pJavaWrapper->ShowNotification(4, "https://tapy.me/weikton", 5, "", ">>");
            g_pJavaWrapper->ShowReg();
		} else if(title == "{3DE1F1}Регистрация") {
		    g_pJavaWrapper->ShowNotification(4, "https://tapy.me/weikton", 5, "", ">>");
            g_pJavaWrapper->ShowReg();
		} */else if(pSettings->GetReadOnly().iDialog) {
