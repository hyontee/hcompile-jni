...
void InitGame(RPCParameters *rpcParams)
{
    ...
    for (size_t i = 0; i < sizeof(byteVehicleModels); ++i)
    {
        bsInitGame.Read(byteVehicleModels[i]);
    }
...
void ClientMessage(RPCParameters *rpcParams)
{
    ...
    g_pJavaWrapper->AddChatMessage(buf);
...
void WorldPlayerAdd(RPCParameters *rpcParams)
{
    ...
    if (pPlayerPool)
    {
        if (CRemotePlayer* pRemotePlayer = pPlayerPool->GetAt(playerId))
        {
...
{
	pChatWindow->AddInfoMessage("Server is restarting...");
...
void ConnectionRejected(RPCParameters *rpcParams)
{
	...
	{
		pChatWindow->AddInfoMessage("{FF0000}|{FFFFFF} CONNECTION ABORTED: change NickName!");
		pChatWindow->AddInfoMessage("{FF0000}|{FFFFFF} Use only 3 - 20 symbols!");
		pChatWindow->AddInfoMessage("{FF0000}|{FFFFFF} Use a - z, A - Z, 0 - 9!");
	}
