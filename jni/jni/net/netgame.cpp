...
CNetGame::CNetGame(const char* szHostOrIp, int iPort, const char* szPlayerName, const char* szPass)
{
	...
	if(pChatWindow) pChatWindow->AddDebugMessage("{c0c0c0} ÎËÂÌÚ {FF0000}LIVE RUSSIA BONUS {c0c0c0}Á‡ÔÛ˘ÂÌ...");
}
...
void CNetGame::Process()
{
		...
		(GetTickCount() - m_dwLastConnectAttempt) > 3000)
	{
		...
		if(pChatWindow) pChatWindow->AddDebugMessage("{c0c0c0}—ÓÂ‰ËÌÂÌËÂ Í LIVE RUSSIA BONUS");
...
void CNetGame::UpdateNetwork()
{
	...
	while(pkt = m_pRakClient->Receive())
	{
		...
		switch(packetIdentifier)
		{
			...
			pChatWindow->AddDebugMessage("òòòòòòòòòòòò òòòòòò! òòòòòòòòòòò.");
			m_pRakClient->Disconnect(0);
			break;
...
#define RPC_UPDATE_LVL_INFO 0x30
#define RPC_JAVA_GREEN_ZONE 0x31
#define RPC_SHOW_NOTIFICATION 0x32
#define RPC_SHOW_ADMIN_SPEC 0x33
#define RPC_IPACKET_ADMIN_RECON 0x34
#define RPC_SHOW_STYLING_CENTER 0x35
#define RPC_IPACKET_STYLING_CENTER 0x36
#define RPC_IPACKET_RGB_COLOR_LIGHT 0x37
#define RPC_IPACKET_STYLING_CENTER_NOT_COST 0x38
#define RPC_HIDE_STYLING_CENTER 0x39
#define RPC_SHOW_ACHIEVEMENTS 0x40
#define RPC_IPACKET_GIVE_COINS 0x41
#define RPC_IPACKET_BUY_EVENT_ITEM 0x42
#define RPC_SHOW_MAGIC_STORE 0x43
#define RPC_SHOW_CHOOSE_SPAWN 0x44
...
void CNetGame::Packet_CustomRPC(Packet* p)
{
	...
	/*		uint16_t id;
		...
		case RPC_UPDATE_LVL_INFO:
		{
			...
			g_pJavaWrapper->UpdateLvl(lvl, exp, max_exp);
			break;
		}
		case RPC_JAVA_GREEN_ZONE:
		{
			int is_show;
			bs.Read(is_show);

			g_pJavaWrapper->JavaGreenZone(is_show);
			break;
		}
		case RPC_SHOW_NOTIFICATION: 
		{
			char szBuff[4096+1];
			char text[64*54];
			char positive_command[64*54];
			char negative_command[64*54];
			...
			//òòòòòòòò òòò òòòòòò
			uint16_t lenPositiveCcommand;
			bs.Read(lenPositiveCcommand);

			memset(negative_command, 0, sizeof(positive_command));
			memset(szBuff, 0, sizeof(szBuff));

			bs.Read(szBuff, lenPositiveCcommand);
			cp1251_to_utf8(positive_command, szBuff);

			//òòòòò òòò òòòòòò
			uint16_t lenNegativeCommand;
			bs.Read(lenNegativeCommand);

			memset(negative_command, 0, sizeof(negative_command));
			memset(szBuff, 0, sizeof(szBuff));

			bs.Read(szBuff, lenNegativeCommand);
			cp1251_to_utf8(negative_command, szBuff);

			g_pJavaWrapper->ShowNotification(type, text, duration, negative_command, negative_command);
			return;
		}
		case RPC_SHOW_ADMIN_SPEC:
		{
			char szBuff[4096+1];
			char name[64*54];
			...
			g_pJavaWrapper->ShowAdminRecon(name, id);
			return;
		}
		case RPC_SHOW_STYLING_CENTER: 
		{
			int money;
			bs.Read(money);

			g_pJavaWrapper->ShowStylingCenter(money);
			break;
		}
		case RPC_HIDE_STYLING_CENTER: 
		{
			g_pJavaWrapper->HideStylingCenter();
			break;
		}
		case RPC_SHOW_ACHIEVEMENTS:
		{
			int progress_1;
			bs.Read(progress_1);
			...
			g_pJavaWrapper->ShowAchievements(progress_1, progress_2, progress_3, progress_4, progress_5);
			break;
		}
		case RPC_SHOW_MAGIC_STORE:
		{
			int bronze_coins;
			bs.Read(bronze_coins);

			int silver_coins;
			bs.Read(silver_coins);

			int gold_coins;
			bs.Read(gold_coins);

			g_pJavaWrapper->ShowMagicStore(bronze_coins, silver_coins, gold_coins);
			break;
		}
		case RPC_SHOW_CHOOSE_SPAWN:
		{
			int organizationstate;
			bs.Read(organizationstate);

			int stationstate;
			bs.Read(stationstate);
			...
			g_pJavaWrapper->ShowChooseSpawn(organizationstate, stationstate, exitstate, garagestate, housestate);
			break;
		}
	}
...
void CNetGame::SendReconIpacket(int id) 
{
	RakNet::BitStream bsSend;
	bsSend.Write((uint8_t)  0x34);
	bsSend.Write((uint32_t) RPC_IPACKET_ADMIN_RECON);
	bsSend.Write((int) id);

	GetRakClient()->Send(&bsSend, MEDIUM_PRIORITY, RELIABLE, 0);
}
void CNetGame::SendStylingIpacket(int id) 
{
	RakNet::BitStream bsSend;
	bsSend.Write((uint8_t)  0x36);
	bsSend.Write((uint32_t) RPC_IPACKET_STYLING_CENTER);
	bsSend.Write((int) id);

	GetRakClient()->Send(&bsSend, MEDIUM_PRIORITY, RELIABLE, 0);
}
void CNetGame::SendIpacketColorPickerRgb(int red, int green, int blue) 
{
	RakNet::BitStream bsSend;
	bsSend.Write((uint8_t)  0x37);
	bsSend.Write((uint32_t) RPC_IPACKET_RGB_COLOR_LIGHT);
	bsSend.Write((int) red);
	bsSend.Write((int) green);
	bsSend.Write((int) blue);

	GetRakClient()->Send(&bsSend, MEDIUM_PRIORITY, RELIABLE, 0);
}
void CNetGame::SendStylingIpacketNotPay(int cost) 
{
	RakNet::BitStream bsSend;
	bsSend.Write((uint8_t)  0x38);
	bsSend.Write((uint32_t) RPC_IPACKET_STYLING_CENTER_NOT_COST);
	bsSend.Write((int) cost);

	GetRakClient()->Send(&bsSend, MEDIUM_PRIORITY, RELIABLE, 0);
}
void CNetGame::SendIpacketGiveLiveCoins(int coins) 
{
	RakNet::BitStream bsSend;
	bsSend.Write((uint8_t)  0x41);
	bsSend.Write((uint32_t) RPC_IPACKET_GIVE_COINS);
	bsSend.Write((int) coins);

	GetRakClient()->Send(&bsSend, MEDIUM_PRIORITY, RELIABLE, 0);
}
void CNetGame::SendQueryBuyEventItem(int cost) 
{
	RakNet::BitStream bsSend;
	bsSend.Write((uint8_t)  0x42);
	bsSend.Write((uint32_t) RPC_IPACKET_BUY_EVENT_ITEM);
	bsSend.Write((int) cost);
...
{
	if(pChatWindow)
		pChatWindow->AddDebugMessage("{c0c0c0}—ÓÂ‰ËÌÂÌÓ. œÓ‰„ÓÚÓ‚Í‡ Í Ë„Â...");
