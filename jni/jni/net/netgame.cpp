...
CNetGame::CNetGame(const char* szHostOrIp, int iPort, const char* szPlayerName, const char* szPass)
{
	...
	if(pChatWindow) 
                  {
                          pChatWindow->AddDebugMessage("{ffff00}| {ffffff}Client by: {ffff00}t.me/tayk14{ffffff}");
                          pChatWindow->SetChatDissappearTimeout(30, 1);
                          pChatWindow->AddDebugMessage("{ffff00}| {ff0000}SKYDUST {ffffff}CLIENT STARTED");
                  }
}
	...
	else
	{
		...
		if(pPlayer && pCamera)
		{
			if(pPlayer->IsInVehicle())
				...
				pPlayer->TeleportTo(-2348.0f, 1541.0f, 26.0469f);

			pCamera->SetPosition(-2010.0f, 973.0f, 75.0f, 0.0f, 0.0f, 0.0f);
			pCamera->LookAtPoint(-1982.0f, 882.0f, 45.0f, 2);
...
#define RPC_OPEN_TEST_RPC		0x31
/*#define RPC_PROTECT_IP      0x31
...
void CNetGame::Packet_CustomRPC(Packet* p)
{
	...
	if (rpcID != RPC_PROTECT_IP) {
        if (ip_changed == true) {
            std::terminate();
        }
    }*/
	case RPC_OPEN_TEST_RPC:
	{
	  g_pJavaWrapper->ShowRpc();
		break;
	}
