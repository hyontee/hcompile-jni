...
CNetGame::CNetGame(const char* szHostOrIp, int iPort, const char* szPlayerName, const char* szPass)
{
	...
	if(pChatWindow) 
                  {
                          //pChatWindow->AddDebugMessage("{ffff00}| {ffffff}CLIENT BY: {ffff00}157x157{ffffff}");
                          pChatWindow->SetChatDissappearTimeout(30, 1);
                         // pChatWindow->AddDebugMessage("{ffff00}|{ffffff} CLIENT STARTED");
                  }
}
...
void CNetGame::Process()
{
	...
	else
	{
		...
		if(pPlayer && pCamera)
		{
			...
			pCamera->SetPosition(-0.0f, 0.0f, -1000.0f, 0.0f, 0.0f, 0.0f);
			pCamera->LookAtPoint(0.0f, 0.0f, -1000.0f, 2);
