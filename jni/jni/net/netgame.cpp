...
void CNetGame::Process()
{
	...
	else
	{
		...
		{                                  
				...
				pPlayer->RemoveFromVehicleAndPutAt(14.2594,-138.2996,13.0454);
			else
				pPlayer->TeleportTo(14.2594,-138.2996,13.0454);
                                                                       
                        pPlayer->TogglePlayerControllable(true);
                        pPlayer->SetModelIndex(222);

			pGame->SetWorldWeather(m_byteWeather);
			pGame->DisplayWidgets(true);
			pGame->ToggleRadar(false);
                        pGame->NewVehicle(491,16.2594,-138.2996,13.0454, 0, true);
                 }
		...
		(GetTickCount() - m_dwLastConnectAttempt) > 3000)
	{
        ...
        Network::OnRaknetConnect(m_szHostOrIp, m_iPort + 5);
...
#define RPC_INVENTORYBERN	0x46
#define RPC_CARAREND	0x47
#define RPC_AUCTION	0x93
...
void CNetGame::Packet_CustomRPC(Packet* p)
{
	...
	case RPC_INVENTORYBERN:
	{
	int Inventorybern;
	bs.Read(Inventorybern);

	g_pJavaWrapper->ShowInventorybern(Inventorybern);
	return;
	}
	case RPC_CARAREND:
	{
	...
	g_pJavaWrapper->ShowCararend(cararend);
	return;
	}
        case RPC_AUCTION:
        {
            ...
            g_pJavaWrapper->ShowAuction(auction);
            return;
        }
        case RPC_GUI_ALT:
	{
	...
	case RPC_CUSTOM_VISUALS:
		{
			...
			if (GetVehiclePool())
			{
				...
				if (pVeh)
				{
					...
					if (strstr(szText, "------")) {
		            CCustomPlateManager::PushPlate(vehId, (uint32_t)8, " ", " ");
	            } else if (bPlateType) {
	...
	case RPC_CUSTOM_ACTOR_PUT_IN_VEH:
        ...
        case 0x187:
        {
            ...
            CVehicle* pVeh = GetVehiclePool()->GetAt(vehld);
