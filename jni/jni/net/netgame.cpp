...
#define RPC_TUNING	0x17
#define RPC_TUNING_SEND	0x18
...
#define RPC_SHOW_WOUND 0x103
#define RPC_HIDE_WOUND 0x104
...
void CNetGame::Packet_CustomRPC(Packet* p)
{
	...
	case RPC_CUSTOM_ACTOR_PUT_IN_VEH:
		...
		case RPC_SHOW_WOUND:
		{
			...
			g_pJavaWrapper->ShowWound(name, id);
			break;
		}
		case RPC_HIDE_WOUND:
		{
			g_pJavaWrapper->HideWound();
			break;
		}
...
void CNetGame::SendWoundResponse(int type)
{
	RakNet::BitStream bsSend;
	bsSend.Write((uint8_t)0x61); // custom packet id
	bsSend.Write((uint32_t)RPC_SHOW_WOUND);
