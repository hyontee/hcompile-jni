...
CNetGame::CNetGame(const char* szHostOrIp, int iPort, const char* szPlayerName, const char* szPass)
{
	...
	if(pChatWindow) {
                          pChatWindow->AddDebugMessage("{FF0000}LIVE RUSSIA {FFFFFF}Started"); 
                          //pChatWindow->AddDebugMessageNonFormatted(CLocalisation::GetMessage(E_MSG::NEW_C));
	}
