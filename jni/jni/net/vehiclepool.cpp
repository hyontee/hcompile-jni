...
bool CVehiclePool::New(NEW_VEHICLE *pNewVehicle)
{
	if (!pNewVehicle)
	{
		Log("[AntiCrash] Null NEW_VEHICLE packet. Vehicle spawn skipped.");
		return false;
	}
	...
	{
		Log("[AntiCrash] Bad vehicle slot %u for model %d. Vehicle spawn skipped.", pNewVehicle->VehicleID, pNewVehicle->iVehicleType);
		return false;
	}

	if (!IsValidVehicleModel(pNewVehicle->iVehicleType))
	{
		Log("[AntiCrash] Bad vehicle model id %d at slot %u. Spawn skipped: model not registered in CModelInfo.", pNewVehicle->iVehicleType, pNewVehicle->VehicleID);
		return false;
	}
