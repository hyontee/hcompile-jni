        ...
        if (GetSlotState(x))
        {
            ...
            if (m_bIsActive[x] && pVehicle)
            {
                ...
                if (pVehicle->GetDistanceFromLocalPlayerPed() < vehicleDistance)
                {
                    ...
                    if(pVehicle->IsDriverLocalPlayer())
                    {
                        pVehicle->SetInvulnerable(true); // false
...
bool CVehiclePool::New(NEW_VEHICLE *pNewVehicle)
{
    ...
    //CCustomPlateManager::PushPlate(pNewVehicle->VehicleID, 1, "m121mm", "95");
    ...
    if(m_pVehicles[pNewVehicle->VehicleID])
    {
        ...
        m_pVehicles[pNewVehicle->VehicleID]->SetInvulnerable(true);
...
VEHICLEID CVehiclePool::findSampIdFromGtaPtr(VEHICLE_TYPE *pGtaVehicle)
{
    int x=1;
