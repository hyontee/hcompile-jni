...
public:
    ...
    bool      GetPosition(PLAYERID id, float& x, float& y, float& z);
    ...
    void SetLevel(PLAYERID playerId, int level); // реализация в .cpp
    int  GetLevel(PLAYERID playerId) const;      // реализация в .cpp
...
private:
    ...
    int           m_iPlayerScores[MAX_PLAYERS]{};     // стандартный Score
    uint32_t      m_dwPlayerPings[MAX_PLAYERS]{};     // Ping
    bool          m_bCollisionChecking[MAX_PLAYERS]{};

    // ===== LEVEL STORAGE =====
    int           m_iPlayerLevels[MAX_PLAYERS]{};     // "уровень" игрока (из Recon/RPC и т.п.)
};
