...
void ScrForceSpawnSelection(RPCParameters *rpcParams)
{
	...
	CPlayerPool::GetLocalPlayer()->ReturnToClassSelection();
}
...
void ScrSetCameraLookAt(RPCParameters *rpcParams)
{
	...
	bsData.Read(vecPos.y);
	bsData.Read(vecPos.z);
    if(CPlayerPool::GetLocalPlayer()->m_byteSpectateMode && CPlayerPool::GetLocalPlayer()->m_byteSpectateType != SPECTATE_TYPE_NONE){
        return;
    }
...
void ScrSetFightingStyle(RPCParameters *rpcParams)
{
	...
	else if(CPlayerPool::GetAt(playerId))
...
void ScrSetPlayerSkin(RPCParameters *rpcParams)
{
	...
	{
		if(CPlayerPool::GetAt(iPlayerID) && CPlayerPool::GetAt(iPlayerID)->GetPlayerPed())
...
void ScrSetSpawnInfo(RPCParameters *rpcParams)
{
	...
	CPlayerPool::GetLocalPlayer()->SetSpawnInfo(&SpawnInfo);
...
void ScrSetPlayerSpecialAction(RPCParameters *rpcParams)
{
	...
	CPlayerPool::GetLocalPlayer()->ApplySpecialAction(byteSpecialAction);
}
...
void ScrPutPlayerInVehicle(RPCParameters *rpcParams)
{
	...
	int iVehicleIndex = CVehiclePool::FindGtaIDFromID(vehicleid);
...
void ScrLinkVehicle(RPCParameters *rpcParams)
{
	...
	CVehiclePool::LinkToInterior(VehicleID, (int)byteInterior);
}
...
void ScrSetVehiclePos(RPCParameters *rpcParams)
{
	...
	if(CVehiclePool::GetAt(VehicleId))
		CVehiclePool::GetAt(VehicleId)->TeleportTo(fX, fY, fZ);
}
...
void ScrCreateObject(RPCParameters* rpcParams)
{
    ...
    Log(OBFUSCATE("RPC_SCRCREATEOBJECT(%d) %d %f %f %f"), iTotalObjects, ModelID, vecPos.x,
		vecPos.y,vecPos.z);
	...
	pObject->SetCameraCollision(bNoCameraCol);
...
void ScrAttachObjectToPlayer(RPCParameters* rpcParams)
{
	...
	if (wPlayerID == CPlayerPool::GetLocalPlayerID())
	{
		...
		CRemotePlayer* pPlayer = CPlayerPool::GetAt(wPlayerID);
...
void ScrTogglePlayerControllable(RPCParameters *rpcParams)
{
    ...
    if(byteControllable){
        CPlayerPool::GetLocalPlayer()->GetPlayerPed()->SetLockTogglePlayerControllable(false);
    }
    CPlayerPool::GetLocalPlayer()->GetPlayerPed()->TogglePlayerControllable((int)byteControllable);
    if(!byteControllable){
        CPlayerPool::GetLocalPlayer()->GetPlayerPed()->SetLockTogglePlayerControllable(true);
    }
}
...
void ScrSetPlayerSkillLevel(RPCParameters *rpcParams)
{
	...
	if (bytePlayerID == CPlayerPool::GetLocalPlayerID())
	{
	...
	{
		CRemotePlayer* pRemotePlayer = CPlayerPool::GetAt(bytePlayerID);
...
void ScrSetPlayerAttachedObject(RPCParameters* rpcParams)
{
	...
	{
		if (CPlayerPool::GetAt(id))
		{
			pPed = CPlayerPool::GetAt(id)->GetPlayerPed();
		}
	}
	if (!pPed) return;
	if (!create)
...
void ScrSetPlayerObjectMaterial(RPCParameters* rpcParams)
{
	...
	bsData.Read(col);
	tex[texLength] = 0;
    Log("texture obj %s %d %d", tex, modelId, matId);
...
void ScrMoveObject(RPCParameters* rpcParams)
{
    ...
    Log("MoveObject %d %f %f %f %f %f %f %f %f %f %f",
        byteObjectID, curx, cury, curz, newx, newy, newz, speed,
        rotx, roty, rotz);
...
void ScrSetPlayerArmed(RPCParameters* rpcParams)
{
	...
	CPlayerPool::GetLocalPlayer()->GetPlayerPed()->SetArmedWeapon(weaponID);
}
void ScrDeathMessage(RPCParameters* rpcParams)
{
	...
	if(CPlayerPool::GetLocalPlayerID() == playerId)
	{
		playername = CPlayerPool::GetLocalPlayerName();
		playercolor = CPlayerPool::GetLocalPlayer()->GetPlayerColorAsARGB();
	}
	else
	{
		if(CPlayerPool::GetAt(playerId))
		{
			playername = CPlayerPool::GetPlayerName(playerId);
			playercolor = CPlayerPool::GetAt(playerId)->GetPlayerColorAsARGB();
		}
		else
		{
			playername = "  ";

			if(CPlayerPool::GetLocalPlayerID() != killerId && !CPlayerPool::GetAt(killerId))
			{
				killername = "  ";
				reason = 255;
			}
	...
	if(CPlayerPool::GetLocalPlayerID() == killerId)
	{
		killername = CPlayerPool::GetLocalPlayerName();
		killercolor = CPlayerPool::GetLocalPlayer()->GetPlayerColorAsARGB();
	}
	else
	{
		if(CPlayerPool::GetAt(killerId))
		{
			killername = CPlayerPool::GetPlayerName(killerId);
			killercolor = CPlayerPool::GetAt(killerId)->GetPlayerColorAsARGB();
		}
		else
		{
			killername = "  ";
...
void DisableRemoteVehicleCollisions(RPCParameters *rpcParams)
{
	Log(OBFUSCATE("RPC: DisableRemoteVehicleCollisions"));
	unsigned char* Data = reinterpret_cast<unsigned char *>(rpcParams->input);
	int iBitLength = rpcParams->numberOfBitsOfData;
	RakNet::BitStream bsData((unsigned char*)Data, (iBitLength / 8) + 1, false);
...
void RegisterScriptRPCs(RakClientInterface* pRakClient)
{
	...
	pRakClient->RegisterAsRemoteProcedureCall(&RPC_DisableRemoteVehicleCollision, DisableRemoteVehicleCollisions);
}
...
void UnRegisterScriptRPCs(RakClientInterface* pRakClient)
{
	...
	pRakClient->UnregisterAsRemoteProcedureCall(&RPC_DisableRemoteVehicleCollision);
}