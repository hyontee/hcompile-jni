...
void ScrApplyPlayerAnimation(RPCParameters *rpcParams)
{
    ...
    static char szAnimLib[256] = {0}, szAnimName[256] = {0};
    float fS;
    bool opt[4];
    int opt5;
    ...
    CPlayerPed *pPlayerPed = nullptr;
    PLAYERID localPlayerID = pPlayerPool->GetLocalPlayerID();
    ...
    static std::unordered_map<PLAYERID, std::pair<std::string, std::string>> playerCurrentAnimations;
    auto &currentAnim = playerCurrentAnimations[playerId];

    if (currentAnim.first == szAnimLib && currentAnim.second == szAnimName)
    {
        return;
    }
    ...
    if (pPlayerPed)
    {
        pPlayerPed->ApplyAnimation(szAnimName, szAnimLib, fS, static_cast<int>(opt[0]),
                                   static_cast<int>(opt[1]), static_cast<int>(opt[2]),
                                   static_cast<int>(opt[3]), opt5);

        currentAnim.first = szAnimLib;
        currentAnim.second = szAnimName;
    }
}
...
void ScrSetSpawnInfo(RPCParameters *rpcParams)
{
    ...
    bsData.Read(reinterpret_cast<char*>(&SpawnInfo), sizeof(PLAYER_SPAWN_INFO));
...
void ScrSetPlayerInterior(RPCParameters *rpcParams)
{
    ...
    if (!rpcParams || !rpcParams->input || rpcParams->numberOfBitsOfData <= 0) return;

    unsigned char* Data = reinterpret_cast<unsigned char *>(rpcParams->input);
    int iBitLength = rpcParams->numberOfBitsOfData;

    RakNet::BitStream bsData(Data, (iBitLength / 8) + 1, false);
    uint8_t byteInterior = 0;
...
std::unordered_map<uint16_t, bool> modelCache;
std::unordered_map<std::string, RwTexture*> textureCache;
...
void ScrCreateObject(RPCParameters* rpcParams)
{
    ...
    if (iMaterialCount > 0) {
        for (int i = 0; i < iMaterialCount; i++) {
            ...
            modelId = (modelId < 0 || modelId > 20000) ? 18631 : modelId;
            ...
            if (modelCache.find(modelId) == modelCache.end()) {
                ...
                modelCache[modelId] = true;
            }

            pObject->m_bMaterials = true;
            pObject->m_pMaterials[slot].wModelID = modelId;

            std::string texKey = std::string(str) + "_" + tex;
            if (textureCache.find(texKey) == textureCache.end()) {
                textureCache[texKey] = (RwTexture*)GetTextureFromTXDStore(str, tex);
            }

            pObject->m_pMaterials[slot].pTex = textureCache[texKey];
...
void ScrPlayAudioStream(RPCParameters* rpcParams)
{
	...
	if (pAudioStream)
	{
		...
		if (pChatWindow) {
			//pChatWindow->AddInfoMessage("", szURL);
		}
	}
	else
	{
