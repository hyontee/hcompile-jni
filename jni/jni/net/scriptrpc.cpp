...
static bool IsValidSkinModel(unsigned int skinId)
{
	if (skinId >= 20000)
		return false;

	uintptr_t* modelInfo = (uintptr_t*)(g_libGTASA + 0x87BF48);
	return modelInfo[skinId] != 0;
}
...
void ScrSetPlayerSkin(RPCParameters *rpcParams)
{
	...
	if (!IsValidSkinModel(uiSkin))
	{
		Log("[AntiCrash] Bad skin model id %u for player %d. Skin change skipped: model not registered in CModelInfo.", uiSkin, iPlayerID);
		if (pChatWindow)
		{
			pChatWindow->AddDebugMessage("[AntiCrash] Bad skin id %u for player %d. Skipped.", uiSkin, iPlayerID);
		}
		return;
	}
	...
	if (!pPlayerPool)
	{
		Log("[AntiCrash] ScrSetPlayerSkin skipped: player pool is null.");
		return;
	}

	if(iPlayerID == pPlayerPool->GetLocalPlayerID())
	{
		if (pPlayerPool->GetLocalPlayer() && pPlayerPool->GetLocalPlayer()->GetPlayerPed())
			pPlayerPool->GetLocalPlayer()->GetPlayerPed()->SetModelIndex(uiSkin);
		else
			Log("[AntiCrash] ScrSetPlayerSkin skipped: local player ped is null.");
	}
	else
	{
