...
void CPlayerPed::GetTransformedBonePosition(int iBoneID, CVector* vecOut)
{
	...
	(( void (*)(PED_TYPE*, CVector*, int, int))(g_libGTASA+0x4383C0+1))(m_pPed, vecOut, iBoneID, 0);
}
				...
				if (pPlayerPed)
				{
					...
					//if (pPlayerPed->GetCurrentWeapon() != WEAPON_SNIPER || btData->pEntity)
					{
						if (btData->pEntity)
						{
							...
							PlayerID = CPlayerPool::FindRemotePlayerIDFromGtaPtr((PED_TYPE*)btData->pEntity);
							if (PlayerID == INVALID_PLAYER_ID)
							{
								VehicleID = CVehiclePool::FindIDFromGtaPtr((VEHICLE_TYPE*)btData->pEntity);
								if (VehicleID == INVALID_VEHICLE_ID)
								{
									ObjectID = CObjectPool::FindIDFromGtaPtr(btData->pEntity);
...
void CPlayerPed::ApplyCommandTask(char* a2, int a4, int a5, int a6, CVector* a7, char a8, float a9, int a10, int a11, char a12)
{
	...
	// 00463188 addr
	((int(*)(uintptr_t a1, char* a2, uint32_t a3, int a4, int a5, int a6, CVector* a7, char a8, float a9, int a10, int a11, char a12))(SA_ADDR(0x463188 + 1)))
...
void CPlayerPed::DeattachObject(int iSlot)
{
	if (m_aAttachedObjects[iSlot].bState)
	{
		if(m_aAttachedObjects[iSlot].pColoredObject){
			CObjectPool::RemoveColoredObject(m_aAttachedObjects[iSlot].pColoredObject);
...
RwMatrix* RwMatrixDivisionByVector(CVector* out, RwMatrix* a2, RwMatrix* in)
{
    RwMatrix* result;
    CVector* v4;

    result = a2;
    CVector vecOne{1.0f, 1.0f, 1.0f};
    CVector vecOut;
...
{
	((int(*)(RwMatrix*, CVector*, float, int))(SA_ADDR(0x1B9118 + 1)))(pMat, axis, angle, 1);
}
			...
			*(uint32_t*)((uintptr_t)pObject->m_pEntity + 28) &= 0xFFFFFFFE; // disable collision
			...
			if(pColoredObject){
				if (pColoredObject->m_pEntity->m_RwObject)
				{
					if (pColoredObject->m_pEntity->mat)
					{
						...
						if (v8)
						{
							((int(*)(RwMatrix*, uintptr_t))(SA_ADDR(0x3E862C + 1)))(pColoredObject->m_pEntity->mat, v8); // CEntity::UpdateRwFrame
						}
					}
				}
			}
			...
			if (pObject->m_pEntity->m_RwObject)
			{
				if (pObject->m_pEntity->mat)
				{
					...
					if (v8)
					{
						((int(*)(RwMatrix*, uintptr_t))(SA_ADDR(0x3E862C + 1)))(pObject->m_pEntity->mat, v8); // CEntity::UpdateRwFrame
					}
				}
			}
	...
	if(m_pPed) 
	{
		...
		if(byteDeathReason < WEAPON_CAMERA || byteDeathReason == WEAPON_HELIBLADES || byteDeathReason == WEAPON_EXPLOSION) { // It's a weapon of some sort.
			...
			if(m_pPed->pdwDamageEntity) { // check for a player pointer.
				...
				bytePlayerIDWhoKilled = CPlayerPool::FindRemotePlayerIDFromGtaPtr((PED_TYPE*)m_pPed->pdwDamageEntity);
				...
				if(bytePlayerIDWhoKilled != INVALID_PLAYER_ID) {
					...
					*nPlayer = bytePlayerIDWhoKilled;
					...
					if(CVehiclePool::FindIDFromGtaPtr((VEHICLE_TYPE *)m_pPed->pdwDamageEntity) != INVALID_VEHICLE_ID) {
						VEHICLE_TYPE *pGtaVehicle = (VEHICLE_TYPE *)m_pPed->pdwDamageEntity;
						bytePlayerIDWhoKilled = CPlayerPool::FindRemotePlayerIDFromGtaPtr((PED_TYPE*)pGtaVehicle->pDriver);
			...
			*nPlayer = INVALID_PLAYER_ID;
			...
			if(m_pPed->pdwDamageEntity) {
				...
				if(CVehiclePool::FindIDFromGtaPtr((VEHICLE_TYPE *)m_pPed->pdwDamageEntity) != INVALID_VEHICLE_ID)
				{
					...
					bytePlayerIDWhoKilled = CPlayerPool::
						FindRemotePlayerIDFromGtaPtr(pGtaVehicle->pDriver);
...
void CPlayerPed::GetBonePosition(int iBoneID, CVector* vecOut)
{
	...
	(( void (*)(PED_TYPE*, CVector*, int, int))(SA_ADDR(0x436590 + 1)))(m_pPed, vecOut, iBoneID, 0);
}
