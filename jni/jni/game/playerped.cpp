...
CVehicle* CPlayerPed::GetCurrentVehicle()
{
    ...
    for(const auto & pVehicle : pVehiclePool->m_pVehicles)
    {
        ...
        if(reinterpret_cast<uint32_t>(pVehicle->m_pVehicle) == m_pPed->pVehicle)
            return pVehicle;
    }
    return nullptr;
}
...
{
    Log("CPlayerPed::SetInitialState() called");

    return reinterpret_cast<int(*)(PED_TYPE*, bool)>(g_libGTASA + 0x458D1C + 1)(m_pPed, false);
}
