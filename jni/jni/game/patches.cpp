...
void ApplyPatches_level0()
{
	...
	*(int**)(g_libGTASA + 0x0063D4F0) = &test_pointersLibArray[0];*/
    ...
    makeNOP(g_libGTASA + 0x339240, 7); // фикс тряски
    makeNOP(g_libGTASA + 0x4E55E2, 2);
	...
	makeNOP(g_libGTASA + 0x00185DE8, 2);//_rwOpenGLNativeTextureReadXBOXPvS_
	makeNOP(g_libGTASA + 0x002F9E5C, 10);
	makeNOP(g_libGTASA + 0x0040E536, 2);
	makeNOP(g_libGTASA + 0x00398972, 2); // get out fucking roadblocks

    WriteMemory(g_libGTASA + 0x1A2976, (uintptr_t)"\x41\xF2\x00\x11", 4u);	// RQ_Command_rqInit
    WriteMemory(g_libGTASA + 0x3E1A2C, (uintptr_t)"\x67\xE0", 2);
    WriteMemory(g_libGTASA + 0x1BDD4A, (uintptr_t)"\x10\x46\xA2\xF1\x04\x0B", 6u);
}
...
void ApplyDefaultPlatePatch()
{
	...
	RET(g_libGTASA + 0x0052FF5C); //CCustomCarPlateMgr::GeneratePlateText

	RET(g_libGTASA + 0x0052FE90);
	RET(g_libGTASA + 0x00530098);
	RET(g_libGTASA + 0x00530104);
	RET(g_libGTASA + 0x0053012C);
	RET(g_libGTASA + 0x00530158);
	RET(g_libGTASA + 0x00530190);

	RET(g_libGTASA + 0x0053021C);
	RET(g_libGTASA + 0x0053038C);
	RET(g_libGTASA + 0x00530420);

	RET(g_libGTASA + 0x0050E37C);
}
...
void ApplyPatches()
{
	...
	if(pSettings->GetReadOnly().iGraphics == 2)
	{
		// fix black roads
	...
	ATOMIC_MODELS = new _ATOMIC_MODEL[25000];
	...
	makeNOP(g_libGTASA + 0x261A50, 2); // Language
    makeNOP(g_libGTASA + 0x2661DA, 2); // Display - Shadows (Фикс фпс на фоне теней)
    //makeNOP(g_libGTASA + 0x2661A2, 2); // Display - Дальность прорисовки
    //makeNOP(g_libGTASA + 0x26612E, 2); // Display - Визуальные эффекты
    makeNOP(g_libGTASA + 0x2665EE, 2); // Game - SocialClub

    // From CGame::Init2
    makeNOP(g_libGTASA + 0x00408706, 2); // CClothes::Init
    RET(g_libGTASA + 0x003F1030); // CClothes::RebuildPlayer
  ...
  sub_45A32(dword_14A458 + 0x2DC8E0, nullsub_39, 2);
	*/
	...
	RET(g_libGTASA + 0x4DD00C); // CAutomobile::CustomCarPlate_BeforeRenderingStart
	RET(g_libGTASA + 0x4DD038); // CAutomobile::CustomCarPlate_AfterRenderingStop

	WriteMemory(g_libGTASA + 0x293218, "\x01\x20\x70\x47", 4); 					// CStreaming::RemoveLoadedVehicle
    ...
    //makeNOP(g_libGTASA + 0x39ADE6, 2);	// CCoronas::RenderSunReflection
	...
	*/
	...
	// nop render moon
	//makeNOP(g_libGTASA + 0x529E84, 2);
	//makeNOP(g_libGTASA + 0x529F4A, 2);
	//makeNOP(g_libGTASA + 0x529FEC, 2);

    // pause patch
    WriteMemory(g_libGTASA + 0x56CD78, (uintptr_t)"\x00\x22", 2);

    //RET(g_libGTASA + 0x5292CC); // CClouds::Render
    RET(g_libGTASA + 0x52CDCC); // CClouds::MovingFogRender

    // не давать оружие при выходе из машины
    makeNOP(g_libGTASA + 0x0051018A, 2);
    makeNOP(g_libGTASA + 0x005101A6, 2);

    int refl = 0;
    //WriteMemory(g_libGTASA + 0x00518CB2, &refl, 1);

    RET(g_libGTASA + 0x0051DEC4);
}
...
void ApplyInGamePatches()
{
    ...
    *(float*)(g_libGTASA + 0x608558) = 0.4f; // Эффект разгона авто
	...
	// legend in radar
	makeNOP(g_libGTASA + 0x26B504, 2);
	makeNOP(g_libGTASA + 0x26B514, 2);
