...
static uint8_t ClampSampFps(int fps)
{
    if (fps < 40)
        return 40;
...
void ApplyFPS()
{
    ...
    if (pSettings)
    {
        fps = ClampSampFps(pSettings->GetReadOnly().iFPS);
    }
...
void patches_unlock_limit()
{
    Log("[patches_unlock_limit] Applying stable GTA:SA/SAMP limits unlock.");

    // Keep CStreaming from rewriting ms_memoryAvailable with a low device value.
    CHook::NOP(g_libGTASA + 0x005DE734, 2);

    // Give world collision checks enough headroom for custom maps.
    *(float*)(g_libGTASA + 0x8B08F4) = 300.00001f;

    // Collision links limit.
    WriteMemory(g_libGTASA + 0x1A7DE8, (uintptr_t)"\x01\x22", 2);

    // CdStreamInit(6) keeps streaming smoother on SAMP maps.
    WriteMemory(g_libGTASA + 0x3981EC, (uintptr_t)"\x06\x20", 2);

    // Touch pointer arrays for multi-touch stability.
