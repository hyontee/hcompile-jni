...
std::string CCustomPlateManager::nomer = "";
std::string CCustomPlateManager::region = "";

bool CCustomPlateManager::isActive = false;
...
{
	char path[0xFF];
	sprintf(path, "%smultiplayer/plates/ru_font.ttf", g_pszStorage);

	m_pRUFont = CFontRenderer::AddFont(path, 128);
	m_pKZFont = CFontRenderer::AddFont(path, 132);

	sprintf(path, "%smultiplayer/plates/ua_font.ttf", g_pszStorage);
...
const char* CCustomPlateManager::GetNumberByVehicleId(uint16_t vehicleId)
{
    for (auto& plate : m_Plates)
    {
        if (plate.vehicleId == vehicleId)
            return plate.GetNumber();
    }
    return "";
}

const char* CCustomPlateManager::GetRegionByVehicleId(uint16_t vehicleId)
{
    for (auto& plate : m_Plates)
    {
        if (plate.vehicleId == vehicleId)
...
void CCustomPlateManager::PushPlate(uint16_t vehicleId, uint32_t dwType, char* szNumber, char* szRegion)
{
    ...
    plate.dwType = dwType;
    strncpy(plate.szNumber, szNumber, sizeof(SCustomPlate::szNumber) - 1);
    plate.szNumber[sizeof(SCustomPlate::szNumber) - 1] = '\0';

    if (szRegion)
    {
        strncpy(plate.szRegion, szRegion, sizeof(SCustomPlate::szRegion) - 1);
        plate.szRegion[sizeof(SCustomPlate::szRegion) - 1] = '\0';
    }

    m_Plates.push_back(plate);

    Log("PushPlate - Plate: %s, Region: %s", plate.szNumber, plate.szRegion);
    nomer = plate.szNumber;
    region = plate.szRegion;
}
