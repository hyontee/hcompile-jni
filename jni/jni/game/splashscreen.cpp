...
const int numTextures = sizeof(textureNames) / sizeof(textureNames[0]);
...
{
    Log("[DEBUG] Загрузка текстуры заставки..");

    // Получаем случайный индекс текстуры
    int randomIndex = GetRandomNumber(0, numTextures - 1);
    std::string textureName = textureNames[randomIndex];

    // Загружаем выбранную текстуру
    splashTexture = (RwTexture*)LoadTextureFromDB("samp", textureName.c_str());
...
void RenderSplash()
{
	...
	rect.x1 = RsGlobal->maximumWidth * 0.050f;
	rect.y1 = RsGlobal->maximumHeight * 0.940f;
	rect.x2 = (RsGlobal->maximumWidth * 0.955f) * mult;
	rect.y2 = RsGlobal->maximumHeight * 0.935f;

	Draw(&rect, COLOR_BRED);
}
...
void RenderSplashScreen()
{
	...
	sprintf(szLoadingPercent, "Загрузка...");
	if(percent <= 1.0)
		sprintf(szLoadingPercent, "Загрузка Ресурсов");
	if(percent >= 95.0)
		sprintf(szLoadingPercent, "Ресурсы Загружены.");

	ImGui::GetBackgroundDrawList()->AddText(ImVec2(pGUI->ScaleX(30.0f), pGUI->ScaleY(15.0f)), COLOR_GREY, szLoadingPercent
	);
