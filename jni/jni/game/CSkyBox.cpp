...
CObject* CSkyBox::CreateObjectScaled(int iModel, float fScale)
{
	...
	*(uint8_t*)((uintptr_t)retn->m_pEntity + 29) |= 1;
	...
	if (retn->m_pEntity->m_RwObject)
	{
		if (retn->m_pEntity->mat)
		{
			uintptr_t v8 = *(uintptr_t*)(retn->m_pEntity->m_RwObject + 4) + 16;
			if (v8)
			{
...
{
	char buff[255];
	snprintf(buff, 255, "%sdata/ctimecy.dat", CMain::GetBaseStorage());
...
void CSkyBox::Process()
{
	...
	{
		WriteMemory(g_libGTASA + 0x005BAF5C, (uintptr_t)"TIMECYC.DAT", 12);
	...
	else if (!m_bEnabled && m_bPendingStatus)
	{
		...
		WriteMemory(g_libGTASA + 0x005BAF5C, (uintptr_t)"CTIMECY.DAT", 12);
