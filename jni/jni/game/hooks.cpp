...
int64_t hook_CChat__AddDebugMessage(const char* format, ...)
{
    ...
    const std::string from = "GEAR RUSSIA";
    const std::string to = "GEAR RUSSIA";
    size_t pos = 0;
...
{
    return pRakClient->Connect(xorstr("188.127.241.74"), 1256
, 0, 0, 5);
}
    ...
    if(pktId == BR_ID_USER_INTERFACE_SYNC) {
        ...
        if(jsonLen > 0) {
            ...
            if(guiId == 10) {
             ...
             //       CChat::AddDebugMessage(xorstr("GEAR RUSSIA"));
