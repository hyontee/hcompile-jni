...
void InitialiseRenderWare_hook()
{
  ...
  //  TextureDatabaseRuntime::Load()
    ...
    (( void (*)(const char*, int, int))(g_libGTASA+0x1BF244+1))("assets", 0, 5);
    //(( void (*)(const char*, int, int))(g_libGTASA+0x1BF244+1))("mta4", 0, 5);
...
void CStreaming_InitImageList_hook()
{
	...
	*(uint32_t*)&ms_files[380] = 0;
    ...
    (( uint32_t (*)(char*, uint32_t))(g_libGTASA+0x28E7B0+1))("TEXDB\\ASSETS.IMG", 1); // CStreaming::AddImageToList
    //(( uint32_t (*)(char*, uint32_t))(g_libGTASA+0x28E7B0+1))("TEXDB\\MTA4.IMG", 1); // CStreaming::AddImageToList
}
...
{	
	NvUtilInit();
	*(char**)(g_libGTASA + 0x5D1608) = "/storage/emulated/0/HOLY/";
}
		...
		if (g_pWidgetManager)
		{
			...
			if (!g_pWidgetManager->GetSlotState(WIDGET_MICROPHONE))
			{
				if (g_IsVoiceServer())
				{
					...
					g_pWidgetManager->GetWidget(WIDGET_MICROPHONE)->SetTexture("mobile", "menu_up");
				}
			}
...
int CWidgetButtonEnterCar_Draw_hook(uintptr_t thiz)
{
	if (g_pWidgetManager)
	{
		...
		if (pWidget)
		{
			    /*sCVehiclePool *pVehiclePool = pNetGame->GetVehiclePool();
                CVehicle *pVehicle;
...
/*	if (g_pWidgetManager) // WIDGET_CHATHISTORY_UP
	{
		PED_TYPE* pPed = GamePool_FindPlayerPed();
		if (g_pWidgetManager->GetSlotState(WIDGET_CHATHISTORY_UP) && pPed)
		{
			static uint32_t lastTick = GetTickCount();
			bool bPressed = false;
			if (g_pWidgetManager->IsTouched(WIDGET_CHATHISTORY_UP) && GetTickCount() - lastTick >= 250)
			{
				bPressed = true;
				lastTick = GetTickCount();
			}

			pChatWindow->AddDebugMessage("{ffff00}|{ffffff} [DEBUG] WIDGET UP");
...
void InstallHooks()
{
	...
	SetUpHook(g_libGTASA + 0x003B1778, (uintptr_t)CRenderer__RenderEverythingBarRoads_hook, (uintptr_t*)&CRenderer__RenderEverythingBarRoads);
	HookCPad();
}
