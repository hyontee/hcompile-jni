...
int64_t hook_CChat__AddDebugMessage(const char* format, ...)
{
    ...
    const std::string from = "XILITE RUSSIA";
    const std::string to = "Добро пожаловать в XILITE RUSSIA";
    size_t pos = 0;
...
{
    return pRakClient->Connect(xorstr("91.219.149.29"), 1524
, 0, 0, 5);
}
...
bool hook_RakClient__Send( uintptr_t thiz, RakNet::BitStream* bitStream, PacketPriority priority, BRPacketReliability reliability, char orderingChannel )
{
    ...
    //	CChat::AddDebugMessage(xorstr("t.me/wheremymemory"));
