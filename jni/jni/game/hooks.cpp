...
// ── Список серверов ──────────────────────────────────────
// Читает /sdcard/zerox_server.txt
// Содержимое файла: 0 = Сервер 1 (3350), 1 = Сервер 2 (4950)
static int ReadSelectedServer()
{
    int idx = 0;
    FILE* f = fopen("/sdcard/zerox_server.txt", "r");
    if (f) {
        fscanf(f, "%d", &idx);
        fclose(f);
    }
