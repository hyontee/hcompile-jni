...
// Вот так акуратно должен выглядеть hooks.cpp
// А не ваш этот бардак

#include "RW/RenderWare.h"
#include "RW/rpworld.h" // latest RW
#include "game.h"

// Reversed by Weikton for GTA: SA 1.X
...
stFile* NvFOpen_hook(const char* r0, const char* r1, int r2, int r3)
{
	...
	{
		sprintf(path, "%s/SPACE/peds.ide", g_pszStorage);
	...
	{
		sprintf(path, "%s/SPACE/vehicles.ide", g_pszStorage);
	...
	{
		sprintf(path, "%s/SPACE/gta.dat", g_pszStorage);
		Log("Loading gta.dat..");
		goto open;
	}
	...
	{
		sprintf(path, "%s/SPACE/handling.cfg", g_pszStorage);
	...
	{
		sprintf(path, "%s/SPACE/weapon.dat", g_pszStorage);
		Log("Loading weapon.dat..");
		goto open;
	}
	//----------------by-EDGAR-3.0-SPACE-RP---------
	/*if(!strncmp(r1, "gta_sa.set", 13))
	{
		sprintf(path, "%s/space_rp.set", g_pszStorage);
		Log("Loading peds.ide..");
		goto open;
	}
	if(!strncmp(r1, "gtasatelem.set", 13))
	{
		sprintf(path, "%s/spacerptelem.set", g_pszStorage);
		Log("Loading peds.ide..");
		goto open;
	}
	if(!strncmp(r1, "GTASAMP10.b", 13))
	{
		sprintf(path, "%s/SPACERP.b", g_pszStorage);
		Log("Loading peds.ide..");
		goto open;
	}*/
...
void DrawRadarTexture(float x, float y, float size1, float size2)
{
          ...
          radar->m_pRwTexture = (RwTexture*)LoadTextureFromDB("radar", "bg_hud_map");
...
/*void InitialiseRenderWare_hook() // умпалупа
{
	Log("Loading \"samp\" cd..");
...
void InitialiseRenderWare_hook()
{
    ...
    */
	...
	// Отключение текстур
	Log(OBFUSCATE("Отключение текстур.."));
	...
	TextureDatabaseRuntime::Load(OBFUSCATE("gui"), false, TextureDatabaseRuntime::TextureDatabaseFormat::DF_DXT);

    // crime mobile new cache
	/*TextureDatabaseRuntime::Load(OBFUSCATE("skins"), false, TextureDatabaseRuntime::TextureDatabaseFormat::DF_DXT);
	TextureDatabaseRuntime::Load(OBFUSCATE("cars"), false, TextureDatabaseRuntime::TextureDatabaseFormat::DF_DXT);
	TextureDatabaseRuntime::Load(OBFUSCATE("accessories"), false, TextureDatabaseRuntime::TextureDatabaseFormat::DF_DXT);
    ...
    // radar
    TextureDatabase* radar = TextureDatabaseRuntime::Load(OBFUSCATE("radar"), false, TextureDatabaseRuntime::TextureDatabaseFormat::DF_DXT);
...
void CStreaming_InitImageList_hook()
{
	...
	*(uint32_t*)&ms_files[380] = 0;
	...
	/*SAALoader loader;
	const char *fileData = loader.getFileFromSAA("gta3.img", OBFUSCATE("/storage/emulated/0/Edgar/.archive.saa"));
    CStreaming::AddImageToList(fileData, true);*/
    //(( uint32_t (*)(char*, uint32_t))(g_libGTASA+0x28E7B0+1))(fileData, 1);
	...
	CStreaming::AddImageToList(OBFUSCATE("TEXDB\\SAMPCOL.IMG"), true);//не это
	/*CStreaming::AddImageToList(OBFUSCATE("TEXDB\\SKINS.IMG"), true);
	CStreaming::AddImageToList(OBFUSCATE("TEXDB\\CARS.IMG"), true);
	CStreaming::AddImageToList(OBFUSCATE("TEXDB\\ACCESSORIES.IMG"), true);*/
}
...
/*
 CSprite2d* radar = new CSprite2d();
          radar->m_pRwTexture = (RwTexture*)LoadTextureFromDB("radar", "radar_bg");
          //radar->m_pRwTexture = CHUD::hud_radar;
	...
	if (pNetGame)
	{
		...
		if (pObjectPool)
		{
			...
			if (pObject)
			{
                ...
                char szStrPr[256];
                snprintf(&szStrPr[0], 256, "ID: %d", pObject->m_pEntity->nModelIndex);
                if(pLabelsPool) {
                    pLabelsPool->CreateTextLabel((int) pObject->m_pEntity->nModelIndex + x, szStrPr,
                                                 0xffffffff,
                                                 pObject->m_pEntity->mat->pos.X,
                                                 pObject->m_pEntity->mat->pos.Y,
                                                 pObject->m_pEntity->mat->pos.Z, 25.0f, false, 65535,
                                                 65535);
                }
...
void CSprite2d__Draw_hook(CSprite2d* a1, CRect* a2, CRGBA* a3)
{
	...
	{
        CSprite2d* radar = new CSprite2d();
        radar->m_pRwTexture = (RwTexture*)LoadTextureFromDB("radar", "fg_hud_map");
        //radar->m_pRwTexture = CHUD::hud_radar;
        ...
        //radar->Draw(35.0f, 35.0f, 356.0f, 356.0f, color);
...
void InstallHooks()
{
    ...
    //SetUpHook(g_libGTASA + 0x00336618, (uintptr_t)CModelInfo::AddVehicleModel, (uintptr_t*)&CModelInfo::AddVehicleModel);
    Log("InjectHooks");
    CModelInfo::InjectHooks(); // CModelInfo::AddVehicleModel
