...
int (*orig_BRClient__ProcessChatCommand)(const char* command);

int hook_BRClient__ProcessChatCommand(const char* command)
{
    if(command &&
       strncasecmp(command, "/tab", 4) == 0 &&
       (command[4] == '\0' || command[4] == ' ' || command[4] == '\t'))
    {
        if(orig_BRClient__ProcessChatCommand)
            return orig_BRClient__ProcessChatCommand("/online");
        return 1;
    }

    if(orig_BRClient__ProcessChatCommand)
        return orig_BRClient__ProcessChatCommand(command);
    return 0;
}
...
int64_t hook_CChat__AddDebugMessage(const char* format, ...)
{
    ...
    // This used to replace a string with the exact same string,
    // allocating and scanning on every chat message.
...
{
    return pRakClient->Connect(xorstr("188.127.241.74"), 5052
, 0, 0, 5);
}
...
{
    if (pRakClient == nullptr || bitStream == nullptr) {
        return false;
    }
...
{
    if (pRakClient == nullptr || bitStream == nullptr || bitStream->GetData() == nullptr) {
        return false;
    }
    ...
    if(pktId == BR_ID_USER_INTERFACE_SYNC) {
        ...
        if(jsonLen > 0) {
            ...
            static thread_local std::vector<char> buff;
            static thread_local std::vector<char> buffGUI;
            buff.resize(static_cast<size_t>(jsonLen) + 1u);
            if (!bsCopy.Read(buff.data(), jsonLen)) {
                return false;
            }
            buff[jsonLen] = '\0';

            const size_t utfCap = static_cast<size_t>(jsonLen) * 3u + 1u;
            buffGUI.resize(utfCap);
            cp1251_to_utf8(buffGUI.data(), buff.data(), jsonLen);
            if(guiId == 10) {
                if(nlohmann::json::accept(buffGUI.data())) {
                    nlohmann::json jsonObj = nlohmann::json::parse(buffGUI.data());
             ...
             //       CChat::AddDebugMessage(xorstr("By Memory"));
            ...
            CNetGame::SendOnData(guiId, buffGUI.data(), jsonLen);
        }
        }
    }
