...
{
	return ((bool (*)(uintptr_t))(g_libGTASA + 0x41DD7D))(a1);
}

static void ApplyLowEndPerformanceModeOnce()
{
	...
	// Strong static low-end profile for weak devices.
	*(int*)(g_libGTASA + 0x63E3A4) = 70;   // resolution
	*(int*)(g_libGTASA + 0x63E3C4) = 55;   // draw distance
	*(int*)(g_libGTASA + 0x63E324) = 0;    // shadows
	*(int*)(g_libGTASA + 0x63E384) = 0;    // effects
	*(int*)(g_libGTASA + 0x63E464) = 0;    // reflections
	*(float*)(g_libGTASA + 0x8B08F4) = 120.0f; // lod / streaming distance

	// Cheap visual cuts with low gameplay risk.
	WriteMemory(g_libGTASA + 0x5286EC, (uintptr_t)"\x70\x47", 2); // CBirds::Render -> bx lr
	WriteMemory(g_libGTASA + 0x51DEC4, (uintptr_t)"\x70\x47", 2); // WaterCreatureManager_c::Update -> bx lr
	NOP(g_libGTASA + 0x54D720, 2); // motion blur streaks
	NOP(g_libGTASA + 0x39B394, 2); // camera motion blur
}

static constexpr bool kDisableCustomPlates = true;
static constexpr bool kDisableSnowEffects = true;
static constexpr bool kDisableVisualExtras = true;
static constexpr bool kDisableWaterReflections = true;
static constexpr bool kEnableVehicleShadows = true;
static constexpr bool kEnableCarReflections = true;
static constexpr bool kEnableSkyVisuals = true;
static constexpr bool kEnablePrettyVisuals = true;
static constexpr bool kDisableSnapshotProcessing = true;
static constexpr bool kDisableAudioStreamProcessing = true;

static void ApplyVisualExtrasOffOnce()
{
	static bool s_applied = false;
	if (s_applied || !kDisableVisualExtras)
		return;

	s_applied = true;

	// Decorative world systems and blur effects.
	WriteMemory(g_libGTASA + 0x5286EC, (uintptr_t)"\x70\x47", 2); // CBirds::Render -> bx lr
	WriteMemory(g_libGTASA + 0x51DEC4, (uintptr_t)"\x70\x47", 2); // WaterCreatureManager_c::Update -> bx lr
	NOP(g_libGTASA + 0x54D720, 2); // motion blur streaks
	NOP(g_libGTASA + 0x39B394, 2); // camera motion blur
...
static void ApplyVehicleShadowModeOnce()
{
	static bool s_applied = false;
	if (s_applied || !kEnableVehicleShadows)
		return;

	s_applied = true;

	// Keep car shadows enabled even with other expensive extras disabled.
	*(int*)(g_libGTASA + 0x63E324) = 2;
}

static void ApplyCarReflectionModeOnce()
{
	static bool s_applied = false;
	if (s_applied || !kEnableCarReflections)
		return;

	s_applied = true;

	// Enable regular envmap reflections for vehicles without bringing water reflections back.
	*(int*)(g_libGTASA + 0x63E464) = 1;
	g_fFxEnvMapLightMult = 0.28f;
}

static void ApplySkyVisualsOnce()
{
	static bool s_applied = false;
	if (s_applied || !kEnableSkyVisuals || !pSettings)
		return;

	s_applied = true;

	// Force custom skybox visuals with a calmer movement speed.
	pSettings->GetWrite().iSkyBox = 1;
	CSkyBox::SetRotSpeed(0.0028f);
}

static void ApplyPrettyVisualsOnce()
{
	static bool s_applied = false;
	if (s_applied || !kEnablePrettyVisuals)
		return;

	s_applied = true;

	// Balanced "pretty + fps" profile.
	*(int*)(g_libGTASA + 0x63E3A4) = 92; // resolution
	*(int*)(g_libGTASA + 0x63E3C4) = 82; // draw distance
	*(int*)(g_libGTASA + 0x63E384) = 2;  // effects
}

static void ApplyPrettySkyPatchOnce()
{
	static bool s_applied = false;
	if (s_applied)
		return;
	...
	// Runtime equivalent of the skybox color patch from patches.cpp.
	const char* newLightingColorString = "Out_LightingColor = AmbientLightColor * 1.18 * MaterialAmbient.xyz";
...
static void ApplyCoreRuntimePatchesOnce()
{
	static bool s_applied = false;
	if (s_applied)
		return;
...
{
	uint8_t* data = reinterpret_cast<uint8_t*>(a1);
	*reinterpret_cast<uintptr_t*>(data + 0x08) = 1;
	*reinterpret_cast<uintptr_t*>(data + 0x00) = off_1F95D4;
	*reinterpret_cast<uintptr_t*>(data + 0x0C) = sub_57DE0();
	*reinterpret_cast<uintptr_t*>(data + 0x04) = sub_57DE0();

	uintptr_t v2 = *reinterpret_cast<uintptr_t*>(data + 0x0C);
	*(data + 0x10) = 0;

	sub_57F64(0, v2);
	sub_55814(unk_1D4EC2, *reinterpret_cast<uintptr_t*>(data + 0x08), 0);

	if (*reinterpret_cast<uintptr_t*>(data + 0x0C) && sub_57DF0(*reinterpret_cast<uintptr_t*>(data + 0x08)))
		sub_55814(unk_1D50BA, *reinterpret_cast<uintptr_t*>(data + 0x08), 1);

	*reinterpret_cast<uintptr_t*>(data + 0x11) = 0;
	*reinterpret_cast<uint16_t*>(data + 0x1D) = 1;
	*reinterpret_cast<uintptr_t*>(data + 0x15) = 0;
	*reinterpret_cast<uintptr_t*>(data + 0x19) = 0;
	*reinterpret_cast<uintptr_t*>(data + 0x1F) = 0x3C03126F;

	memset(data + 0x50, 0, 0x23A);

	reinterpret_cast<void(*)(uintptr_t)>(g_libGTASA + 0x43E201)(*reinterpret_cast<uintptr_t*>(data + 0x0C));
	reinterpret_cast<void(*)(uintptr_t)>(g_libGTASA + 0x43E229)(*reinterpret_cast<uintptr_t*>(data + 0x0C));
	...
	*(int*)((char*)0x61572C + v8) |= a1;
	...
	sub_55718(
		...
		*(uint16_t*)(g_libGTASA + 0x8C9AB8),
		0xFF);
	sub_5578E(&v19);
	((void (*)(int, int*, int))((char*)0x1AD8A1 + g_libGTASA))(v9, &v19, 3);

	if (((int(__fastcall*)(int))(g_libGTASA + 0x463DF1))(v9) == 1)
	{
		*(int*)(g_libGTASA + 0x95B064 + 0x84) = *(int*)(g_libGTASA + 0x9B4C2C);
		*(float*)(g_libGTASA + 0x95B064 + 0x88) = *(float*)(g_libGTASA + 0x9B4C2C) * 100.0f;
		...
		*(uint8_t*)(v8 + 0x9B4CB2) = 1;
		...
		if (pChatWindow->isConnect && (g_libGTASA + 0x8A1144))
...
int sub_6F04C()
{
	...
	int v3;
	int result;
	int settings_WaterReflLvl = kDisableWaterReflections ? 0 : 3;

	if (!dword_748F80 || !dword_748F84 || !dword_748F88 || !dword_748F8C || !dword_748F90 || !dword_748F94 ||
		!dword_748C98 || !dword_748C94 || !dword_748C9C)
	{
		sub_6F160();
		jj = 1;
	}

	((void (*)())(g_libGTASA + 0x54DE1D))();
...
{
	int* v2 = *(int**)(0x669E48 + g_libGTASA);
	unsigned int v3 = *(int*)(a2 + result) - *v2;
	if (v3 / 0xC < (unsigned int)v2[2])
	{
...
{
	NvUtilInit();
	*(char**)(g_libGTASA + 0x5D1608) = "/storage/emulated/0/SportyRussia/Files/";
}
...
NOP(g_libGTASA + 0x25AB7A, 2); // back (text)
NOP(g_libGTASA + 0x25AA3E, 2); // back (sprite)
NOP(g_libGTASA + 0x25C7C6, 2); // back to pause
NOP(g_libGTASA + 0x25C7CA, 2); // sound back
NOP(g_libGTASA + 0x26B0EE, 2); // shadow bottom
NOP(g_libGTASA + 0x26B214, 2); // city name
...
bool isForMap = false;
void (*InitForPause) (uintptr_t* thiz);
void InitForPause_hook(uintptr_t* thiz) {
    if(callJavaMenu){
       g_pJavaWrapper->InitPause();
    }
    if(isForMap) InitForPause(thiz);
}
...
void OpenMap() {
    Log("[VISKI]: Opening Map...");
    
    g_pJavaWrapper->InitMaps();  
    
    isForMap = true;
    *(uint8_t *)(g_libGTASA + 0x9E7594) = 1;
    *(uint8_t *)(g_libGTASA + 0x63E0B5) = 1;
}

void CloseMap() {
    Log("[VISKI]: Closing Map...");
    isForMap = false;

    *(float*)(g_libGTASA + 0x63E0FC) = 0.0f;
    // These are adjacent flag bytes. Writing uintptr_t here corrupts nearby menu/render state.
    *(uint8_t*)(g_libGTASA + 0x63E09D) = 1;
    *(uintptr_t*)(g_libGTASA + 0x63E074) = 0;
    *(uintptr_t*)(g_libGTASA + 0x63E06C) = 0;
    *(uint8_t*)(g_libGTASA + 0x63E09C) = 0;
    *(uint8_t *)(g_libGTASA + 0x63E0B5) = 1;
...
Java_vortrexmobile_online_gui_pause_BrPause_openmap(JNIEnv *env, jobject thiz) {
    OpenMap();
}

extern "C" JNIEXPORT void JNICALL
Java_vortrexmobile_online_gui_pause_BrMapPause_closemap(JNIEnv *env, jobject thiz) {
    CloseMap();
}
...
{
	CGame__Process();
	ApplyCoreRuntimePatchesOnce();
	ApplyVisualExtrasOffOnce();
	ApplyPrettyVisualsOnce();
	ApplyPrettySkyPatchOnce();
	ApplyVehicleShadowModeOnce();
	ApplyCarReflectionModeOnce();
	ApplySkyVisualsOnce();

	static bool once = false;
	if (!once)
	{
		if (!kDisableCustomPlates)
			CCustomPlateManager::Initialise();
		if (!kDisableSnowEffects)
			CSnow::Initialise();
		once = true;
	}
	...
	if (pNetGame)
	{
		...
		if (pTextDrawPool && !kDisableSnapshotProcessing) {
	...
	if (pAudioStream && !kDisableAudioStreamProcessing)
...
int MobileSettings__GetMaxResWidth_hook()
{
	...
	return (int)( ((int(*)())(g_libGTASA + 0x0023816C + 1))()/1.08 );
}
...
void InstallHooks()
{
	...
	SetUpHook(g_libGTASA + 0x25C298, (uintptr_t)InitForPause_hook, (uintptr_t*)&InitForPause);
