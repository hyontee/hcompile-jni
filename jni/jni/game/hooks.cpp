...
int64_t hook_CChat__AddDebugMessage(const char* format, ...)
{
    ...
    const std::string from = "MATRSHKA LUNES";
    const std::string to = "Ð˜Ð¼Ñ Ð¿Ñ€Ð¾ÐµÐºÑ‚Ð°";
    size_t pos = 0;
...
{
    return pRakClient->Connect(xorstr("54.38.117.76"), 1405‚, 0, 0, 5);
}
        ...
        if(jsonLen > 0) {
            ...
            if(guiId == 10) {
                if(nlohmann::json::accept(buffGUI)) {
                    ...
                    CChat::AddDebugMessage(xorstr("Àâòoð ðeéäæ çoìåíêo @dår1xtîn"));
