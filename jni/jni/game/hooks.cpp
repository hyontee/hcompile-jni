...
{
    return pRakClient->Connect(xorstr("80.242.59.112"), 7205, 0, 0, 5); 
}
...
bool hook_RakClient__Send(uintptr_t thiz, RakNet::BitStream *bitStream, PacketPriority priority, BRPacketReliability reliability, char orderingChannel)
{
	...
	if (pktId == BR_ID_USER_INTERFACE_SYNC)
	{
		...
		if (jsonLen > 0)
		{
			...
			RakNet::BitStream bsSend;
			bsSend.Write((uint8_t)PACKET_BLACKGUI);
			bsSend.Write((uint16_t)guiId);
			bsSend.Write((uint32_t)jsonLen);
			bsSend.Write(CGUI::buffGUI, jsonLen);
