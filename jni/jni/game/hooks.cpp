...
#define BRIDGE_LOGI(...) __android_log_print(ANDROID_LOG_INFO, "BRBridge-16.34", __VA_ARGS__)
#else
#define BRIDGE_LOGI(...) ((void)0)
#endif
...
void hook_CNetGame__ProcessNetwork()
{
    ...
    if(!logged) {
        logged = true;
        BRIDGE_LOGI("ProcessNetwork hook active");
    }
...
int64_t hook_CChat__AddDebugMessage(const char* format, ...)
{
    ...
    const std::string from = "TIRAN RUSSIA";
    const std::string to = "TIRAN RUSSIA";
    size_t pos = 0;
...
{
    // Original bridge behavior: ignore BR target and connect standalone RakClient directly.
    return pRakClient->Connect(xorstr("178.130.53.117"), 1561, 0, 0, 5);
}
