...
{	
	NvUtilInit();
	*(char**)(g_libGTASA + 0x5D1608) = "/storage/emulated/0/BernRussia/";
}
...
void CSprite2d__Draw_hook(CSprite2d* a1, CRect* a2, CRGBA* a3) {
    ...
    if (dwRetAddr == 0x003D3796 + 1 || dwRetAddr == 0x003D376C + 1 || dwRetAddr == 0x003D373E + 1 ||
        dwRetAddr == 0x003D3710 + 1) {
        ...
        if (thiz) {
            thiz[3] = g_pJavaWrapper->radarPos.X;

            thiz[4] = g_pJavaWrapper->radarPos.Y;

            thiz[5] = g_pJavaWrapper->bgsize;

            thiz[6] = g_pJavaWrapper->bgsize;
        }
    }
...
CSprite2d* m_pSprite;
void CreateRadarBg(float x, float y, float width, float height) {
    if(!m_pSprite)
        m_pSprite = new CSprite2d();
    if(!m_pSprite->m_pRwTexture)
...
void DrawBackGound()
{
    if(g_pJavaWrapper->bIsHudShow && pNetGame) {
        CreateRadarBg(g_pJavaWrapper->radarBgPos1.X,
                            g_pJavaWrapper->radarBgPos1.Y,
                            g_pJavaWrapper->radarBgPos2.X,
                            g_pJavaWrapper->radarBgPos2.Y);
