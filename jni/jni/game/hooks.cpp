...
PED_MODEL PedsModels[8192];
int PedsModelsCount = 0;

PED_MODEL* (*CModelInfo_AddPedModel)(int id);
PED_MODEL* CModelInfo_AddPedModel_hook(int id)
{
	if (PedsModelsCount >= 8192)
	{
		Log("PedsModels overflow! id=%d, count=%d - модель пропущена, чтобы не портить память", id, PedsModelsCount);
		return nullptr;
	}
...
{	
	NvUtilInit();
	*(char**)(g_libGTASA + 0x5D1608) = "/storage/emulated/0/Wizzi/";
}
...
ATOMIC_MODEL AtomicModels[32000];
int AtomicModelsCount = 0;

ATOMIC_MODEL* (*CModelInfo_AddAtomicModel)(int id);
ATOMIC_MODEL* CModelInfo_AddAtomicModel_hook(int id)
{
	if (AtomicModelsCount >= 32000)
	{
		Log("AtomicModels overflow! id=%d, count=%d - модель пропущена, чтобы не портить память", id, AtomicModelsCount);
		return nullptr;
	}
...
VEHICLE_MODEL VehicleModels[8192];
...
{
	if (VehicleModelsCount >= 8192)
	{
		Log("VehicleModels overflow! id=%d, count=%d - модель пропущена, чтобы не портить память", id, VehicleModelsCount);
		return nullptr;
	}
