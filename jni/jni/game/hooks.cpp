...
stFile* NvFOpen_hook(const char* r0, const char* r1, int r2, int r3)
{
	...
	{
		sprintf(path, obfuscate("%s/SAMP/handling.cfg"), g_pszStorage);
		goto open;
	}

	if (!strncmp(r1, obfuscate("DATA/WEAPON.DAT"), 15))
	{
		sprintf(path, obfuscate("%s/SAMP/weapon.dat"), g_pszStorage);
		goto open;
	}*/

orig:
    Log(obfuscate("NvFOpen_hook | Open: %s"), r1);
...
open:
	...
	if(f)
	{
        Log(obfuscate("NvFOpen_hook | Redirect: %s"), path);
	...
	{
		Log(obfuscate("NvFOpen_hook | Error: %s (not found)"), path);
...
void InitialiseRenderWare_hook()
{
    ...
    Log(obfuscate("Loading \"samp\" texturedatabase..."));
	(( void (*)(const char*, int, int))(g_libGTASA+0x1BF244+1))(obfuscate("samp"), 0, 5);
...
void CStreaming_InitImageList_hook()
{
	...
	*(uint32_t*)&ms_files[380] = 0;
    ...
    Log(obfuscate("Loading \"gta3\" image..."));
	(( uint32_t (*)(char*, uint32_t))(g_libGTASA+0x28E7B0+1))(obfuscate("TEXDB\\GTA3.IMG"), 1);

    Log(obfuscate("Loading \"gta_int\" image..."));
	(( uint32_t (*)(char*, uint32_t))(g_libGTASA+0x28E7B0+1))(obfuscate("TEXDB\\GTA_INT.IMG"), 1);

    Log(obfuscate("Loading \"samp\" image..."));
	(( uint32_t (*)(char*, uint32_t))(g_libGTASA+0x28E7B0+1))(obfuscate("TEXDB\\SAMP.IMG"), 1);

    Log(obfuscate("Loading \"sampcol\" image..."));
	(( uint32_t (*)(char*, uint32_t))(g_libGTASA+0x28E7B0+1))(obfuscate("TEXDB\\SAMPCOL.IMG"), 1);
}
...
{	
	...
	*(char**)(g_libGTASA + 0x5D1608) = "/storage/emulated/0/Golda/";
}
...
//              (uintptr_t*)&GetTexture_orig);
	...
	SetUpHook(g_libGTASA + 0x258910, (uintptr_t)GetTexture_hook, (uintptr_t*)&GetTexture_orig);
	...
	//SetUpHook(g_libGTASA + 0x4D6F84, (uintptr_t)CAutomobile__BreakTowLink_hook, (uintptr_t*)&CAutomobile__BreakTowLink);
	//SetUpHook(g_libGTASA + 0x45F1A4, (uintptr_t)CPopulation__AddPed_hook, (uintptr_t*)&CPopulation__AddPed);
	...
	SetUpHook(g_libGTASA + 0x31B164, (uintptr_t)FxEmitterBP_c__Render_hook, (uintptr_t*)& FxEmitterBP_c__Render); //its maybe remove effect checkpoints...
