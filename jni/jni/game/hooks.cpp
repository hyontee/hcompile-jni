...
void (*CStreaming__RequestModel)(int modelId, int flags);
...
static int ClampGraphicValue(int value, int minValue, int maxValue, int defaultValue)
{
...
static void ApplyOpenGLRuntimeProfile()
{
...
static void ApplyCachedGraphicSettings()
{
	...
	int resolutionValue = ClampGraphicValue(ReadGraphicValue(filePath.c_str(), table.c_str(), "resolution", 80), 40, 100, 80);
	int effectsValue = ClampGraphicValue(ReadGraphicValue(filePath.c_str(), table.c_str(), "effects", 1), 0, 2, 1);
	int reflectionValue = ClampGraphicValue(ReadGraphicValue(filePath.c_str(), table.c_str(), "reflections", 0), 0, 3, 0);

	if (resolutionValue != lastResolution)
	{
		graphic.ApplyResolution(resolutionValue);
		lastResolution = resolutionValue;
	}

	if (effectsValue != lastEffects)
	{
		graphic.ApplyEffects(effectsValue);
		lastEffects = effectsValue;
	}

	if (reflectionValue != lastReflections)
	{
		graphic.ApplyReflections(reflectionValue);
		lastReflections = reflectionValue;
	}
}
...
bool IsModelInfoRegisteredForStreaming(int modelId)
{
	...
	uintptr_t* modelInfoPtrs = (uintptr_t*)(g_libGTASA + 0x87BF48);
	return modelInfoPtrs[modelId] != 0;
}

uintptr_t GetModelInfoPtrSafe(int modelId)
{
	if (modelId < 0 || modelId >= 20000)
	{
		return 0;
	...
	uintptr_t* modelInfoPtrs = (uintptr_t*)(g_libGTASA + 0x87BF48);
	return modelInfoPtrs[modelId];
}

uintptr_t GetModelColModelPtrSafe(int modelId)
{
	uintptr_t modelInfo = GetModelInfoPtrSafe(modelId);
	if (!modelInfo)
	{
		return 0;
	}

	return *(uintptr_t*)(modelInfo + 0x2C);
}

bool IsEntityModelSafeForBounds(ENTITY_TYPE* entity)
{
	if (!entity)
	{
		return false;
	...
	return GetModelColModelPtrSafe(entity->nModelIndex) != 0;
}

void CStreaming__RequestModel_hook(int modelId, int flags)
{
	if (!IsModelInfoRegisteredForStreaming(modelId))
	{
		Log("[AntiCrash] CStreaming::RequestModel skipped bad model id %d.", modelId);
		return;
	}

	CStreaming__RequestModel(modelId, flags);
}
...
PED_MODEL* CModelInfo_AddPedModel_hook(int id)
{
	if (id <= 0 || id >= 20000)
	{
		Log("[AntiCrash] CModelInfo_AddPedModel skipped bad id %d.", id);
		return nullptr;
	}

	if (PedsModelsCount >= (int)(sizeof(PedsModels) / sizeof(PedsModels[0])))
	{
		Log("[AntiCrash] CModelInfo_AddPedModel pool overflow at id %d. Increase PedsModels.", id);
		return nullptr;
	}
...
void (*CFileLoader__LoadPedObject)(const char* line);
void CFileLoader__LoadPedObject_hook(const char* line)
{
	if (!line)
	{
		Log("[AntiCrash] CFileLoader::LoadPedObject skipped null line.");
		return;
	}
	...
	char statsName[32] = { 0 };
	char animGroup[32] = { 0 };
	char carsCanDrive[32] = { 0 };
	int flags = 0;

	int fields = sscanf(line,
		" %d , %31[^,] , %31[^,] , %31[^,] , %31[^,] , %31[^,] , %31[^,] , %d",
		&modelId, modelName, txdName, pedType, statsName, animGroup, carsCanDrive, &flags);

	if (fields < 7 || modelId <= 0 || modelId >= 20000)
	{
		Log("[AntiCrash] CFileLoader::LoadPedObject skipped bad line: %.120s", line);
		return;
	}

	snprintf(safeLine, sizeof(safeLine), "%d, %s, %s, %s, %s, %s, %s, %d",
		modelId, modelName, txdName, pedType, statsName, animGroup, carsCanDrive, flags);

	CFileLoader__LoadPedObject(safeLine);
}
/* ====================================================== */
...
extern "C" bool NotifyEnterVehicle(VEHICLE_TYPE * _pVehicle)
{
	...
	if (!_pVehicle || !IsEntityModelSafeForBounds(&_pVehicle->entity) || !_pVehicle->entity.mat || !_pVehicle->entity.m_RwObject) return false;
...
bool IsVehicleSafeForEnterExit(VEHICLE_TYPE* vehicle)
{
	if (!vehicle || (uintptr_t)vehicle < 0x10000)
	{
		return false;
	}

	return vehicle->entity.mat &&
		vehicle->entity.m_RwObject &&
		IsEntityModelSafeForBounds(&vehicle->entity);
}

extern "C" void call_taskEnterCarAsDriver(uintptr_t a, uint32_t b)
{
	if (!a || !IsVehicleSafeForEnterExit((VEHICLE_TYPE*)b))
	{
		Log("[AntiCrash] CTaskComplexEnterCarAsDriver skipped bad vehicle %p.", (void*)b);
		return;
	}
...
{
	if (!thiz || !IsVehicleSafeForEnterExit(pVehicle))
	{
		Log("[AntiCrash] CTaskComplexLeaveCar skipped bad vehicle %p.", (void*)pVehicle);
		return;
	}
...
void CAnimManager__UncompressAnimation_hook(int a1, int a2)
{
	if (a1 && a1 > 0x10000)
	{
		uintptr_t anim = *(uintptr_t*)(a1 + 4);
		if (!anim || anim < 0x10000)
		{
			return;
		}
...
uint8_t g_DummyAnimAssociation[32] = { 0 };
uint8_t g_DummyAnimBlendHierarchy[16] = { 0 };
uint8_t g_DummyRtAnimAnimation[16] = { 0 };

uintptr_t GetDummyAnimAssociation()
{
	*(uintptr_t*)(g_DummyAnimAssociation + 16) = (uintptr_t)g_DummyAnimBlendHierarchy;
	*(uintptr_t*)(g_DummyAnimBlendHierarchy + 4) = (uintptr_t)g_DummyRtAnimAnimation;
	*(uint16_t*)(g_DummyRtAnimAnimation + 4) = 0;
	*(int16_t*)(g_DummyRtAnimAnimation + 6) = 0;
	*(uintptr_t*)(g_DummyRtAnimAnimation + 8) = 0;
	return (uintptr_t)g_DummyAnimAssociation;
}

bool IsAnimAssociationUsable(uintptr_t assoc)
{
	if (!assoc || assoc < 0x10000)
	{
		return false;
	}

	uintptr_t hierarchy = *(uintptr_t*)(assoc + 16);
	if (!hierarchy || hierarchy < 0x10000)
	{
		return false;
	}

	uintptr_t animation = *(uintptr_t*)(hierarchy + 4);
	if (!animation || animation < 0x10000)
	{
		return false;
	}

	int16_t frameCount = *(int16_t*)(animation + 6);
	if (frameCount > 0 && !*(uintptr_t*)(animation + 8))
	{
		return false;
	}
...
uintptr_t (*CAnimManager__GetAnimAssociation)(int group, int anim);
uintptr_t CAnimManager__GetAnimAssociation_hook(int group, int anim)
{
	uintptr_t assoc = CAnimManager__GetAnimAssociation(group, anim);
	if (!IsAnimAssociationUsable(assoc))
	{
		Log("[AntiCrash] Missing anim association group %d anim %d.", group, anim);
		return GetDummyAnimAssociation();
	}

	return assoc;
}

void (*CCarEnterExit__SetAnimOffsetForEnterOrExitVehicle)();
void CCarEnterExit__SetAnimOffsetForEnterOrExitVehicle_hook()
{
	CCarEnterExit__SetAnimOffsetForEnterOrExitVehicle();
}

void (*CAnimManager__CreateAnimAssocGroups)();
void SanitizeAnimAssocDefinitions()
{
	const uintptr_t gotBase = g_libGTASA + 0x5CD760;
	int* animAssocDefCount = *(int**)(gotBase + 0x2C10);
	uint8_t* animAssocDefs = (uint8_t*)(*(uintptr_t*)(gotBase + 0x2EA4));

	if (!animAssocDefCount || !animAssocDefs)
	{
		Log("[AntiCrash] CAnimManager::CreateAnimAssocGroups skipped sanitize: definitions unavailable.");
		return;
	}

	int count = *animAssocDefCount;
	if (count <= 0 || count > 512)
	{
		Log("[AntiCrash] CAnimManager::CreateAnimAssocGroups bad definition count %d.", count);
		return;
	}

	uintptr_t* modelInfoPtrs = (uintptr_t*)(g_libGTASA + 0x87BF48);
	uint8_t* def = animAssocDefs + 16;

	for (int i = 0; i < count; ++i, def += 48)
	{
		int* modelId = (int*)(def + 16);
		if (*modelId == -1)
		{
			continue;
		}

		if (*modelId < 0 || *modelId >= 20000 || !modelInfoPtrs[*modelId])
		{
			Log("[AntiCrash] Anim assoc group %d uses missing model %d, disabling clump creation.", i, *modelId);
			*modelId = -1;
		}
	}
}

void CAnimManager__CreateAnimAssocGroups_hook()
{
	SanitizeAnimAssocDefinitions();
	CAnimManager__CreateAnimAssocGroups();
}
...
ATOMIC_MODEL* CModelInfo_AddAtomicModel_hook(int id)
{
	if (id <= 0 || id >= 20000)
	{
		Log("[AntiCrash] CModelInfo_AddAtomicModel skipped bad id %d.", id);
		return nullptr;
	}

	if (AtomicModelsCount >= (int)(sizeof(AtomicModels) / sizeof(AtomicModels[0])))
	{
		Log("[AntiCrash] CModelInfo_AddAtomicModel pool overflow at id %d. Increase AtomicModels.", id);
		return nullptr;
	}
...
VEHICLE_MODEL* CModelInfo_AddVehicleModel_hook(int id)
{
	if (id <= 0 || id >= 20000)
	{
		Log("[AntiCrash] CModelInfo_AddVehicleModel skipped bad id %d.", id);
		return nullptr;
	}

	if (VehicleModelsCount >= (int)(sizeof(VehicleModels) / sizeof(VehicleModels[0])))
	{
		Log("[AntiCrash] CModelInfo_AddVehicleModel pool overflow at id %d. Increase VehicleModels.", id);
		return nullptr;
	}
...
void CAutomobile__ProcessEntityCollision_hook(VEHICLE_TYPE* a1, ENTITY_TYPE* a2, int a3)
{
	...
	if (!a1 || !IsEntityModelSafeForBounds(&a1->entity) || !IsEntityModelSafeForBounds(a2))
	{
		return;
	}
...
{
	if (!IsEntityModelSafeForBounds(thiz))
	{
		return 0;
	}
...
void (*CEntity__GetBoundRect)(RECT* out, ENTITY_TYPE* thiz);
void CEntity__GetBoundRect_hook(RECT* out, ENTITY_TYPE* thiz)
{
	if (!out)
	{
		return;
	...
	if (!IsEntityModelSafeForBounds(thiz))
	{
		...
		if (thiz)
		{
			Log("[AntiCrash] CEntity::GetBoundRect fallback for model %u.", thiz->nModelIndex);
		}
		return;
	}

	CEntity__GetBoundRect(out, thiz);
}
...
{
	if (!thiz || !IsEntityModelSafeForBounds(&thiz->entity) || !IsEntityModelSafeForBounds(ent))
	{
		return;
	}
...
void InstallHooks()
{
	...
	SetUpHook(g_libGTASA + 0x28EB10, (uintptr_t)CStreaming__RequestModel_hook, (uintptr_t*)&CStreaming__RequestModel);
	SetUpHook(g_libGTASA + 0x4054C8, (uintptr_t)CFileLoader__LoadPedObject_hook, (uintptr_t*)&CFileLoader__LoadPedObject);
	...
	SetUpHook(g_libGTASA + 0x0033DCE4, (uintptr_t)CAnimManager__GetAnimAssociation_hook, (uintptr_t*)&CAnimManager__GetAnimAssociation);
	SetUpHook(g_libGTASA + 0x0033E430, (uintptr_t)CAnimManager__CreateAnimAssocGroups_hook, (uintptr_t*)&CAnimManager__CreateAnimAssocGroups);
	SetUpHook(g_libGTASA + 0x00493484, (uintptr_t)CCarEnterExit__SetAnimOffsetForEnterOrExitVehicle_hook, (uintptr_t*)&CCarEnterExit__SetAnimOffsetForEnterOrExitVehicle);
	...
	SetUpHook(g_libGTASA + 0x00390AA8, (uintptr_t)CEntity__GetBoundRect_hook, (uintptr_t*)&CEntity__GetBoundRect);
