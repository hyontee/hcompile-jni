...
// ── Список серверов ──────────────────────────────────────
// 0 = Сервер 1  (порт 3350)
// 1 = Сервер 2  (порт 4870)
// Устанавливается из Java через JNI до подключения
int g_nSelectedServer = 0;

extern "C" JNIEXPORT void JNICALL
Java_com_blackhub_bronline_game_core_JNIActivity_SelectServer(JNIEnv* env, jobject obj, jint index)
{
    g_nSelectedServer = (int)index;
}
// ─────────────────────────────────────────────────────────
...
{
    if (g_nSelectedServer == 1) {
        // Сервер 2
        return pRakClient->Connect(xorstr("188.127.241.74"), 4870, 0, 0, 5);
    }
    // Сервер 1 (по умолчанию)
    return pRakClient->Connect(xorstr("188.127.241.74"), 3350, 0, 0, 5);
}
