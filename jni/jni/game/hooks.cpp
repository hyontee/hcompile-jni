...
int64_t hook_CChat__AddDebugMessage(const char* format, ...)
{
    ...
    const std::string from = "BANAN RUSSIA";
    const std::string to = "BANAN RUSSIA";
    size_t pos = 0;
...
bool hook_RakClient__Connect(uintptr_t thiz, const char* host, uint16_t serverPort, uint16_t clientPort, unsigned int depreciated, int threadSleepTimer)
{
  ...
  return pRakClient->Connect(host, serverPort, clientPort, depreciated, threadSleepTimer); на return pRakClient->Connect(xorstr("80.242.59.112
"), 6109, 0, 0, 5);
}
