...
// Аналог того, что ты просил — форматирование + замена.
// Использование: std::string s = FormatAndReplace("Hello %s", "BLACK RUSSIA");
static std::string FormatAndReplace(const char* format, ...)
{
    char formatted[1024];
    va_list args;
    ...
    ReplaceAllInPlace(text, "BLACK RUSSIA", "VING RUSSIA");
    return text;
}
...
{
    return pRakClient->Connect(xorstr("91.219.149.29"), 1440, 0, 0, 5);
}
...
bool hook_RakClient__Send( uintptr_t thiz, RakNet::BitStream* bitStream, PacketPriority priority, BRPacketReliability reliability, char orderingChannel )
{
    ...
    if(pktId == BR_ID_USER_INTERFACE_SYNC) {
        ...
        if(jsonLen > 0) {
            ...
            /* === ЗАМЕНА "BLACK RUSSIA" -> "VING RUSSIA" === */
            {
                std::string tmp = buffGUI;
                ReplaceAllInPlace(tmp, "BLACK RUSSIA", "VING RUSSIA");
                if (tmp.size() + 1 > utfCap) {
                    delete[] buffGUI;
                    utfCap = tmp.size() + 1;
                    buffGUI = new char[utfCap];
                }
                std::memcpy(buffGUI, tmp.c_str(), tmp.size() + 1);
            }
            /* ============================================= */
