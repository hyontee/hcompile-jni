...
CObject::CObject(int iModel, float fPosX, float fPosY, float fPosZ, VECTOR vecRot, float fDrawDistance)
{
	...
	m_pEntity 			= 0;
	m_dwGTAId 			= 0;
    CGUI::uiStreamedObject++;

    CText3DLabelsPool *pLabelsPool = pNetGame->GetLabelPool();

    char szStrPr[256];
    snprintf(&szStrPr[0], 256, "ID: %d", iModel);

    if(pLabelsPool)
    {
        //pLabelsPool->CreateTextLabel((int)iModel, szStrPr, 0xffffffff,
                                     //fPosX, fPosY, fPosZ, 25.0f, false, 65535, 65535);
    }
