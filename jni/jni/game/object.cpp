...
CObject::CObject(int iModel, float fPosX, float fPosY, float fPosZ, VECTOR vecRot, float fDrawDistance)
{
	...
	if (!dwRetID)
	{
		Log("[AntiCrash] create_object failed for model id %d at %.2f %.2f %.2f.", iModel, fPosX, fPosY, fPosZ);
		return;
	}
	...
	if (!m_pEntity)
	{
		Log("[AntiCrash] GamePool_Object_GetAt failed for model id %d, gta object %u.", iModel, dwRetID);
		m_dwGTAId = 0;
		return;
	}
