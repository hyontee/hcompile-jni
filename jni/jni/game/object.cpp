...
{
    float fSX = (matPos->pos.X - m_matTarget.pos.X) * (matPos->pos.X - m_matTarget.pos.X);
    float fSY = (matPos->pos.Y - m_matTarget.pos.Y) * (matPos->pos.Y - m_matTarget.pos.Y);
    float fSZ = (matPos->pos.Z - m_matTarget.pos.Z) * (matPos->pos.Z - m_matTarget.pos.Z);
    ...
    if (GamePool_Object_GetAt(m_dwGTAId))
    {
        if (!ScriptCommand(&is_object_attached, m_dwGTAId))
        {
            ScriptCommand(&attach_object_to_car, m_dwGTAId, pVehicle->m_dwGTAId, m_vecAttachedOffset.X,
                          m_vecAttachedOffset.Y, m_vecAttachedOffset.Z, m_vecAttachedRotation.X,
                          m_vecAttachedRotation.Y, m_vecAttachedRotation.Z);
        }
    }
}
