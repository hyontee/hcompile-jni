...
{
	if (nModelIndex < 0 || nModelIndex >= 20000)
	{
		return 0;
	}
...
void WorldAddEntity(uintptr_t pEnt)
{
	...
	ENTITY_TYPE* entity = (ENTITY_TYPE*)pEnt;
	if (entity->nModelIndex >= 20000)
	{
		Log("[AntiCrash] WorldAddEntity skipped bad model id %u.", entity->nModelIndex);
		return;
	}

	uintptr_t* dwModelArray = (uintptr_t*)(g_libGTASA + 0x87BF48);
	uintptr_t modelInfo = dwModelArray[entity->nModelIndex];
	if (!modelInfo || !*(uintptr_t*)(modelInfo + 0x2C))
	{
		Log("[AntiCrash] WorldAddEntity skipped entity without col model, model %u.", entity->nModelIndex);
		return;
	}
