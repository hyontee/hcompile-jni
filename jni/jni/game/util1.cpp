...
void RwMatrixRotate(RwMatrix* mat, eAxis axis, float angle)
{
...
eWidgetState ProcessFixed(WIDGET_TYPE* pWidget)
{
	...
	switch (idWidget)
	{
		...
		case TYPE_ENTERCAR:
		{
			...
			{
				VEHICLEID idPoolVeh = CVehiclePool::FindNearestToLocalPlayerPed();
				if(idPoolVeh != INVALID_VEHICLE_ID) 
				{
					if(GetDistanceBetween3DPoints(pGame->FindPlayerPed()->m_pEntity->mat->pos, CVehiclePool::GetAt(idPoolVeh)->m_pEntity->mat->pos) > 10.0f)
							return STATE_FIXED;
				}
			}
			break;
