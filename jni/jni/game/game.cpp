...
static bool IsValidGameModelId(int modelId)
{
...
{
	if (!IsValidGameModelId(iType))
	{
		Log("[AntiCrash] Bad vehicle model id %d. Spawn skipped: model is outside limits or not loaded in CModelInfo.", iType);
		return nullptr;
	}
...
{
	if (!IsValidGameModelId(iModel))
	{
		Log("[AntiCrash] Bad object model id %d. Object skipped: model is outside limits or not loaded in CModelInfo.", iModel);
		return nullptr;
	}

	CObject *pObjectNew = new CObject(iModel, fPosX, fPosY, fPosZ, vecRot, fDrawDistance);
	if (!pObjectNew || !pObjectNew->m_pEntity)
	{
		Log("[AntiCrash] Object wrapper for model id %d has no GTA entity. Spawn skipped.", iModel);
		delete pObjectNew;
		return nullptr;
	}
