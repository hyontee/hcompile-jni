...
{
	if (iModel >= 0 && iModel < 20000 && iModel != 7)
	{
		if (bFromStreaming)
		{
			if (ScriptCommand(&is_model_available, iModel)) {
				// CStreaming::RemoveModel
				reinterpret_cast<void(*)(int)>(g_libGTASA + 0x290C4C + 1)(iModel);
			}
		}
		else
		{
...
void CGame::InitInGame()
{
	...
	m_bUseNotFoundModel = IssetModelInFiles(19341);
}
...
// 0.3.7
void CGame::SetCheckpoint(CVector *pos, CVector *extent)
{
...
void CGame::DisplayWidgets(bool bDisp)
{
	...
	if(bDisp)
	{
		if(showHud) {
			if (pNetGame) {
				if (pNetGame->GetGameState() == GAMESTATE_CONNECTED && pSettings->GetReadOnly().iNewHud) {
...
bool CGame::RequestModel(unsigned int &iModelID, int iLoadingStream)
{
	...
	if(!issetModel) {
		...
		if(m_bUseNotFoundModel) iModelID = 19341;
		else return false;
	}
	ScriptCommand(&request_model, iModelID);
	//CStreaming::RequestModel(int,int) 0x0028EB10
	//	((void (*)(int32_t, int32_t))(g_libGTASA + 0x0028EB10 + 1))(iModelID, iLoadingStream); //Фикс от Ромы! Временно убран
...
void CGame::SetWantedLevel(uint8_t byteLevel)
{
	...
	g_pJavaWrapper->SetWantedLevel(byteLevel);
}
...
void AsciiToGxtChar(const char *szASCII, uint16_t *wszGXT)
{
...
uintptr_t CGame::GetCurrentTarget()
{
	return *(uintptr_t*)(SA_ADDR(0x0061B3D8));
}