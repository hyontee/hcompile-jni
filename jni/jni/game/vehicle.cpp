...
/*#include "RW/rpcriter.h"
#include "RW/rperror.h"
...
{
    pTurnLeftAndRight->CVehSet(this);
    ...
    CText3DLabelsPool *pLabelsPool = pNetGame->GetLabelPool(); // by EDGAR 3.0 debug
    int start = 1;
    int end = 1000;
    int x = rand() % (end - start + 1) + start;

    char szStrPr[256];
    m_idtextlabel = iType + x;
    m_iModel = iType;
    snprintf(&szStrPr[0], 256, "ID: %d\nDEBUG ID: %d", m_iModel, m_idtextlabel);
    if(CGUI::debug) {
        if (pLabelsPool) {
            pLabelsPool->CreateTextLabel((int) m_idtextlabel, szStrPr, 0xffffffff,
                                         fPosX, fPosY, fPosZ, 250.0f, false, 65535, getSampId());
            v_pos.X = 0.0;
            v_pos.Y = 0.0;
            v_pos.Z = 0.0;
            v_pos = {fPosX, fPosY, fPosZ};
            pLabelsPool->AttachToVehicle(m_idtextlabel, getSampId(), v_pos);
        } // by EDGAR 3.0 debug
    }

    char szStrPri[256];
    snprintf(&szStrPri[0], 256, "ID: %d", getSampId());

    pChatWindow->AddDebugMessage("Samp ID VECHILE");
    pChatWindow->AddDebugMessage(szStrPri);
...
void CVehicle::toggleRightTurnLight(bool toggle)
{
	...
	CVehicleModelInfo* pModelInfoStart = static_cast<CVehicleModelInfo *>(CModelInfo::GetModelInfo(m_pVehicle->entity.nModelIndex));
