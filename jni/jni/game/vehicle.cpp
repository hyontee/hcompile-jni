...
CVehicle::CVehicle(int iType, float fPosX, float fPosY, float fPosZ, float fRotation, bool bSiren)
{
	...
	if (m_pVehicle && m_pVehicle->entity.m_RwObject)
	{
...
{
    if(!m_pVehicle) return INVALID_VEHICLE_ID;
...
{
	if (m_pTrailer && m_pTrailer->m_dwGTAId && m_dwGTAId)
	{
...
{
	if (!m_pVehicle || !m_dwGTAId) return false;
