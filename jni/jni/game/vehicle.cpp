...
CVehicle::~CVehicle()
{
	...
	if(pNumberPlateTexture) {
		RwTextureDestroy(pNumberPlateTexture);
		pNumberPlateTexture = nullptr;
	}
...
CVehicle* CVehicle::GetTrailer()
{
	...
	if (pNetGame && dwTrailerGTAPtr) {
		VEHICLEID TrailerID = (VEHICLEID)CVehiclePool::FindIDFromGtaPtr((VEHICLE_TYPE*)dwTrailerGTAPtr);
		if (TrailerID < MAX_VEHICLES && CVehiclePool::GetAt(TrailerID)) {
			return CVehiclePool::GetAt(TrailerID);
		}
	}
...
void CVehicle::CreatePlate(ePlateType eType, char* szNumber, char* szRegion)
{
    if(pNumberPlateTexture) {
        pNumberPlateTexture = CCustomPlateManager::createRuPlate("","");
    }

    Log("createplate %s %s %d", szNumber, szRegion, eType);
    pNumberPlateTexture = CCustomPlateManager::createTexture(eType, szNumber, szRegion);
}
