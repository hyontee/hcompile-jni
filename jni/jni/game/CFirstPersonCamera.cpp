...
void CFirstPersonCamera::ProcessCameraOnFoot(uintptr_t pCam, CPlayerPed* pPed)
{
    ...
    {
        ChangeFloatValue(g_libGTASA + 0x037D958, 65.0); // FOV
        return;
    }

    VECTOR* pVec = (VECTOR*)(pCam + 372);
    VECTOR vecOffset = { 0.35f, 0.1f, 0.1f };

    MATRIX4X4 matCamera;
    pGame->GetCamera()->GetMatrix(&matCamera);

    VECTOR camForward;
    camForward.X = matCamera.at.X;
    camForward.Y = matCamera.at.Y;
    camForward.Z = 0;

    float heading = atan2(-camForward.X, camForward.Y) * (180.0f / M_PI);
    heading = fmod(heading + 360.0f, 360.0f);
    pPed->SetRotation(heading);

    ChangeFloatValue(g_libGTASA + 0x037D958, 100.0); // FOV
